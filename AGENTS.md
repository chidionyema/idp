...
| Reloader is off on a workload a custom resource owns; that operator owns its config rollout, and Reloader on top of an operator is a loop, not a control | crew#684 rung 2 | `kyverno` | tests/fixtures/reloader/operator-owned.yaml | tests/fixtures/reloader/estate-owned.yaml |
