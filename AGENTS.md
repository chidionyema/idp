...
| Every surface the founder can open survives losing one node | docs/reference/policy/availability-standard.md | `python3 bin/idp-availability-gate` | tests/fixtures/availability/bad | tests/fixtures/availability/good |
...
| Reloader is off on a workload a custom resource owns; that operator owns its config rollout, and Reloader on top of an operator is a loop, not a control | crew#684 rung 2 | `kyverno` | tests/fixtures/reloader-blind | tests/fixtures/reloader |
...
| A release lock the founder wrote is measured against the estate rather than believed: a freeze issue open on the board while a workflow it names can still run is refused | LAW 2, LAW 28, crew#739 | `python3 bin/idp-freeze` | tests/fixtures/freeze/breached.json | tests/fixtures/freeze/honoured.json |
