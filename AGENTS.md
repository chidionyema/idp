...
| A change is proven in the shadow dimension only when the workload still converges after it is applied: ready, at its previous replica count, its probes green and its own tests green; a shadow observation silent on any assertion is a fail-closed FAIL, never a pass | W2.2 | `python3 bin/idp-shadow-verify` | tests/fixtures/shadow-verify/not-ready.json | tests/fixtures/shadow-verify/converged.json |
