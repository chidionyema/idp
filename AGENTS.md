...
| The router refuses a single call carrying more than a working turn, and the module, its callback registration, its mount and its import path must all be present together -- a ceiling that exists on disk with nothing loading it refuses nothing, which is exactly the state the estate was in when one key sent 86,525,924 tokens through an hour | LAW 2, incident 2026-09-13 | `python3 bin/idp-request-ceiling` | — | — |
| Spend is reported PER KEY, naming the biggest single call and the share of budget already spent -- because the estate-total check that already existed cannot see one session sending 737,169 tokens per call, and LiteLLM stops a key at 100% with no warning at all | LAW 2, incident 2026-09-13 | `python3 bin/idp-spend-floor` | — | — |
