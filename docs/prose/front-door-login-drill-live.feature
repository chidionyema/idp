...
Feature: A drill signs in at the front door and sees the catalogue
  ...
  Scenario: Every published path answers (crew#307)
    Given the drill is signed in to the catalogue
    When it visits every published sidebar path
    Then a body containing "PAGE NOT FOUND" or "ERROR 404" fails that path
    And a page that throws a JavaScript error fails that path
    And the drill prints "paths N/M" and fails once listing every broken path
