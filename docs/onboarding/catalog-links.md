...
- The two hostnames come from `http_hostnames()` on the observability layer's `spec.path`
  (the route files sitting in that path). No hostname is typed as a literal in the generator —
  the no-hardcoding law — and the `${ESTATE_ZONE}` placeholder rides through until Flux
  substitutes it at apply time from the cluster's `estate-config` ConfigMap.
...
a new observability service, give it a hostname in the observability layer's routes and teach
