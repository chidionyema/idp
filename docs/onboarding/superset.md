...
3. The Langfuse data source and the "Boardroom" dashboard are already there — the
   `superset-boardroom-seed` job adds them from git, and re-runs whenever the seed
   changes. Trace data lives in ClickHouse (host
   signoz-clickhouse.observability.svc.cluster.local, database `langfuse`); Langfuse's
   postgres holds app metadata only.
4. Open "SQL Lab" to write a query, or "Datasets" to register a table for point-and-click charts.
5. Two datasets are seeded over the trace store: `model_calls` has one row per model call
   (model, prompt, cost, latency, from the `observations` table) and `trace_activity` one
   row per traced request (from the `traces` table).
