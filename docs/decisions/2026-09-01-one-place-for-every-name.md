...
HAPPEN AGAIN" / "Needs monitoring for any drift" / "and PR rejection". Recorded as ruling R70 on
crew#796. Nothing in this record touches the cluster; every change is a branch he merges.
...
- `mumchimp.com` is registered at 123-Reg, expires 2027-06-16; its nameservers are Cloudflare
  (tony/danica). Records are created by external-dns on the cluster from HTTPRoute hostnames
...
- The platform already had the one place since crew#269: `clusters/oke/estate-config.yaml`
...
The gate existed (crew#269) but graded only idp `platform/`. A rule that grades one repository is a
wish for the other six (LAW 44). The class on the ledger is `gate-landed-after-branch` seen sideways:
the gate landed after the store was written and never widened. So the fix is the existing gate,
widened, reused by every plane that can refuse (LAW 3, LAW 45), never a second script.

## What now refuses (all one file, `bin/estate-zone-gate`, one exemption list)

1. **Pull-request rejection.** `bin/pr-report` saves the PR diff, runs the gate in `--diff` mode
   ...
   else's debt: only what the PR adds is graded.
...
4. **Exemptions are loud.** A file may opt out with `estate-zone-gate: exempt (<why>)` in its
   ...
   `APPROVE: zone-exempt` on the PR (`input.pr.approvals`).
...
4. Flip `ESTATE_ZONE` in `clusters/oke/estate-config.yaml` on a branch, set `vars.ESTATE_ZONE`, he
   merges: Flux reconciles every HTTPRoute, external-dns creates the new records, cert-manager
   issues the new certificates. The store's overlay follows in the same reconcile.
...
- Set the repository variable `ESTATE_ZONE` once (`gh variable set ESTATE_ZONE --body <zone>` on
  each repository, or organisation-wide after the crew#785 move); until then the store's
  workflows that read `vars.ESTATE_ZONE` refuse with a plain message rather than guessing.
- The registry (`ghcr.io/chidionyema`) and organisation names follow the same shape in crew#785 A6;
...
- Board: crew#796 (this), crew#785 (org and registry names), crew#269 (the original ruling).
