...
Not architecture. Not money either, which the first draft got wrong.
...
Two pods hold most of it, and both are in `observability`:
...
So the shape of the problem is inverted from the first draft. The workload that actually consumes
CPU is under-declared and being squeezed; two neighbours that consume almost none are holding 2000m
between them, which is 17 percent of the cluster. The scheduler is full on paper while the machines
are half idle, and nothing new can be placed — including Crossplane.

**The precondition is right-sizing three requests, not buying a node pool.** That is free, and it
is the same root cause as the ClickHouse ceiling and the silent collector a peer session reported on
2026-09-07. Those pods are that session's lane; the measurement was handed over rather than acted
on. Until it lands, this record is the design and nothing is installed.
