...
machine's keyboard is a defect — the standing law that the founder does not run scripts. Credentials rotate themselves; after the one Bitwarden
seed he never touches one again.

## Decision

1. **Workload identity over pasted secrets, everywhere a provider supports it.**
   GitHub Actions authenticates to OCI by exchanging its own signed single sign-on token — no
   stored cloud key.
...
2. **The Bitwarden seam is one machine-account token, and that is the whole manual surface.**
   ...
   no single sign-on, no Kubernetes service-account federation
   ...
   — the one-root-then-code rule — and the record says so honestly rather than promising federation the vendor does not sell.
3. **No new pasted credential, ever.** A new provider is admitted only if (a) it federates via
   single sign-on or workload identity, or (b) its root is minted by code from a root we already hold. A pinned
   message asking a person to run a seeding command is the rejected shape; agents seed repository and
...
6. **Already enforced, for the record:** agents write declarative YAML into git only and operators
   apply the declared state (permanent ruling 2026-09-01, "agents never deploy"; Flux is that
   operator), and
