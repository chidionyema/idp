...
(`platform/oci/*.tf`, the one-root-then-code rule) and External Secrets pulls them through the one
...
wherever his browser put them. Chat is banned as a carrier — no secret ever rides a message; nothing else was named.
...
- **Writing our own Bitwarden sync script** — never reinvent a wheel a mature tool already
  ships and do a worse job; the bridge already exists inside the operator.
