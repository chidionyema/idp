# The inventory is read from the control planes, never from git (crew#740)
...
already had three inventories (`bin/catalog-platform` from the Flux manifests, the LAW 39 Mac
...
2. Per-plane shell loops over each vendor CLI — rejected under LAW 43; Steampipe already models
...
is adopted (imported into Terraform) or killed. The next PR (E1) makes the nightly run a required
check with `--strict`. The read-only cloud principal for this job is minted in Terraform in the
E2 PR with the other agent-scope work; until then it runs on the oke-check identity, which is the
one the founder's blueprint allows for the first run.

**Architecture laws.** LAW 39 (inventory every asset), LAW 43 (Steampipe, not a script per API),
LAW 50 (coverage is proved by querying the backend, never by scanning files), LAW 21 (read-only,
no root in the file; credentials come from the vault, R52).
