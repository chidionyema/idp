...
Five changes, in order, each its own pull request with its own fixture pair. Written to be
implemented without asking questions.

Founder instruction, verbatim, recorded at
...
And the instruction that ordered this spec rather than a consultant,
...
The answer this spec encodes: the shape is not the unknown — a vendor-owned tenant beside a
separate control-plane identity is what AWS, Stripe and Auth0 all converge on, and nobody needs
paying to redraw it. What is unknown is whether **our** estate actually holds the line, and no
consultant can measure that. So decision 0021's own diligence test stops being prose and becomes
the acceptance gate of change 5.
...
1. **One secret, one tenant.** A `secret_ref` or `outbound_secret_ref` that appears under two
   different `tenant_id` values is a FAIL. This is decision 0021 rule 4, and it is the check that
   would have caught the original merge.
...
## Change 5 — the diligence test, executable

This is the change the founder's consultant question actually asked for, and it is the only one
that produces new knowledge rather than new structure.
...
is what stops the split regressing while the slower changes land. 5 last, because it grades all
four. Each is one pull request; none is large; none needs a decision that is not already made.
