...
backend, and on layers that send model traces (Hermes, the model gateway) **Model traces**
opens the tracing backend. One tap from "what is this thing" to "what is it doing right now".
...
that were never sent. Regenerate with `bin/catalog-platform`; the check mode fails CI when the
committed file drifts from what the cluster manifests say.
