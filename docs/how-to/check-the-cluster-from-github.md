...
idp#514 by hand; `helm-retry` resets every Failed HelmRelease with `flux reconcile --reset`), and applies the original list again from an `EXIT` trap, so a failed playbook
