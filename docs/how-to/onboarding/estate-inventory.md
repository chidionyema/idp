...
- `bin/idp-inventory` reads each plane from its own control plane. The cloud plane rides on
  ...
  labels, owner references and each Kustomization's applied inventory; the three SaaS planes are
...
- A plane that cannot be read prints `BLIND inventory <plane>` and the table says UNKNOWN for it.
...
## Not done yet
- Audit mode: no red row fails the run. E1 (next PR) turns on `--strict` as a required check.
- The read-only cloud principal is a Terraform change in the E2 PR; this first run uses the
  oke-check identity.
