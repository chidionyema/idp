...
- `bin/estate-diagram --check` is the gate: it exits 1 when `docs/explanation/architecture/live.md` is not
  what the catalogue says, 3 (BLIND) when there is no catalogue.
- Never edit `docs/explanation/architecture/live.md`. Change the inventory collector or `bin/catalog-gen`
  and re-render.
