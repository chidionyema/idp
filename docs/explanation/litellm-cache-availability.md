...
The availability standard (crew#555) refuses any one-pod workload in a founder-facing
namespace, because one pod is one node event away from an outage. For almost everything
...
llm namespace is untouched, and the router itself stays at two spread replicas. It
lives in `platform/edge` beside the estate's other exceptions because exceptions bind
only from the kyverno namespace and that directory applies independently of the layer
