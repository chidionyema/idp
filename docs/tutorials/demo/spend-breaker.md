...
`spend-velocity-check` CronJob in the `llm` namespace; you can also trigger one run by hand:
...
Every hour on the hour the `spend-digest` CronJob writes a one-page spend summary from the
