...
because the operating model (crew#286, `docs/reference/policy/enterprise-operating-model.md`)
...
`docs/reference/policy/` without an `Approval-word:` line (that rule was retired on 2026-08-27; a
`DENY: <word>` from the founder still refuses):

```
$ bin/pr-report 155
rule=founder_approval_required | the PR changes a founder-facing surface (backstage/, platform/identity/, platform/edge/, docs/reference/policy/, estate-defaults.yaml) and names no approval word | fix: add a line `Approval-word: <word>` to the PR body; the founder replies `APPROVE: <word>` or `DENY: <word>`
