...
`~/.estate/state/inventory.json` by `bin/catalog-gen`) and writes `docs/explanation/architecture/live.md`.

```
$ bin/catalog-gen && bin/estate-diagram
estate-diagram: 129 lines -> docs/explanation/architecture/live.md
$ bin/estate-diagram --check
ok    estate-diagram: .../docs/explanation/architecture/live.md matches the catalogue
$ echo "hand drawn" >> docs/explanation/architecture/live.md && bin/estate-diagram --check; echo rc=$?
