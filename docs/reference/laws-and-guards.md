...
Generated by `bin/idp-laws-guards-report` from the laws file and the guards tree at `c20e586`. Do not edit by hand; run the script and commit. Every guard runs through one door, `opa-hook.py` (crew#603): `settings.json` names one command per event and `policy/adapters.rego` holds the order below.
...
## PreToolUse: 8 guards, in this order
...
| 6 | `rule-guard.py` | PreToolUse guard: turn written rules into refusals | 0, 25, 46 | Bash |
| 7 | `ticket-gate.py` | Every session carries a GitHub issue, or it does not get to change anything | 32, 33 | Bash, Edit, Write, MultiEdit, NotebookEdit |
...
| # | Guard | What it does | Laws it cites | Tools |
|---|---|---|---|---|
| 1 | `secret-scrub.py` | Keep live credentials out of the files agents and shells write | 21, 28 | every tool |
| 2 | `laws-link-guard.py` | Keep every provider's rules file pointing at ~/AGENTS.md | 24 | every tool |
| 3 | `jargon-guard.py` | Refuse a reply that talks to the founder in jargon | — | every tool |
| 4 | `vendor-lock-guard.py` | vendor-lock-guard: no plan step, checkpoint or instruction may require a vendor-only channel | 34 | every tool |
...
Total: 35 guard rows across four events.
