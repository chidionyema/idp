...
Generated by `bin/idp-laws-guards-report` from the laws file, the guards tree at `0ef86d5` and the harness settings file. Do not edit by hand; run the script and commit. The settings file names, per hook event and in the order below, every command the harness runs; `hook-run.py` runs each guard and writes its ledger row, and `opa-hook.py` is one of the guards, not a door in front of them.
...
## SessionStart: 7 guards, in this order

| # | Guard | What it does | Laws it cites | Tools |
|---|---|---|---|---|
| 1 | `laws-link-guard.py` | Keep every provider's rules file pointing at ~/AGENTS.md | 24 | every tool |
| 2 | `peer-loop-fence.py` | Refuse a peer message that re-raises something already on the estate board | 12 | every tool |
...
| 6 | `friction-relay.py` | Carry the founder's complaints to every session, not just the one that heard them | 6, 9 | every tool |
| 7 | `feed-guard.py` | feed-guard: every session writes a six-line handoff to ~/.estate/feed.md every 30 minutes | 28 | every tool |
...
| 11 | `opa-hook.py` | Ask OPA about a hook payload. Decides nothing itself | 38 | Artifact |
...
| # | Guard | What it does | Laws it cites | Tools |
|---|---|---|---|---|
| 1 | `opa-hook.py` | Ask OPA about a hook payload. Decides nothing itself | 38 | every tool |
...
Total: 38 guard rows across 5 events.
