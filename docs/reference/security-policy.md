...
| Admission policy is code and is proved both ways before merge | A.8.9 | `bin/idp-rules run --only licence-policy,placement-policy` | PASS | none |
...
| `require-auto-reload` | Audit 2, Enforce 1 | Every workload restarts when its config changes | `edge` | `platform/edge/require-auto-reload.yaml` |
