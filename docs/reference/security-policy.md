...
| Admission policy is code and is proved both ways before merge | A.8.9 | `bin/idp-ci` (kyverno rung) | PASS, and both ways: the rung renders every chart under `bin/idp-kyverno-dirs` and requires the unpatched-chart fixture to be **refused**, so a policy that admits everything fails it | judges before the PR; Kyverno itself judges at admission, after merge. BLIND (rc 2), never a pass, when `helm`/`kyverno` are absent |
