...
| `hermes-agent-env` (vendor keys, Telegram) | platform/hermes-agent/gateway.yaml | Anthropic, OpenRouter, Exa, Cursor, Telegram | same registry; the bot token is `SEED_TELEGRAM_HERMES_BOT_TOKEN`, made once with BotFather; Cursor is `SEED_CURSOR_API_KEY` (crew#751) | MEETS | `bin/idp-bootstrap-vendors` |
