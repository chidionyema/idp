...
| `otto-staging-telegram` | platform/otto-staging/telegram-secret.yaml, platform/otto-staging-secret/webhook-substitution.yaml | Telegram (BotFather) | `token` is born on the founder's phone in BotFather and goes into the human secret store on his phone (decision 0017), bridged to the vault by code — never a terminal, a file or a chat; `webhook_secret` is minted in-process and never printed | MEETS | `bin/idp-vault-put` |
