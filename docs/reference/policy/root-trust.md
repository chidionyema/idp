...
| `notify-apprise-founder-telegram` | platform/notify/external-secret.yaml | Telegram (founder's bot) | a pair of repository secrets set once (`SEED_NOTIFY_TELEGRAM_BOT_TOKEN` + `SEED_NOTIFY_TELEGRAM_CHAT_ID`, the bot made once at BotFather under the one-root-credential rule), graded together by `getChat`, `--merge` → vault in the apply run; the ExternalSecret composes the `tgram://` URL | MEETS | `bin/idp-bootstrap-vendors` |
