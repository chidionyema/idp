...
| `signoz-prover` | .github/workflows/verdict-signoz.yml (bin/idp-prove signoz, crew#631 CP9) | SigNoz | root login from the vault → `POST /api/v1/service_accounts/{id}/keys` over a port-forward → vault in-process | MEETS | `bin/idp-signoz-key` (bin/idp-estate-seed step 4) |
