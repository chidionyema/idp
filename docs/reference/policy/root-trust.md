...
| `cloudflare-workers-token` | platform/lifeboat (Otto's home 2 at the edge) | Cloudflare | Operator | the same root, then a Workers token via `POST /user/tokens` carrying Workers Scripts Write + Workers AI Write on the account and Workers Routes Write on the zone; proved by listing the account's scripts before the vault write | MEETS | `bin/idp-bootstrap-cloudflare` |
...
| `cyrus` (`LITELLM_API_KEY`) | platform/cyrus/external-secret.yaml | estate router | Operator | `POST /key/generate` via `bin/idp-router-key --entry cyrus`, scoped to the minimax, minimax_m27, gemini, default and fast lanes; the OpenCode engine's one provider, so every alias the router declares is a model an issue can pick (founder 2026-09-09) | MEETS | `bin/idp-estate-seed` |
...
| `jit-broker` | platform/jit/deployment.yaml | Estate | Operator | `JIT_SIGNING_KEY` minted by `bin/idp-vault-put jit-broker JIT_SIGNING_KEY=...` from a value `secrets.token_urlsafe` generates; never typed by a human and never outside the vault. Nothing else in this entry is read: the bot token, the chat id AND the webhook secret all come from `flux-telegram`, because the broker collects his tap from a mirrored copy of the webhook otto-gateway registered on that one bot (platform/otto-gateway/telegram-mirror.yaml), and Telegram signs each delivery with the secret that registration used. A second bot token, or a webhook secret of the broker's own, would be a second copy of one credential and would refuse every real tap (LAW 54). `JIT_AGENT_KEY` is minted the same way (`bin/idp-vault-put --merge jit-broker JIT_AGENT_KEY=...` over a `secrets.token_urlsafe` value) and is the weakest key in the entry on purpose: presenting it at the broker's `/identity` door buys a one-hour `agent-reader` token, which is reads, and nothing else. It exists because WJ.1's last clause -- "`bin/idp-kube` stops minting the founder's OCI principal" -- could not be met while the only credential on a laptop able to create an agent identity was the founder's own OCI principal. `--merge`, so the entry's other two keys are kept rather than re-seeded. | MEETS | `bin/idp-vault-put` |
...
| `cyrus-linear` `cyrus-linear-client-id` `cyrus-linear-client-secret` | platform/cyrus/external-secret.yaml | Linear | Customer | The OAuth client id and secret are two plain human-vault entries, one value each, born at Linear Settings, API, OAuth applications (no create-API exists). The earlier claim on this row was wrong: Linear does publish a create-API for the webhook half (`webhookCreate`), so only ONE of the two is human-born. The personal API key is, and can be born nowhere else — Settings, Account, Security & access, New API key, scoped rather than full access. It rides the human road: born in the founder's browser, saved into Bitwarden, pulled by the cluster through the `human-vault` store, never typed into a terminal or an estate env file. `webhook_secret` is then minted by code through `webhookCreate` using that key and written to the machine vault, so there is no second console visit (R52). MISS until the key is in Bitwarden; the manifest is already pointing at it. Since 2026-09-09 the same item also carries the OAuth client pair (`client_id`, `client_secret`, from Linear Settings, API, OAuth applications; no create-API), read by `cyrus-linear-oauth` in the same file: `cyrus self-auth` mints the access token from it after one consent click, and the token lives on the pod's claim, never in a vault. | MISS | [the Cyrus board ticket](https://github.com/chidionyema/crew/issues/834) |
...
| `DEEPSEEK_API_KEY` `MINIMAX_API_KEY` `MOONSHOT_API_KEY` `OPENROUTER_API_KEY` `GEMINI_API_KEY` `GROQ_API_KEY` `EXA_API_KEY` `CURSOR_API_KEY` | platform/human-vault-bridge/externalsecrets.yaml | DeepSeek, Minimax, Moonshot, OpenRouter, Google, Groq, Exa, Cursor | Customer | The founder's own vendor keys on the human road: born in his browser at each vendor's console, held in Bitwarden, read by the cluster through the `human-vault` ClusterSecretStore and never typed into a terminal or an estate env file (decision 0017). `platform/human-vault-bridge/pushsecrets.yaml` seeds Bitwarden from the entries the estate already holds so the first read has something to find, but that is a migration, not a birth: no code can mint a vendor key, so no bootstrapper can exist for these and the row stays MISS by construction. What is still missing is the door — the one-shot ingest through the portal (decision 0020 road one), so a rotation needs no Bitwarden web form. | MISS | [the API key lifecycle ticket](https://github.com/chidionyema/crew/issues/832) |
| `GOOGLE_OAUTH_CLIENT_ID` `GOOGLE_OAUTH_CLIENT_SECRET` | platform/human-vault-bridge/externalsecrets.yaml | Google Cloud (OAuth consent screen) | Operator | The installation's own sign-in credential, born once in the Google Cloud console when the OAuth consent screen is configured: Google publishes no create-API for an OAuth client, so a human makes it and always will. It rides the human road into Bitwarden rather than an estate env file, which is what keeps LAW 54 (no terminal, no repository secret) true for the one step a person cannot avoid. | MISS | [the bridge ticket](https://github.com/chidionyema/crew/issues/809) |
| `TELEGRAM_ALERTS_BOT_TOKEN` `TELEGRAM_ALERTS_CHAT_ID` `TELEGRAM_HERMES_BOT_TOKEN` | platform/human-vault-bridge/externalsecrets.yaml | Telegram (BotFather) | Customer | Bot tokens are born on the founder's phone in BotFather, which has no create-API, and the chat id is read off the chat itself. Until this bridge they reached the cluster through `bin/idp-vault-put --merge`, a terminal command reading an env file — exactly the founder-facing step LAW 54 refuses. On the human road they are pasted once into Bitwarden and never again. | MISS | [the API key lifecycle ticket](https://github.com/chidionyema/crew/issues/832) |
| `STRIPE_SECRET_KEY` | platform/human-vault-bridge/externalsecrets.yaml | Stripe | Operator | The live secret key of the estate's own Stripe account. Stripe does publish a create-API for restricted keys, but not for the account's root secret key that mints them, so the first one is human-born by definition. It is held in Bitwarden and read through `human-vault`; nothing in this repository writes it, and no run prints it. | MISS | [the bridge ticket](https://github.com/chidionyema/crew/issues/809) |
