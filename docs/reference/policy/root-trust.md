...
| `DEEPSEEK_API_KEY` `MINIMAX_API_KEY` `MOONSHOT_API_KEY` `OPENROUTER_API_KEY` `GEMINI_API_KEY` `EXA_API_KEY` `CURSOR_API_KEY` | platform/human-vault-bridge/externalsecrets.yaml | DeepSeek, Minimax, Moonshot, OpenRouter, Google, Exa, Cursor | Customer | The founder's own vendor keys on the human road: born in his browser at each vendor's console, held in Bitwarden, read by the cluster through the `human-vault` ClusterSecretStore and never typed into a terminal or an estate env file (decision 0017). `platform/human-vault-bridge/pushsecrets.yaml` seeds Bitwarden from the entries the estate already holds so the first read has something to find, but that is a migration, not a birth: no code can mint a vendor key, so no bootstrapper can exist for these and the row stays MISS by construction. What is still missing is the door — the one-shot ingest through the portal (decision 0020 road one), so a rotation needs no Bitwarden web form. | MISS | [the API key lifecycle ticket](https://github.com/chidionyema/crew/issues/832) |
