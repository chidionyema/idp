...
| `jit-broker` | platform/jit/deployment.yaml | Estate | Operator | minted by `bin/idp-vault-put jit-broker JIT_SIGNING_KEY=... TELEGRAM_WEBHOOK_SECRET=...` from values `secrets.token_urlsafe` generates; neither is ever typed by a human and neither exists outside the vault. The bot token and chat id are NOT here: the broker reads the estate's one bot from `flux-telegram` rather than holding a second copy (LAW 54). | MEETS | `bin/idp-vault-put` |
