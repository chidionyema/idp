...
| `infra-crew` (`LITELLM_API_KEY`) | platform/infra-crew/external-secret.yaml | estate router | `POST /key/generate` via `bin/idp-router-key --entry infra-crew`, scoped to the kimi, minimax and deepseek lanes with its own daily budget; until 2026-09-04 the crew read the router's MASTER key instead (idp#1491) | MEETS | `bin/idp-estate-seed` |
