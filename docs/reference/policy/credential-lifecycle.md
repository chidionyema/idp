...
| `SEED_CURSOR_API_KEY` | Cursor | dashboard, Integrations, API Keys (user key; `bin/idp-set-root cursor`) | Cloud Agents API on the founder's Cursor plan: start and read background agents on repositories his account can see; no model calls outside them | none by Cursor; the plan's monthly allowance is the ceiling | as Anthropic | delete the key in the dashboard | Cursor dashboard usage per key; the research ledger names the run, never the key |
