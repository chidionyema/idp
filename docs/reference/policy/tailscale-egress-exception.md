...
them as StatefulSets in the `tailscale` namespace, and they need network
...
namespace, defined in `platform/edge/tailscale-egress-exception.yaml`.

## What it covers, and nothing else

- Namespace: `tailscale` only.
...
Any other workload in the `tailscale` namespace, and any workload anywhere
else, still faces the full baseline.

## Receipt

The Kyverno operator log recorded the denial of both StatefulSets at
