...
| `BLIND inventory cloudflare: vault entry cloudflare-api-token unread` | the bootstrapper has not minted the token | an `oke-check` apply runs `bin/idp-bootstrap-cloudflare`; needs the one root (R52) |
...
| `ORPHAN` rows | live, declared nowhere: adopt (`tofu import` in a PR) or kill (founder's word) | one PR per plane, imports proved by a clean `tofu plan` |
