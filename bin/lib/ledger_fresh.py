...
SKEW = dt.timedelta(
    minutes=5
)  # a writer's clock may run this far ahead of GitHub's; more is a dead clock
...
def ledger_fresh(
    ledger: pathlib.Path, hours: int, now: dt.datetime | None = None
) -> int:
    ...
    if now is None:
        print(
            "BLIND research ledger: no clock to measure against (gh api gave no Date header); the local clock is not one"
        )
        return 2
    newest = max(seen)
    if newest > now + SKEW:
        print(
            f"BLIND research ledger: a clock is behind the ledger; newest entry {newest.isoformat(timespec='seconds')} "
    ...
    print(
        f"research ledger: newest entry {age_h:.1f}h old, {verdict} {hours}h of {n} entries "
