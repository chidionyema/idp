        ...
        "resourceLogs": [
            {
                "resource": {
                    "attributes": [
                        {"key": "service.name", "value": {"stringValue": "conscience"}}
                    ]
                },
                "scopeLogs": [
                    {
                        "scope": {"name": "bin/idp-conscience"},
                        "logRecords": [
                            {
                                "timeUnixNano": str(
                                    int(
                                ...
                                "attributes": [
                                    {
                                        "key": "science.source",
                                        "value": {"stringValue": "conscience"},
                                    },
                                    {
                                        "key": "conscience.green",
                                        "value": {
                                            "intValue": str(receipt["score"]["green"])
                                        },
                                    },
                                    {
                                        "key": "conscience.total",
                                        "value": {
                                            "intValue": str(receipt["score"]["total"])
                                        },
                                    },
                                    {
                                        "key": "conscience.red",
...
def emit(report: pathlib.Path) -> int:
    ...
    user, pw = (
        os.environ.get("OTLP_INGEST_USER", ""),
        os.environ.get("OTLP_INGEST_PASSWORD", ""),
    )
    if not (base and user and pw):
        print(
            "BLIND conscience emit: OTEL_EXPORTER_OTLP_ENDPOINT, OTLP_INGEST_USER or OTLP_INGEST_PASSWORD unset"
        )
        return 2
    receipt = json.loads(report.read_text())
    req = urllib.request.Request(
        f"{base}/v1/logs",
        data=json.dumps(payload(receipt)).encode(),
        method="POST",
        headers={
    ...
    try:
        ...
        return 1
    print(
        f"{'ok  ' if code // 100 == 2 else 'FAIL'} conscience emit: {base}/v1/logs -> HTTP {code}, score {receipt['score']['green']}/{receipt['score']['total']} as science.source=conscience"
    )
...
if __name__ == "__main__":
    sys.exit(
        emit(
            pathlib.Path(
                sys.argv[1] if len(sys.argv) > 1 else "reports/conscience.json"
            )
        )
    )
