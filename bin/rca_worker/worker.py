...
# require-pod-probes (Kyverno) needs a real liveness signal, and this daemon has no HTTP
# port to check -- same shape as dagster-daemon's own `dagster-daemon liveness-check`
# (platform/dagster/dagster.yaml). run_forever()'s consume loop touches this file every
# pass (at most every block=2000ms), so `--healthcheck` answers "is the consume loop still
# turning", not merely "is the process alive". Written under the already-mounted `tmp`
# emptyDir (rca.yaml runs readOnlyRootFilesystem: true, so /tmp is the only writable path).
HEARTBEAT_PATH = Path(
    os.environ.get("VN_HEARTBEAT_FILE", "/tmp/via-negativa-heartbeat")  # noqa: S108
)
...
def _healthcheck() -> int:
    """`python3 worker.py --healthcheck` -- the exec probe body (LAW: no fake probes).

    Fresh heartbeat = the consume loop is turning = exit 0. Missing or stale (the loop
    hasn't reached the top of its while in HEARTBEAT_STALE_S, more than 10x the 2s
    xreadgroup block) = exit 1, matching kubectl exec probe semantics.
    """
    import time
    ...
    try:
        ...
        print(f"no heartbeat at {HEARTBEAT_PATH}")
        return 1
    if age_s > HEARTBEAT_STALE_S:
        print(f"heartbeat stale: {age_s:.1f}s old (limit {HEARTBEAT_STALE_S}s)")
        return 1
    print(f"heartbeat fresh: {age_s:.1f}s old")
    return 0
