...
def _env_file_path() -> Path:
    return Path(
        os.environ.get(
            "ESTATE_ENV", str(Path.home() / ".config" / "estate" / "estate.env")
        )
    )
...
def _default_sidecar_target() -> str:
    ...
    db = os.path.expanduser(
        os.environ.get("MAESTRO_DB", "~/.maestro/experience_graph.db")
    )
...
KEYS: dict[str, KeySpec] = {
    "estate.home": KeySpec(
        str(_estate_home()), "path", "ESTATE_HOME", "estate state root"
    ),
    "estate.env_file": KeySpec(
        str(_env_file_path()), "path", "ESTATE_ENV", "estate.env location"
    ),
    "estate.public_url": KeySpec(
        None, "str", "ESTATE_PUBLIC_URL", "cloudflared tunnel URL for the Mini App"
    ),
    "estate.alert_inbox": KeySpec(
    ...
    "temporal.address": KeySpec(
        None, "str", "TEMPORAL_ADDRESS", "overrides host:port together"
    ),
    "temporal.namespace": KeySpec("estate", "str", "TEMPORAL_NAMESPACE", ""),
    "temporal.task_queue": KeySpec("sovereign", "str", "TEMPORAL_TASK_QUEUE", ""),
    "temporal.db_filename": KeySpec(
        str(_estate_home() / "temporal" / "dev.db"), "path", "TEMPORAL_DB", ""
    ),
    ...
    "kini.cp1_tests": KeySpec(
        "tests/bdd/test_self_aware_cp1.py",
        "str",
        "KINI_CP1_TESTS",
        "space-separated pytest paths",
    ),
    "kini.cp2_tests": KeySpec(
        "tests/bdd/test_self_aware_cp2.py tests/bdd/test_cp2_litellm_real.py",
        "str",
        "KINI_CP2_TESTS",
        "",
    ),
    "kini.cp3_tests": KeySpec(
        "tests/bdd/test_self_aware_cp3.py tests/bdd/test_cp3_worker_registry.py",
        "str",
        "KINI_CP3_TESTS",
        "",
    ),
    "kini.cp4_tests": KeySpec(
        "", "str", "KINI_CP4_TESTS", "presence UI: unbound until its feature lands"
    ),
    "kini.cp5_tests": KeySpec(
        "", "str", "KINI_CP5_TESTS", "daily digest: unbound until its feature lands"
    ),
    "kini.cp6_tests": KeySpec(
        "", "str", "KINI_CP6_TESTS", "auto-termination: unbound until its feature lands"
    ),
    "kini.cp7_tests": KeySpec(
        "", "str", "KINI_CP7_TESTS", "identity: unbound until its feature lands"
    ),
    "kini.sovereign_dir": KeySpec(
        str(Path(__file__).resolve().parent),
        "path",
        "KINI_SOVEREIGN_DIR",
        "pytest cwd; bdd_features_base_dir=.. resolves from here",
    ),
    "kini.pytest_args": KeySpec(
        ["-q", "-p", "no:cacheprovider"],
        "list",
        "KINI_PYTEST_ARGS",
        "the worker's rootfs is read-only",
    ),
    "kini.pytest_exit_no_tests": KeySpec(
        5, "int", None, "pytest exit code for 'no tests collected'"
    ),
    "kini.output_tail_lines": KeySpec(
        40, "int", "KINI_OUTPUT_TAIL_LINES", "lines of pytest output kept in the result"
    ),
    "kini.cp_timeout_s": KeySpec(
        900, "int", "KINI_CP_TIMEOUT_S", "start_to_close per checkpoint activity"
    ),
    "kini.cp_heartbeat_s": KeySpec(
        60, "int", "KINI_CP_HEARTBEAT_S", "heartbeat interval while pytest runs"
    ),
    "kini.heartbeat_timeout_s": KeySpec(
        120,
        "int",
        "KINI_HEARTBEAT_TIMEOUT_S",
        "Temporal heartbeat_timeout on both activities",
    ),
    "kini.cp_attempts": KeySpec(
        3,
        "int",
        "KINI_CP_ATTEMPTS",
        "RetryPolicy maximum_attempts per checkpoint activity",
    ),
    "kini.heal_attempts": KeySpec(
        3,
        "int",
        "KINI_HEAL_ATTEMPTS",
        "RetryPolicy maximum_attempts for the heal activity",
    ),
    "kini.heal_timeout_s": KeySpec(
        600,
        "int",
        "KINI_HEAL_TIMEOUT_S",
        "how long the heal activity waits for the cluster to report ready",
    ),
    "kini.heal_poll_s": KeySpec(
        30, "int", "KINI_HEAL_POLL_S", "pause between a platform-fault and the re-run"
    ),
    "kini.heal_max_rounds": KeySpec(
        5,
        "int",
        "KINI_HEAL_MAX_ROUNDS",
        "platform-fault -> heal -> re-run rounds per checkpoint before giving up",
    ),
    "kini.workflow_id": KeySpec(
        "kini-finish",
        "str",
        "KINI_WORKFLOW_ID",
        "one run at a time: Temporal refuses a second start while it runs",
    ),
    "kini.k8s_host_env": KeySpec(
        "KUBERNETES_SERVICE_HOST", "str", None, "set only inside a pod"
    ),
    "kini.k8s_port_env": KeySpec("KUBERNETES_SERVICE_PORT", "str", None, ""),
    "kini.k8s_token_file": KeySpec(
        "/var/run/secrets/kubernetes.io/serviceaccount/token",
        "path",
        "KINI_K8S_TOKEN_FILE",
        "",
    ),
    "kini.k8s_ca_file": KeySpec(
        "/var/run/secrets/kubernetes.io/serviceaccount/ca.crt",
        "path",
        "KINI_K8S_CA_FILE",
        "",
    ),
    "kini.k8s_nodes_path": KeySpec(
        "/api/v1/nodes",
        "str",
        None,
        "the same probe platform/state reads for the cluster-state receipt",
    ),
    "kini.k8s_request_timeout_s": KeySpec(15, "int", "KINI_K8S_REQUEST_TIMEOUT_S", ""),
    "receipts.path": KeySpec(
        str(_estate_home() / "sovereign" / "receipts.jsonl"), "path", "SB_RECEIPTS", ""
    ),
    "receipts.keychain_service": KeySpec(
        "sovereign-receipts", "str", None, "macOS Keychain -s"
    ),
    "receipts.keychain_account": KeySpec("estate", "str", None, "macOS Keychain -a"),
    "receipts.keychain_timeout_s": KeySpec(
        10, "int", "SB_RECEIPTS_KEYCHAIN_TIMEOUT_S", ""
    ),
    "receipts.key_bytes": KeySpec(
        32, "int", "SB_RECEIPTS_KEY_BYTES", "signing key length"
    ),
    "receipts.key_file_mode": KeySpec(
        0o600, "int", None, "software_file key permissions"
    ),
    "receipts.lock_timeout_s": KeySpec(10, "int", "SB_RECEIPTS_LOCK_TIMEOUT_S", ""),
    "receipts.hash_hex_len": KeySpec(
        64, "int", None, "sha256 hexdigest length, the genesis prev_hash"
    ),
    "receipts.json_field_sep": KeySpec(
        ",", "str", None, "canonical json.dumps separators[0]"
    ),
    "receipts.json_kv_sep": KeySpec(
        ":", "str", None, "canonical json.dumps separators[1]"
    ),
    "receipts.head_dir": KeySpec(
        str(_estate_home() / "sovereign"),
        "path",
        None,
        "dir for the signed head anchor (cp19 tail-truncation defense)",
    ),
    "receipts.head_filename": KeySpec(
        "receipts.head",
        "str",
        "SB_RECEIPTS_HEAD_FILENAME",
        "signed head-anchor filename, rewritten after every append",
    ),
    "sidecar.target": KeySpec(
        _default_sidecar_target(),
        "str",
        "SB_SIDECAR_TARGET",
        "cp8: '<db path>#<table>' this sidecar mirrors, DB logic never changed",
    ),
    "sidecar.dag_dir": KeySpec(
        str(_estate_home() / "sovereign" / "dag"),
        "path",
        "SB_SIDECAR_DAG_DIR",
        "cp8: Merkle DAG node directory, one JSON file per observed write",
    ),
    "sidecar.head_filename": KeySpec(
        "HEAD.json",
        "str",
        None,
        "cp8: current DAG head marker filename inside sidecar.dag_dir",
    ),
    "shadow.heads_dir": KeySpec(
        str(_estate_home() / "heads"),
        "path",
        "SB_SHADOW_HEADS_DIR",
        "cp9/R15: dir holding every branch-pointer file; spec 3.1 topology is <estate.home>/heads, one level under estate.home and never a second .estate under it",
    ),
    "shadow.head_filename": KeySpec(
        "shadow_main",
        "str",
        None,
        "cp9: shadow_main -- the head that always names the DAG root equal to the legacy DB's current state",
    ),
    "shadow.legacy_heads_dirs": KeySpec(
        [str(_estate_home() / ".estate" / "heads")],
        "list",
        "SB_SHADOW_LEGACY_HEADS_DIRS",
        "R15: extra dirs the sweep also reads. The default is where the pre-fix doubled-.estate default wrote heads on this Mac; the sweep must be able to see an instance it no longer creates",
    ),
    "dag.main_head_filename": KeySpec(
        "main",
        "str",
        "SB_DAG_MAIN_HEAD",
        "R15: the head every rewind/recover moves; sits beside shadow_main in shadow.heads_dir",
    ),
    "dag.node_suffix": KeySpec(
        ".json", "str", None, "R15: filename suffix of one content-addressed DAG node"
    ),
    "dag.max_walk_nodes": KeySpec(
        1000000,
        "int",
        "SB_DAG_MAX_WALK_NODES",
        "R15: hard stop on a DAG walk, so a corrupt chain can never spin forever",
    ),
    "ops.fs_commit_tokens": KeySpec(
        200,
        "int",
        "SB_OPS_FS_COMMIT_TOKENS",
        "R9: tokens charged for one fs_commit (spec 3.1 checkpoint example: added_tokens 200)",
    ),
    "ops.default_tokens": KeySpec(
        100,
        "int",
        "SB_OPS_DEFAULT_TOKENS",
        "R9: tokens charged for an op with no entry of its own",
    ),
    "ops.nondestructive": KeySpec(
        [
            "fs_commit",
            "fs_read",
            "git_status",
        ...
        "list",
        "SB_OPS_NONDESTRUCTIVE",
        "R9: ops that need budget only -- no quorum, no hardware signature (spec 2.3 step 3)",
    ),
    "ops.destructive": KeySpec(
        [
            "fs_delete",
            "git_push_force",
            "db_drop",
            "service_destroy",
            "rewind",
            "provision_paid_compute",
        ],
        "list",
        "SB_OPS_DESTRUCTIVE",
        "R9: ops that need quorum and a hardware signature on top of budget",
    ),
    "budget.db_filename": KeySpec(
        str(_estate_home() / "sovereign" / "budget.db"),
        "path",
        "SB_BUDGET_DB",
        "R29: sqlite file holding one versioned budget row per session, the optimistic lock",
    ),
    "budget.max_cas_retries": KeySpec(
        50,
        "int",
        "SB_BUDGET_MAX_CAS_RETRIES",
        "R29: compare-and-swap attempts before a spend gives up rather than spinning",
    ),
    "budget.busy_timeout_ms": KeySpec(
        5000,
        "int",
        "SB_BUDGET_BUSY_TIMEOUT_MS",
        "R29: sqlite busy_timeout for the budget connection",
    ),
    "fsm.cycle_path": KeySpec(
        ["planning", "tool_use", "synthesis"],
        "list",
        "SB_FSM_CYCLE_PATH",
        "R30: the repeating loop counted against fsm.max_cycles (spec 4.3)",
    ),
    "fsm.initial_state": KeySpec(
        "init", "str", None, "R28: the state every session starts in"
    ),
    "fsm.terminal_state": KeySpec(
        "terminal", "str", None, "R28: the state no transition leaves"
    ),
    "interventions.dir": KeySpec(
        str(_estate_home() / "interventions"),
        "path",
        "SB_INTERVENTIONS_DIR",
        "R17: append-only transparency log dir, spec 3.1 topology <estate.home>/interventions",
    ),
    "interventions.filename_sep": KeySpec(
        "_", "str", None, "R17: separator in <counter><sep><hash>.json"
    ),
    "interventions.kinds": KeySpec(
        [
            "stop",
            "approve",
            "deny",
        ...
        "list",
        "SB_INTERVENTIONS_KINDS",
        "R17: receipt kinds mirrored into the intervention log",
    ),
    "receipts.counter_filename": KeySpec(
        "receipts.counter",
        "str",
        "SB_RECEIPTS_COUNTER_FILENAME",
        "R23: signed monotonic-counter watermark; survives deletion of the log itself",
    ),
    "undo.git_timeout_s": KeySpec(
        30,
        "int",
        "SB_UNDO_GIT_TIMEOUT_S",
        "R7: timeout on one git subprocess call made by undo/rewind",
    ),
    "undo.parent_suffix": KeySpec(
        "^", "str", None, "R7: git revision suffix naming a commit's first parent"
    ),
    "recover.start_services": KeySpec(
        True,
        "bool",
        "SB_RECOVER_START_SERVICES",
        "cp35: whether `sb recover` also brings the worker and Temporal back up",
    ),
    "views.dir": KeySpec(
        str(_estate_home() / "sovereign" / "views"),
        "path",
        "SB_VIEWS_DIR",
        "cp33/cp35: dir holding the projection views rebuilt from the DAG by rewind and recover",
    ),
    "views.main_filename": KeySpec(
        "main.json",
        "str",
        None,
        "cp33/cp35: the projection view of heads/main, materialized state plus the root it came from",
    ),
    "dualread.max_overhead_ms": KeySpec(
        15,
        "int",
        "SB_DUALREAD_MAX_OVERHEAD_MS",
        "cp10: p95 budget for the dual-read router's added cost (DAG walk + receipt) over the legacy-only read, measured over 1000 reads",
    ),
    "dualread.latency_round_ndigits": KeySpec(
        4,
        "int",
        None,
        "cp10: decimal places dual-read latencies are rounded to before entering a receipt",
    ),
    "time.ms_per_second": KeySpec(
        1000,
        "int",
        None,
        "milliseconds per second, for every perf_counter() duration reported in ms",
    ),
    "fork.dir": KeySpec(
        str(_estate_home() / "sovereign" / "forks"),
        "path",
        "SB_FORK_DIR",
        "cp12: disk-backed fork state/dag/receipts root, used once fork.max_parallel is exceeded",
    ),
    "fork.max_ms": KeySpec(
        1000,
        "int",
        "SB_FORK_MAX_MS",
        "cp12: budget for `sb fork`'s elapsed_ms -- a fork is one pointer-file copy, never a data copy, so this is a correctness bound, not a target to tune toward",
    ),
    "fork.max_parallel": KeySpec(
        3,
        "int",
        "SB_FORK_MAX_PARALLEL",
        "cp12: in-memory forks allowed open at once before a new fork spills to fork.dir on disk",
    ),
    "fork.working_pointer_filename": KeySpec(
        "working_branch",
        "str",
        None,
        "cp12: file under sovereign/ naming the branch `sb switch` last pointed at",
    ),
    "fork.memory_dsn": KeySpec(
        ":memory:",
        "str",
        None,
        "cp12: sqlite3.connect() DSN for an in-memory fork, below fork.max_parallel",
    ),
    "telegram.bot_token": KeySpec(
        _ENV_FILE_VALUES.get("TELEGRAM_BOT_TOKEN"),
        "str",
        "TELEGRAM_BOT_TOKEN",
        "",
        secret=True,
    ),
    "telegram.home_channel": KeySpec(
        _ENV_FILE_VALUES.get("TELEGRAM_HOME_CHANNEL"),
        "str",
    ...
    "langfuse.host": KeySpec(
        _ENV_FILE_VALUES.get("LANGFUSE_HOST"), "str", "LANGFUSE_HOST", ""
    ),
    "langfuse.public_key": KeySpec(
        _ENV_FILE_VALUES.get("LANGFUSE_PUBLIC_KEY"),
        "str",
        "LANGFUSE_PUBLIC_KEY",
        "",
        secret=True,
    ),
    "langfuse.secret_key": KeySpec(
        _ENV_FILE_VALUES.get("LANGFUSE_SECRET_KEY"),
        "str",
        "LANGFUSE_SECRET_KEY",
        "",
        secret=True,
    ),
    "litellm.base_url": KeySpec(
        _secret("LITELLM_BASE_URL"),
        "str",
        "LITELLM_BASE_URL",
        "from the estate secret store (secrets/<env>/LITELLM_BASE_URL.yaml), estate.env as the fallback",
    ),
    "litellm.api_key": KeySpec(
        _secret("LITELLM_API_KEY"),
        "str",
        "LITELLM_API_KEY",
        "a budgeted LiteLLM virtual key (alias sovereign-kernel), never the proxy master key; from the secret store",
        secret=True,
    ),
    "litellm.chat_completions_path": KeySpec(
        "/chat/completions", "str", "LITELLM_CHAT_COMPLETIONS_PATH", ""
    ),
    "budget.default": KeySpec(
        None,
        "int",
        "SB_DEFAULT_BUDGET",
        "tokens; None means budget is required at start",
    ),
    "runner.default": KeySpec("echo", "str", "SB_DEFAULT_RUNNER", ""),
    "model.default": KeySpec(
        "minimax",
        "str",
        "SB_MODEL",
        "LiteLLM alias for the llm runner; the twin of `[routing] default` in AGENTS.md (moved off the dead deepseek account 2026-09-08)",
    ),
    "model.vision": KeySpec(
        "vision", "str", "SB_MODEL_VISION", "LiteLLM alias, vision-capable"
    ),
    "model.consensus": KeySpec(
        ["deepseek", "minimax", "gemini"],
        "list",
        "SB_MODEL_CONSENSUS",
        "comma-separated aliases; three different models (spec 4.2): one local, two hosted, because a second local voter on a 16 GB laptop misses the 30 s deadline (measured 2026-08-26: 3-4B models 60-75 s per answer)",
    ),
    "step.start_to_close_min": KeySpec(
        30, "int", "SB_STEP_TIMEOUT_MIN", "activity start-to-close timeout"
    ),
    "step.heartbeat_s": KeySpec(
        10, "int", "SB_HEARTBEAT_S", "activity heartbeat timeout"
    ),
    "burn.tokens_per_step": KeySpec(
        100, "int", "SB_BURN_TOKENS", "the burn runner's tokens per step"
    ),
    ...
    "alerts.digest_over_per_hour": KeySpec(
        50, "int", "SB_ALERTS_DIGEST_OVER_PER_HOUR", ""
    ),
    "card.poll_s": KeySpec(3, "int", "SB_CARD_POLL_S", ""),
    "approval.timeout_min": KeySpec(
        15,
        "int",
        "SB_APPROVAL_TIMEOUT_MIN",
        "R12 default-deny: minutes an approval request may go unanswered before the workflow halts itself. Silence is not consent, so this halts rather than denies -- a denial would end the session and lose the work, and the founder who was in a meeting can still refill/approve",
    ),
    "trust.require_signed_approval": KeySpec(
        True,
        "bool",
        "SB_REQUIRE_SIGNED_APPROVAL",
        "R11/R22: refuse `sb approve` without a verified signature. Off only for a host with no trust anchor at all, and the receipt records which",
    ),
    "trust.backend": KeySpec("auto", "str", "SB_TRUST_BACKEND", ""),
    "presence.default": KeySpec("ghost", "str", "SB_PRESENCE_DEFAULT", ""),
    "cli.port_probe_timeout_s": KeySpec(
        0.3,
        "float",
        "SB_PORT_PROBE_TIMEOUT_S",
        "socket connect_ex timeout when probing a port",
    ),
    "cli.up_wait_deadline_s": KeySpec(
        20,
        "int",
        "SB_UP_WAIT_DEADLINE_S",
        "seconds `sb up` waits for temporal's port to open",
    ),
    "cli.up_poll_interval_s": KeySpec(
        0.5, "float", "SB_UP_POLL_INTERVAL_S", "poll interval while `sb up` waits"
    ),
    "cli.down_wait_deadline_s": KeySpec(
        10,
        "int",
        "SB_DOWN_WAIT_DEADLINE_S",
        "seconds `sb down` waits for a SIGTERM'd pid to exit before SIGKILL",
    ),
    "net.host_port_sep": KeySpec(
        ":", "str", None, "separator between host and port in an address string"
    ),
    "session.id_hex_len": KeySpec(
        8, "int", "SB_SESSION_ID_HEX_LEN", "hex chars from uuid4 used for a session id"
    ),
    "session.last_output_max_chars": KeySpec(
        500, "int", "SB_LAST_OUTPUT_MAX_CHARS", "last_output truncation length"
    ),
    "runner.token_estimate_divisor": KeySpec(
        4, "int", "SB_TOKEN_ESTIMATE_DIVISOR", "len(output)//N token estimate"
    ),
    "runner.ask_prefix": KeySpec(
        "needs",
        "str",
        "SB_ASK_PREFIX",
        "prefix the ask runner recognizes in a task, before the separator",
    ),
    "runner.ask_prefix_sep": KeySpec(
        ":", "str", None, "separator between the ask prefix and the rest of the task"
    ),
    "runner.llm_timeout_s": KeySpec(
        120, "int", "SB_LLM_TIMEOUT_S", "httpx timeout for the llm runner"
    ),
    "receipt.activity_timeout_s": KeySpec(
        30,
        "int",
        "SB_RECEIPT_ACTIVITY_TIMEOUT_S",
        "append_receipt start-to-close timeout",
    ),
    "receipt.retry_max_attempts": KeySpec(
        5, "int", "SB_RECEIPT_RETRY_MAX_ATTEMPTS", ""
    ),
    "notify.activity_timeout_s": KeySpec(
        10,
        "int",
        "SB_NOTIFY_ACTIVITY_TIMEOUT_S",
        "notify_change start-to-close timeout -- short: a stuck notify must never hold up the step loop",
    ),
    "notify.retry_max_attempts": KeySpec(
        1,
        "int",
        "SB_NOTIFY_RETRY_MAX_ATTEMPTS",
        "1 == no retry; the next state change notifies again anyway",
    ),
    "step.activity_retry_max_attempts": KeySpec(
        2, "int", "SB_STEP_RETRY_MAX_ATTEMPTS", "run_step activity retry attempts"
    ),
    "cli.exit_usage_error": KeySpec(
        2,
        "int",
        "SB_EXIT_USAGE_ERROR",
        "exit code for a refused command, e.g. missing budget",
    ),
    "client.query_timeout_s": KeySpec(
        5,
        "float",
        "SB_QUERY_TIMEOUT_S",
        "client-side timeout on a single workflow query, so one stuck session never hangs `sb list`",
    ),
    "log.bot_token_redact_pattern": KeySpec(
        r"bot\d+:[A-Za-z0-9_-]+",
        "str",
        None,
        "regex matching a Telegram bot token as it appears in a URL (LAW 21)",
    ),
    "flip.readonly_mode": KeySpec(
        0o444,
        "int",
        None,
        "cp13: filesystem mode `sb flip` chmods the legacy DB file to",
    ),
    "flip.writable_mode": KeySpec(
        0o644, "int", None, "cp13: filesystem mode `sb flip --rollback` restores"
    ),
    "flip.max_downtime_ms": KeySpec(
        250,
        "int",
        "SB_FLIP_MAX_DOWNTIME_MS",
        "cp13: budget for the chmod + one shadow-root verify() a flip takes; reads are never blocked, this bounds the window before the flip is durable",
    ),
    "flip.receipt_template": KeySpec(
        "[✓] FLIP | root:{root} | legacy:readonly",
        "str",
        "SB_FLIP_RECEIPT_TEMPLATE",
        "cp13 receipt line, exact format",
    ),
    "flip.rollback_receipt_template": KeySpec(
        "[✓] FLIP_ROLLBACK | root:{root} | legacy:writable",
        "str",
        "SB_FLIP_ROLLBACK_RECEIPT_TEMPLATE",
        "cp13 receipt line, exact format",
    ),
    "flip.hash_chunk_bytes": KeySpec(
        65536,
        "int",
        None,
        "cp13: read chunk size for the legacy DB's sha256 in flip.py -- a lint-exempt literal would trip config.py's own numeric-literal rule",
    ),
    "projection.store_path": KeySpec(
        str(_estate_home() / "sovereign" / "projection.json"),
        "path",
        "SB_PROJECTION_STORE_PATH",
        "cp14: the hot store rebuilt from the DAG, one JSON file keyed by table then rowid",
    ),
    "rebuild.receipt_template": KeySpec(
        "[✓] REBUILD | root:{root}",
        "str",
        "SB_REBUILD_RECEIPT_TEMPLATE",
        "cp14 receipt line, exact format from features/sovereign-bus/cp14_projection_views.feature",
    ),
    "cross_stack.git_timeout_s": KeySpec(
        5,
        "int",
        "SB_CROSS_STACK_GIT_TIMEOUT_S",
        "cp15: timeout for the `git rev-parse HEAD` subprocess call that produces code_root",
    ),
}
...
_TYPE_TO_SPEC_TYPE: dict[type, str] = {
    str: "str",
    int: "int",
    float: "float",
    bool: "bool",
...
def _merge_external_keys(table: dict[str, tuple[Any, type, str, str]]) -> None:
    for key, (default, typ, env_name, help_text) in table.items():
        if key in KEYS:
            continue
        KEYS[key] = KeySpec(
            default, _TYPE_TO_SPEC_TYPE.get(typ, "str"), env_name, help_text
        )
...
for _name, _usd in POLICY.budget_usd_per_day.items():
    KEYS[f"budget.usd_per_day.{_name}"] = KeySpec(
        float(_usd),
        "float",
        _env_name("SB_BUDGET_USD_PER_DAY_", _name),
        f"R40: default USD per day {_name} may spend; the sum over cost.days_per_month sits inside the cost contract (AGENTS.md)",
    )
KEYS["cost.contract_min_usd_month"] = KeySpec(
    float(POLICY.cost["contract_min_usd_month"]),
    "float",
    "SB_COST_CONTRACT_MIN_USD_MONTH",
    "R40: spec section 8 floor of direct monthly cost (AGENTS.md)",
)
KEYS["cost.contract_max_usd_month"] = KeySpec(
    float(POLICY.cost["contract_max_usd_month"]),
    "float",
    "SB_COST_CONTRACT_MAX_USD_MONTH",
    "R40: spec section 8 ceiling of direct monthly cost (AGENTS.md)",
)
KEYS["cost.days_per_month"] = KeySpec(
    int(POLICY.cost["days_per_month"]),
    "int",
    "SB_COST_DAYS_PER_MONTH",
    "R40: days the per-day budgets are summed over (AGENTS.md)",
)
for _purpose, _alias in POLICY.routing.items():
    KEYS[f"routing.{_purpose}"] = KeySpec(
        list(_alias) if isinstance(_alias, list) else str(_alias),
        "list" if isinstance(_alias, list) else "str",
        _env_name("SB_ROUTING_", _purpose),
        f"R38: LiteLLM alias(es) used for {_purpose} (AGENTS.md routing table)",
    )
KEYS["merge.strict_branches"] = KeySpec(
    list(POLICY.merge["strict_branches"]),
    "list",
    "SB_MERGE_STRICT_BRANCHES",
    "R41: branches whose PRs fail on any pending feature (AGENTS.md)",
)
KEYS["merge.require_bdd_green"] = KeySpec(
    bool(POLICY.merge["require_bdd_green"]),
    "bool",
    "SB_MERGE_REQUIRE_BDD_GREEN",
    "R41: a PR needs sovereign/tests/bdd green (AGENTS.md)",
)
KEYS["merge.pending_owner_required_on"] = KeySpec(
    list(POLICY.merge["pending_owner_required_on"]),
    "list",
    "SB_MERGE_PENDING_OWNER_REQUIRED_ON",
    "R39: branches where a pending mark must name a real owner (AGENTS.md)",
)
...
def get(
    key: str, cli_value: Any = None, *, _toml: dict[str, Any] | None = None
) -> Resolved:
    spec = KEYS[key]
...
TEMPORAL_ADDRESS: str = (
    _R["temporal.address"].value or f"{TEMPORAL_HOST}:{TEMPORAL_PORT}"
)
...
KINI_CP_TESTS: dict[int, list[str]] = {
    n: str(_R[f"kini.cp{n}_tests"].value or "").split()
    for n in range(1, KINI_CHECKPOINT_COUNT + 1)
}
...
RECEIPTS_HEAD: Path = (
    Path(_R["receipts.head_dir"].value) / _R["receipts.head_filename"].value
)
...
RECEIPTS_COUNTER: Path = (
    Path(_R["receipts.head_dir"].value) / _R["receipts.counter_filename"].value
)
...
RECEIPTS_JSON_SEPARATORS: tuple[str, str] = (
    _R["receipts.json_field_sep"].value,
    _R["receipts.json_kv_sep"].value,
)
    ...
    for n in ast.walk(tree):
        if isinstance(n, ast.Call):
            ...
            if (
                ...
                or name.lower() in _LOG_METHOD_NAMES
            ):
...
def lint(root: Path | None = None) -> list[LintHit]:
    ...
    for path in sorted(root.rglob("*.py")):
        ...
        if (
            path.name in ("config.py", "config_keys.py")
