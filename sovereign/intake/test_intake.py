...
def _stub(markdown: str, slug: str, tags: list[str]):
    def _call(model: str, messages) -> str:
        return json.dumps(
            {"markdown": markdown, "slug": slug, "tags": tags, "title": slug}
        )
...
def test_property_receipt_line_is_one_line_and_names_the_commit() -> None:
    line = receipt_line(
        "docs/a_b_c.md", "8f2a1b3c" + "0" * 32, ("ml", "governance"), 200
    )
    assert "\n" not in line
    assert (
        line
        == "[✓] DOC_COMMIT | file:docs/a_b_c.md | hash:8f2a1b3c | tags:#ml,#governance | budget:-200"
    )
...
def test_incident_budget_zero_writes_nothing(
    ...
    req = IntakeRequest(
        image=b"x", caption="c", repo=repo, source="t", channel="t", budget_remaining=0
    )
