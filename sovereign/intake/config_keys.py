...
INTAKE_KEYS: dict[str, tuple[Any, type, str, str]] = {
    ...
    "intake.file_suffix": (
        ".md",
        str,
        "SB_INTAKE_FILE_SUFFIX",
        "Suffix of the committed markdown file",
    ),
    "intake.slug_words": (
        3,
        int,
        "SB_INTAKE_SLUG_WORDS",
    ...
    "intake.session_id": (
        "intake",
        str,
        "SB_INTAKE_SESSION_ID",
        "session_id stamped on an intake receipt when the caller has no session",
    ),
    "intake.runner_name": (
        "intake",
        str,
        "SB_INTAKE_RUNNER_NAME",
        "runner field on an intake receipt",
    ),
    "intake.commit_prefix": (
        "intake",
        str,
        "SB_INTAKE_COMMIT_PREFIX",
        "First word of the git commit subject for a committed document",
    ),
    "intake.laptop_source_name": (
        "laptop",
        str,
        "SB_INTAKE_LAPTOP_SOURCE",
        "source label for the laptop entry point",
    ),
    "intake.phone_source_name": (
        "phone",
        str,
        "SB_INTAKE_PHONE_SOURCE",
        "source label for the phone entry point",
    ),
    "intake.chat_source_name": (
        "chat",
        str,
        "SB_INTAKE_CHAT_SOURCE",
        "source label for the chat entry point",
    ),
    "intake.cli_channel": (
        "cli",
        str,
        "SB_INTAKE_CLI_CHANNEL",
        "channel the CLI's one-line receipt is printed as",
    ),
    "intake.converse_state_name": (
        "converse",
        str,
        "SB_INTAKE_CONVERSE_STATE_NAME",
