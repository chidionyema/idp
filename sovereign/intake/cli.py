...
def register(sub) -> None:
    p = sub.add_parser(
        "intake",
        help="R8 and R42 -- a photo becomes a committed markdown file and one receipt line",
    )
    p.add_argument("image", help="path to the image file")
    p.add_argument(
        "--repo", required=True, help="git checkout the file is committed into"
    )
    ...
    p.add_argument(
        "--budget", type=int, default=None, help="tokens remaining; refused at zero"
    )
