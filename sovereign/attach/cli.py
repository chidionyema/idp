...
def register(sub) -> None:
    ...
    p_halt.add_argument(
        ...
        action="store_true",
        required=True,
        help=ck.get("attach.cli_halt_all_help"),
    )
...
def _cmd_attach(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        for line in result["receipt_lines"]:
            print(line)
        print(
            f"root={result['root']} nodes={result['nodes']} hash={result['hash']} "
...
def _cmd_status(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        for e in result["estates"]:
            print(
                f"{e['root']} mode={e['mode']} running={e['running']} sessions={len(e['sessions'])}"
            )
    return 0
