...
ATTACH_KEYS: dict[str, tuple[Any, type, str, str]] = {
    ...
    "attach.mounted_receipt_template": (
        "[✓] ESTATE_MOUNTED | root:{root} | nodes:{nodes} | hash:{hash}",
        str,
        "SB_ATTACH_MOUNTED_RECEIPT_TEMPLATE",
        "cp21 receipt line, exact format",
    ),
    "attach.policy_inherited_receipt_template": (
        "[✓] POLICY_INHERITED | policy:{policy} | mode:{mode}",
        str,
        "SB_ATTACH_POLICY_INHERITED_RECEIPT_TEMPLATE",
        "cp21 receipt line, exact format",
    ),
    "attach.policy_scaffolded_label": (
        "AGENTS.md (auto-scaffolded)",
        str,
        "SB_ATTACH_POLICY_SCAFFOLDED_LABEL",
        "",
    ),
    "attach.policy_existing_label": (
        "AGENTS.md",
        str,
        "SB_ATTACH_POLICY_EXISTING_LABEL",
        "",
    ),
    "attach.policy_mode": (
        "Ghost",
        str,
        "SB_ATTACH_POLICY_MODE",
        "The conservative default mode named in the POLICY_INHERITED receipt",
    ),
    "attach.git_ls_files_timeout_s": (
        30,
        float,
        "SB_ATTACH_GIT_LS_FILES_TIMEOUT_S",
        "Timeout for `git ls-files` when counting nodes",
    ),
    "attach.ignored_dirnames": (
        [".git", ".estate", "node_modules", "__pycache__", ".venv"],
        list,
        "SB_ATTACH_IGNORED_DIRNAMES",
        "Dirnames skipped when walking a non-git target",
    ),
    "attach.write_verbs": (
        ["write", "edit", "create", "delete", "mv", "mkdir", "chmod"],
        list,
        "SB_ATTACH_WRITE_VERBS",
        "Prefixes/verbs classified as a write operation",
    ),
    "attach.git_write_verbs": (
        ...
        list,
        "SB_ATTACH_GIT_WRITE_VERBS",
        "`git <verb>` subcommands classified as a git write",
    ),
    "attach.default_runner": (
        "echo",
        str,
        "SB_ATTACH_START_RUNNER",
        "Default --runner when `sb start --estate` omits one (mirrors runner.default)",
    ),
    "attach.status_max_sessions_per_estate": (
        50,
        int,
        "SB_ATTACH_STATUS_MAX_SESSIONS_PER_ESTATE",
        "Sessions listed per estate in `sb status` before truncation",
    ),
    "attach.cli_attach_help": (
        "mount a repo, directory or workspace as an estate",
        str,
        "SB_ATTACH_CLI_ATTACH_HELP",
        "argparse help for `sb attach`",
    ),
    "attach.cli_write_policy_help": (
        "also write a scaffolded AGENTS.md into the repo root; default is .estate/ only",
        str,
        "SB_ATTACH_CLI_WRITE_POLICY_HELP",
        "argparse help for `sb attach --write-policy`",
    ),
    "attach.cli_status_help": (
        "list every attached estate and its running sessions",
        str,
        "SB_ATTACH_CLI_STATUS_HELP",
        "argparse help for `sb status`",
    ),
    "attach.cli_halt_help": (
        "stop every running session across every attached estate",
        str,
        "SB_ATTACH_CLI_HALT_HELP",
        "argparse help for `sb halt`",
    ),
    "attach.cli_halt_all_help": (
        "required; halt has no single-session form here, use sb stop",
        str,
        "SB_ATTACH_CLI_HALT_ALL_HELP",
        "argparse help for `sb halt --all`",
    ),
}
...
def get(key: str) -> Any:
    ...
    if typ is list:
        return (
            default if raw is None else [x.strip() for x in raw.split(",") if x.strip()]
        )
    if typ is bool:
