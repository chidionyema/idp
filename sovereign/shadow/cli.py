        ...
        out.update(
            {
                ...
                "refill": verdict.boundary.refill,
            }
        )
...
def register(sub: Any) -> None:
    p = sub.add_parser(
        "branch",
        help="R19 -- fork N silent Temporal child sessions on git branches and merge the winner",
    )
    ...
    p.add_argument(
        "--branches", type=int, default=None, help="default: config branch.count"
    )
    ...
    p = sub.add_parser(
        "distill",
        help="R18/R20 -- train, grade and route one task class on a measured number",
    )
    p.add_argument("--task-class", required=True, dest="task_class")
    p.add_argument(
        "--no-train",
        action="store_true",
        help="grade and route without running the LoRA job",
    )
    p.add_argument("--json", action="store_true")
    ...
    p = sub.add_parser(
        "preauth",
        help="R10/R21 -- run the shadow planning loop over predicted step costs",
    )
    p.add_argument("--session-id", required=True, dest="session_id")
    p.add_argument("--remaining", type=int, required=True)
    p.add_argument(
        "--costs", required=True, help="comma-separated predicted tokens per step"
    )
    p.add_argument(
        "--op",
        default=None,
        help="the op the next step asks for, if not a budget boundary",
    )
    p.add_argument("--json", action="store_true")
