...
def _boundary(op_name: str) -> preauth.Boundary:
    return preauth.Boundary(
        kind=preauth.normalize_op(op_name),
        op=preauth.classify(op_name),
        remaining=0,
        predicted_costs=(1,),
        hit_at_step=1,
        refill=1000,
...
def test_property_no_destructive_or_unknown_op_is_ever_shadow_authorized() -> None:
    ...
    names = list(config.OPS_DESTRUCTIVE) + [
        "git push --force",
        "rm -rf /",
        "",
        "not_in_any_table",
...
def test_property_every_nondestructive_op_is_shadow_authorized_only_above_threshold() -> (
    None
):
...
def test_property_prediction_finds_a_boundary_exactly_when_the_horizon_costs_more_than_remaining() -> (
    None
):
    ...
    for _ in range(500):
        ...
        if sum(window) <= remaining:
            ...
            assert remaining + boundary.refill >= sum(window), (
                "the refill covers the whole trajectory"
            )
            assert boundary.refill % round_to == 0
            assert 1 <= boundary.hit_at_step <= len(window)
            assert boundary.steps_covered == len(window)
            assert isinstance(boundary.op, preauth.NonDestructiveOp), (
                "a refill is a budget movement, not a destructive act"
            )


def test_property_confidence_is_zero_below_min_samples_and_monotonic_in_approvals() -> (
    None
):
    ...
    for approvals in range(minimum, minimum + 200):
        ...
        last = c
    assert preauth.confidence(preauth.History("x", minimum, 0)) >= float(
        config.get("shadow.min_confidence").value
    )
    assert preauth.confidence(preauth.History("x", minimum, 1)) < preauth.confidence(
        preauth.History("x", minimum, 0)
    )
