...
def test_incident_r29_code_alone_is_refused_and_code_with_spec_is_permitted(
    tmp_path: Path,
) -> None:
    repo = _repo(tmp_path)
...
def test_incident_r29_no_code_change_passes_and_missing_base_is_blind(
    tmp_path: Path,
) -> None:
    repo = _repo(tmp_path)
    ...
    p = subprocess.run(
        [str(GATE), "no-such-ref"], cwd=repo, capture_output=True, text=True
    )
