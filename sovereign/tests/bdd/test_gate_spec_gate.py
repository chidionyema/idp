...
def _new(repo: Path, runs: dict) -> None:
    _commit(
        repo,
        {
            "features/new.feature": "Feature: nobody runs me\n",
    ...
    assert (
        r.returncode == 1
        and "FAIL  spec-gate new *.feature" in r.stdout
        and "features/new.feature" in r.stdout
        and "scenarios(" in r.stdout
    ), r.stdout
...
def _residual(runs: dict, prefix: str) -> None:
    ...
    for r in runs.values():
        assert head in r.stdout and "feature file(s) named by no test" in r.stdout, (
            r.stdout
        )
