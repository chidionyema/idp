...
@given(
    'a PR adds resource "oci_identity_domains_app" and no grant, policy or membership'
)
def _half(state: dict) -> None:
    ...
    for scope in (
        ...
        "oci_identity_policy",
        "oci_identity_domains_group_membership",
    ):
...
def _half_refused(state: dict) -> None:
    ...
    line = next(
        l
        for l in state["runs"]["opmodel-half-provisioned.json"].stdout.splitlines()
        if "rule=provisioning_complete" in l
    )
...
def _gui(state: dict, line: str) -> None:
    ...
    assert (
        "FOUNDER ACTION" in fx["pr"]["body"] and "console" in fx["pr"]["body"].lower()
    ), fx["pr"]["body"]
...
def _no_approval(state: dict) -> None:
    ...
    assert any(
        "backstage/" in f or f.startswith("platform/identity/")
        for f in fx["pr"]["files"]
    ), fx["pr"]["files"]
...
@given(
    'a PR whose "Approval-word:" the founder answered with DENY: from his GitHub login'
)
def _denied(state: dict) -> None:
    fx = _fx("opmodel-denied.json")
    word = next(
        l.split(":", 1)[1].strip()
        for l in fx["pr"]["body"].splitlines()
        if l.startswith("Approval-word:")
    )
...
@given(
    "a PR touching platform/oci/ whose Cost-delta-usd-month beats estate-defaults.yaml infrastructure.monthly_cap_usd, or with no canary label"
)
def _cost(state: dict) -> None:
    cap = yaml.safe_load((IDP / "estate-defaults.yaml").read_text())["infrastructure"][
        "monthly_cap_usd"
    ]
    over = _fx("opmodel-over-budget.json")
    assert any(f.startswith("platform/oci/") for f in over["pr"]["files"]), over["pr"][
        "files"
    ]
    delta = float(
        next(
            l.split(":", 1)[1]
            for l in over["pr"]["body"].splitlines()
            if l.startswith("Cost-delta-usd-month:")
        )
    )
...
@given(
    "a pull request that changes a platform layer and adds a row to drills/catalogue.yaml"
)
def _drill_added(state: dict) -> None:
    ...
    assert (
        "drills/catalogue.yaml" in fx["pr"]["files"] and "- name:" in fx["pr"]["added"]
    )
...
def _drill_body(state: dict) -> None:
    ...
    name = re.search(r"(?m)^\+\s*-\s*name:\s*(\S+)", state["fx"]["pr"]["added"]).group(
        1
    )
    assert f"Drill: {name}" in state["fx"]["pr"]["body"]
    assert name not in state["fx"]["drills"], (
        "the fixture must name a drill the catalogue on main does not hold"
    )


@then(
    "the drill names come from the catalogue on main and the catalogue at the PR head"
)
...
@given(
    'a PR body with no "## Architecture laws" section, or one whose law line is a sentence'
)
def _laws_bad(state: dict) -> None:
    assert "## Architecture laws" not in _fx("opmodel-no-laws.json")["pr"]["body"]
    assert (
        "- LAW 3 nervous system: we will wire tracing later"
        in _fx("opmodel-laws-sentence.json")["pr"]["body"]
    )
    state["fixtures"] += [
        "opmodel-no-laws.json",
        "opmodel-laws-sentence.json",
        "opmodel-ok.json",
    ]
...
def _laws_warned(state: dict) -> None:
    ...
    for name in ("opmodel-no-laws.json", "opmodel-laws-sentence.json"):
        ...
        assert "rule=architecture_laws" in r.stdout and "WARN" in r.stdout, (
            name,
            r.stdout,
        )
