...
@then(
    "each permission it names is in manifest.json default_permissions at the same or a lower level"
)
def _subset(state: dict) -> None:
    ...
    for lane, perms in state["lanes"].items():
        ...
        for perm, level in perms.items():
            assert perm in held, (
                f"lane {lane} asks for {perm}, which the App does not hold"
            )
            assert LEVEL[level] <= LEVEL[held[perm]], (
                f"lane {lane} asks {perm}:{level}, App holds {held[perm]}"
            )
...
def _unknown(state: dict, tmp_path: Path) -> None:
    ...
    state["run"] = subprocess.run(
        [str(IDP / "bin" / "idp-github-app"), "token", "no-such-lane"],
        env={
