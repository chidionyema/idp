...
_spec = importlib.util.spec_from_file_location(
    "incident_heartbeat", IDP / "tests/test_incident_drill_never_scheduled.py"
)
...
def _fail(state: dict) -> None:
    ...
    assert (
        "FAIL    heartbeat  login-drill.yml  last success 41 min ago > 20 (run 999)"
        in r.stdout
    ), r.stdout
...
@then(
    "a run with no successful run on record is FAIL, and an unreadable API is BLIND, never ok"
)
def _none_blind(state: dict) -> None:
    assert (
        state["none"].returncode == 1
        and "no successful run on record" in state["none"].stdout
    ), state["none"].stdout
    assert state["blind"].returncode == 2 and state["blind"].stdout.startswith(
        "BLIND   heartbeat"
    ), state["blind"].stdout
    assert state["fresh"].returncode == 0 and state["fresh"].stdout.startswith(
        "ok      heartbeat"
    ), state["fresh"].stdout


@then(
    'drill-heartbeat.yml runs every 15 minutes and opens or comments "P0: login drill failed" on FAIL'
)
def _workflow(state: dict) -> None:
    ...
    assert (
        'gh issue create --title "P0: login drill failed"' in text
        and "gh issue comment" in text
    )
