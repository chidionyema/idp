...
@given(
    "llm/config.yaml lists the hosted models and the local ollama lane",
    target_fixture="laptop",
)
...
@given(
    "the prospector edge has a listener https-llm for llm.${ESTATE_ZONE}",
    target_fixture="listener",
)
def listener() -> str:
...
@given(
    "the vault holds one JSON secret litellm-upstream", target_fixture="external_secret"
)
...
@when(
    "the ExternalSecret materialises it in namespace llm", target_fixture="secret_name"
)
...
def env_refs(secret_name: str) -> None:
    ...
    }, "litellm-upstream is not a mounted volume"
...
def no_key_in_repo() -> None:
    for f in CLUSTER.glob("*.yaml"):
        assert not re.search(
            r"(?i)(api_key|master_key):\s*['\"]?(sk-|[A-Za-z0-9]{32,})", f.read_text()
        ), f


# crew#400/crew#408: the founder picks and adds models in the Admin UI; the console signs in through IDCS.
@given(
    "the router runs the -database image with litellm-db in namespace llm",
...
def deployment() -> dict:
    ...
    assert any(
        d["kind"] == "StatefulSet" and d["metadata"]["name"] == "litellm-db" for d in pg
    )
    return dep
...
def same_domain_as_catalogue(sso_env: dict) -> None:
    ...
    assert "ESTATE_OIDC_DOMAIN_URL" in cfg["data"], (
        "the domain URL is one value in estate-config"
    )
    tf = (ROOT / "platform" / "oci" / "identity" / "main.tf").read_text()
    assert 'redirect_uris             = ["https://llm.${var.zone}/sso/callback"]' in tf
    assert "oci_identity_domains_app.router_console.id" in tf, (
        "founder_emails are granted the console app"
    )
...
def client_from_vault(deployment: dict) -> None:
    es = next(
        d
        for d in yaml.safe_load_all((CLUSTER / "external-secret.yaml").read_text())
        if d["metadata"]["name"] == "litellm-sso"
    )
    keys = {d["secretKey"]: d["remoteRef"]["key"] for d in es["spec"]["data"]}
    assert keys == {
        "GENERIC_CLIENT_ID": "litellm-sso-client-id",
        "GENERIC_CLIENT_SECRET": "litellm-sso-client-secret",
...
def provider_keys_exported() -> None:
    ...
    exported = set(
        re.findall(
            r"([A-Z_]+_API_KEY)=\1", (CLUSTER / "external-secret.yaml").read_text()
        )
    )
