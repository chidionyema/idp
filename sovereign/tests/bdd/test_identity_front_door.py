...
# docs/NEXT.md is generated by bin/estate-next from open crew checkpoints (crew#403 CP6); a checkpoint
# that still plans the broker (crew#281 CP3) names it, and the page quotes the checkpoint verbatim.
ALLOWED = (
    "features/identity/front-door-oidc-client.feature",
    "docs/decisions/",
    "sovereign/tests/bdd/test_identity_front_door.py",
    "docs/NEXT.md",
)
...
def _git(*args: str) -> str:
    return subprocess.run(
        ["git", *args], cwd=IDP, capture_output=True, text=True, check=True
    ).stdout
...
def _gate(state: dict) -> None:
    state["gate"] = subprocess.run(
        [sys.executable, str(IDP / "bin" / "cloud-agnostic-gate")],
        env={**os.environ, "CLOUD_AGNOSTIC_ROOT": str(IDP)},
        capture_output=True,
        text=True,
    )
...
@then(
    "every provider URL is a ${ESTATE_OIDC_*} substitution from clusters/oke/estate-config.yaml"
)
def _substituted(state: dict) -> None:
    ...
    for key in PROVIDER_KEYS:
        m = re.search(
            rf'^\s+{key}:\s*"(\$\{{(ESTATE_OIDC_[A-Z_]+)\}}[^"]*)"', state["pod"], re.M
        )
        assert m, f"{key} is not a ${{ESTATE_OIDC_*}} substitution in {POD.name}"
        assert re.search(rf"^\s+{m.group(2)}:", config, re.M), (
            f"{m.group(2)} is not declared in estate-config.yaml"
        )
...
@then(
    "no tracked file names platform/access, access-apply or ESTATE_LOGIN_GITHUB_USER, except this feature and the decision record that retires them"
)
