...
def _wf(state: dict) -> None:
    ...
    step = next(s for s in doc["jobs"]["seed"]["steps"] if "idp-vault-put" in s.get("name", ""))
...
def _keys(state: dict) -> None:
    needed = set(re.findall(r"\b([A-Z0-9_]+)=\1\b", state["run"])) | set(re.findall(r"\b[a-z_0-9]+=((?:GITHUB_APP|FLUX_WRITER|TEMPORAL_DB|LITELLM_UI)_[A-Z0-9_]+)", state["run"]))
...
@then("the workflow is dispatch-only with entries all, prospector-engine-env, github-app, flux-writer, temporal-db and litellm-ui")
def _dispatch(state: dict) -> None:
    ...
    assert on["workflow_dispatch"]["inputs"]["entry"]["options"] == ["all", "prospector-engine-env", "github-app", "flux-writer", "temporal-db", "litellm-ui"]
