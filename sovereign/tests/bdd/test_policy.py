...
def test_a_stale_value_in_agents_md_is_refused(
    ...
    stale = re.sub(
        r'^"consensus.timeout_s" = 30$',
        '"consensus.timeout_s" = 31',
        real,
        count=1,
        flags=re.MULTILINE,
    )
...
def test_config_keys_come_from_the_policy_block(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    real = policy.agents_md_path().read_text()
    edited = re.sub(
        r"^vision = 0\.5", "vision = 0.75", real, count=1, flags=re.MULTILINE
    )
    assert edited != real
    copy = tmp_path / "AGENTS.md"
...
def test_default_per_day_budgets_fit_the_cost_contract() -> None:
    ...
    per_day = [
        ...
        if k.startswith("budget.usd_per_day.")
    ]
    ...
    assert low <= monthly <= high, (
        f"{monthly} USD/month is outside the {low}-{high} contract"
    )
...
def test_every_routing_alias_exists_in_the_litellm_config() -> None:
    ...
    declared = set(
        re.findall(r"^\s*-\s*model_name:\s*(\S+)", litellm, flags=re.MULTILINE)
    )
    ...
    for purpose, alias in config.POLICY.routing.items():
        ...
        for name in alias if isinstance(alias, list) else [alias]:
            assert name in declared, (
                f"routing.{purpose} names {name!r}; the config and the console declare {sorted(declared)}"
            )
