...
def _filter(state: dict) -> None:
    hr = [
        d
        for d in _docs(IDP / "platform/chaos/mesh/helmrelease.yaml")
...
def _schedule(state: dict) -> None:
    ...
    for f in (IDP / "platform/chaos").rglob("*.yaml"):
        for d in _docs(f):
            if (
                d.get("kind") == "Schedule"
                and d["metadata"]["name"] == "backstage-pod-kill"
            ):
                found.append(d)
...
@then(
    "the Namespace backstage declared under platform/ carries chaos-mesh.org/inject: enabled"
)
def _label(state: dict) -> None:
    ...
    ns = [
        d
        for f in sorted((IDP / "platform").rglob("*.yaml"))
        for d in _docs(f)
    ...
    assert [
        d for d in ns if d["metadata"].get("labels", {}).get(INJECT) == "enabled"
    ], ns


@then(
    "tests/test_incident_chaos_target_namespace_unlabelled.py refuses a target namespace without it"
)
def _incident_test_runs(state: dict) -> None:
    ...
    r = subprocess.run(
        [
            ...
            "-p",
            "no:cacheprovider",
            "tests/test_incident_chaos_target_namespace_unlabelled.py",
        ],
        cwd=IDP,
        capture_output=True,
        text=True,
