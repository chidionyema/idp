...
def _run_incident(key: str) -> str:
    r = subprocess.run(
        [sys.executable, "-m", "pytest", "-q", "-p", "no:cacheprovider", INCIDENT[key]],
        cwd=IDP,
        capture_output=True,
        text=True,
    )
...
def _reconfigure(state: dict) -> None:
    for b in ("bin/idp-oke-rebuild", "bin/idp-identity-apply"):
        live = "\n".join(
            l
            for l in (IDP / b).read_text().splitlines()
            if not l.strip().startswith("#")
        )
        assert (
            "-reconfigure" in live
            and "-migrate-state" not in live
            and "-force-copy" not in live
        ), b
...
@given(
    "oke-check runs bin/idp-oke-rebuild --check on a pull_request touching platform/oci"
)
def _oke_check(state: dict) -> None:
    wf = (IDP / ".github/workflows/oke-check.yml").read_text()
    assert (
        re.search(
            r"run: bin/idp-oke-rebuild --\$\{\{ inputs\.mode \|\| 'check' \}\}", wf
        )
        and "pull_request" in wf
    )
    state["wf"] = wf
...
def _expect_changes(state: dict) -> None:
    assert re.search(
        r"OKE_CHECK_EXPECT_CHANGES:.*github\.event_name == 'pull_request' && '1' \|\| '0'",
        state["wf"],
    ), state["wf"]
...
@then(
    'bin/idp-recreate-guard refuses with the exact "tofu import" command and no vault is created'
)
...
@then(
    "it refuses with rc 3 and names the import, and switches only when the current vault is empty"
)
