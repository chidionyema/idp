...
_spec = importlib.util.spec_from_file_location(
    "incident_task_pod", IDP / "tests/test_incident_chaos_task_pod_admitted.py"
)
...
def _receipt(state: dict, tmp_path: Path, line: str, minutes: int) -> None:
    ...
    (tmp_path / "head.json").write_text(
        json.dumps(
            {
                "last-modified": format_datetime(when_),
...
def _grade(state: dict) -> None:
    ...
    state["verdict"] = (
        r.stdout.strip().splitlines()[0] if r.stdout.strip() else r.stderr
    )
