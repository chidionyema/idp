...
def _run(repo: Path, *args: str) -> subprocess.CompletedProcess:
    return subprocess.run(
        [sys.executable, str(IDP / "bin" / "trace-matrix"), *args],
        env={**os.environ, "TRACE_MATRIX_ROOT": str(repo)},
        capture_output=True,
        text=True,
    )
...
def _repo(state: dict) -> None:
    ...
    (repo / "tests").mkdir()
    (repo / "features" / "bound.feature").write_text(
        "Feature: bound\n  Scenario: runs\n    Given x\n"
    )
    (repo / "features" / "prose.feature").write_text(
        "Feature: prose\n  Scenario: one\n    Given x\n  Scenario: two\n    Given y\n"
    )
    (repo / "tests" / "test_bound.py").write_text(
...
def _bind(state: dict) -> None:
    (state["repo"] / "tests" / "test_prose.py").write_text(
...
def _prose_counted(state: dict) -> None:
    ...
    assert (
        "| UNBOUND |" not in state["page"]
        and "**1 PROSE**, **0 UNBOUND** of 2" in state["page"]
    )
...
def _order(state: dict) -> None:
    page = state["page"]
    assert page.index("| UNBOUND | `features/prose.feature`") < page.index(
        "| BOUND | `features/bound.feature`"
    )
...
def _finding(state: dict, code: int) -> None:
    assert state["check"].returncode == code, (
        state["check"].stdout + state["check"].stderr
    )
