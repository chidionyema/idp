...
def _gen_then_check(state: dict) -> None:
    ...
    g = subprocess.run(
        [sys.executable, str(IDP / "bin" / "catalog-gen")],
        env={
            **os.environ,
            "INV": str(state["inv"]),
            "OUT": str(state["out"]),
    ...
    state["run"] = subprocess.run(
        [
            sys.executable,
            str(IDP / "bin" / "catalog-links-check"),
            str(state["out"] / "catalog-info.yaml"),
        ],
        capture_output=True,
        text=True,
    )
...
def _check_only(state: dict) -> None:
    state["run"] = subprocess.run(
        [sys.executable, str(IDP / "bin" / "catalog-links-check"), str(state["path"])],
        capture_output=True,
        text=True,
    )
...
def _ok(state: dict, line: str) -> None:
    r = state["run"]
    assert r.returncode == 0 and line in r.stdout, r.stdout + r.stderr
