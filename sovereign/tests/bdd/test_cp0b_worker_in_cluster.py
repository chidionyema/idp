...
def _list(state: dict) -> None:
    r = subprocess.run(
        [str(IDP / "bin/dockerfiles")], capture_output=True, text=True, check=True
    )
    state["rows"] = {
        l.split()[0]: l.split() for l in r.stdout.splitlines() if l.strip()
    }


@then("it lists sovereign-worker built from sovereign-worker.Dockerfile")
def _listed(state: dict) -> None:
    assert state["rows"].get("sovereign-worker") == [
        "sovereign-worker",
        "sovereign-worker.Dockerfile",
        ".",
    ], state["rows"]
...
@then(
    "a Deployment sovereign-worker points TEMPORAL_HOST at the chart's frontend Service"
)
def _host(state: dict) -> None:
    env = {e["name"]: e.get("value") for e in state["c"]["env"]}
    hr = next(d for d in state["docs"] if d["kind"] == "HelmRelease")
    assert env["TEMPORAL_HOST"] == f"{hr['metadata']['name']}-frontend.temporal.svc", (
        env
    )
    assert env["TEMPORAL_NAMESPACE"] in {
        n["name"]
        for n in hr["spec"]["values"]["server"]["config"]["namespaces"]["namespace"]
    }, env


@then(
    "its image is ghcr.io/chidionyema/sovereign-worker on a tag the image policy rewrites"
)
def _image(state: dict) -> None:
    name, tag = state["c"]["image"].rsplit(":", 1)
    assert name == "ghcr.io/chidionyema/sovereign-worker" and tag != "latest", state[
        "c"
    ]["image"]
    ...
    ia = [
        ...
        for d in yaml.safe_load_all(
            (IDP / "platform/image-automation/sovereign-worker.yaml").read_text()
        )
        if d
    ]
    ...
    autos = [
        d
        for f in (IDP / "platform/image-automation").glob("*.yaml")
        for d in yaml.safe_load_all(f.read_text())
        if d and d.get("kind") == "ImageUpdateAutomation"
    ]
    assert len(autos) == 1, autos
    assert "platform/temporal".startswith(
        autos[0]["spec"]["update"]["path"].removeprefix("./")
    ), autos
...
def _probes(state: dict) -> None:
    for probe in ("startupProbe", "readinessProbe", "livenessProbe"):
        assert state["c"][probe]["exec"]["command"] == ["test", "-f", READY], (
            probe,
            state["c"].get(probe),
        )
    env = {e["name"]: e.get("value") for e in state["c"]["env"]}
...
def _pull(state: dict) -> None:
    es = next(
        d
        for d in state["docs"]
        if d["kind"] == "ExternalSecret" and d["metadata"]["name"] == "ghcr-pull"
    )
    ...
    assert {
        p["name"] for p in state["dep"]["spec"]["template"]["spec"]["imagePullSecrets"]
    } == {"ghcr-pull"}
    store = [
        d
        for d in yaml.safe_load_all(
            (IDP / "platform/image-automation/pull-secret.yaml").read_text()
        )
        if d and d["kind"] == "ClusterSecretStore"
    ]
    assert (
        store
        and store[0]["spec"]["provider"]["kubernetes"]["remoteNamespace"] == "backstage"
    ), store
