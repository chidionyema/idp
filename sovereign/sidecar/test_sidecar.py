...
class SidecarTestBase(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self._counter_patch = patch.object(
            config, "RECEIPTS_COUNTER", root / "receipts.counter"
        )
        ...
        self._interventions_patch = patch.object(
            config, "INTERVENTIONS_DIR", root / "interventions"
        )
    ...
    def _legacy_row(self, row_id: str) -> tuple | None:
        return self.conn.execute(
            "SELECT id, lane, note FROM episodes WHERE id = ?", (row_id,)
        ).fetchone()
...
class SidecarWriteMirrorTest(SidecarTestBase):
    def test_property_every_write_diff_reproduces_the_row_and_never_alters_the_legacy_table(
        self,
    ) -> None:
        ...
        for row_id, want in expected.items():
            self.assertEqual(
                self._legacy_row(row_id),
                want,
                "legacy DB must hold exactly the written row",
            )

        write_receipts = self._receipts_of_kind("sidecar_write")
        self.assertEqual(
            len(write_receipts), 200, "one receipt per write, no more, no fewer"
        )

        node_files = sorted(
            p
            for p in self.dag_dir.glob("*.json")
            if p.name != config.SIDECAR_HEAD_FILENAME
        )
        ...
        for path in node_files:
            ...
            self.assertEqual(
                row,
                expected[node["row"]["id"]],
                "the DAG node's row must reproduce the write exactly",
            )
    ...
    def test_a_write_to_another_table_on_the_same_connection_is_ignored(self) -> None:
        ...
        self.assertEqual(
            self.sc.drain(),
            0,
            "no trigger fired for a table this sidecar was never attached to",
        )
        self.assertEqual(self._receipts_of_kind("sidecar_write"), [])
        self.assertEqual(
            list(self.dag_dir.glob("*.json")) if self.dag_dir.exists() else [], []
        )
...
class SidecarDegradedTest(SidecarTestBase):
    def test_incident_cp8_sidecar_never_blocks_the_legacy_write(self) -> None:
        ...
        self.conn.execute(
            "INSERT INTO episodes (id, lane, note) VALUES ('ep-1', 'ops', 'degraded')"
        )
        self.conn.commit()
        processed = self.sc.drain()

        self.assertEqual(
            self._legacy_row("ep-1"),
            ("ep-1", "ops", "degraded"),
            "legacy write must succeed even when the sidecar cannot",
        )
        self.assertEqual(
            processed, 0, "the DAG write failed, so nothing was processed this drain"
        )
        self.assertEqual(self.sc.missed, 1)
        self.assertEqual(self._receipts_of_kind("sidecar_write"), [])
        self.assertEqual(
            self._receipts_of_kind("sidecar_degraded"),
            [],
            "not degraded yet -- still unwritable",
        )
        ...
        self.assertEqual(
            self._legacy_row("ep-1"),
            ("ep-1", "ops", "degraded"),
            "row was never lost, only delayed -- it drains once writable",
        )
        write_receipts = self._receipts_of_kind("sidecar_write")
        self.assertEqual(
            len(write_receipts),
            1,
            "the once-missed write is drained, not dropped, once the DAG is writable again",
        )
    ...
    def test_recovery_via_the_next_drain_also_flushes_degraded(self) -> None:
        ...
        self.addCleanup(lambda: os.chmod(self.dag_dir, stat.S_IRWXU))
        self.conn.execute(
            "INSERT INTO episodes (id, lane, note) VALUES ('ep-1', 'ops', 'a')"
        )
        self.conn.commit()
        self.sc.drain()
        self.assertEqual(self.sc.missed, 1)
        ...
        os.chmod(self.dag_dir, stat.S_IRWXU)
        self.conn.execute(
            "INSERT INTO episodes (id, lane, note) VALUES ('ep-2', 'ops', 'b')"
        )
        self.conn.commit()
        processed = self.sc.drain()

        self.assertEqual(
            processed, 2, "ep-1 (queued) and ep-2 (new) both drain in the one pass"
        )
        self.assertEqual(self.sc.missed, 0)
        self.assertEqual(len(self._receipts_of_kind("sidecar_degraded")), 1)
        self.assertEqual(
            len(self._receipts_of_kind("sidecar_write")),
            2,
            "both ep-1 and ep-2 -- neither write was truly lost",
        )
