...
def register(sub) -> None:
    p_card = sub.add_parser(
        "card", help="Show Otto's pinned-card state (does not send)"
    )
    ...
    p_install = sub.add_parser(
        "install-plugin", help=ck.get("otto.cli_install_plugin_help")
    )
...
def _cmd_card(args) -> int:
    ...
    out = {
        "card_message_id": otto.get("card_message_id"),
        "sends": otto.get("sends", 0),
        "edits": otto.get("edits", 0),
        "lines": len(otto.get("lines", {})),
    }
    if args.json:
        print(json.dumps(out))
    else:
        print(
            f"card_message_id={out['card_message_id']} sends={out['sends']} "
...
def _cmd_install_plugin(args) -> int:
    ...
    if args.json:
        ...
        print(
            "Restart hermes-agent (gateway or CLI session) to load the sovereign plugin. "
