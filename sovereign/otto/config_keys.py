...
OTTO_KEYS: dict[str, tuple[Any, type, str, str]] = {
    ...
    "otto.done_today_window_hours": (
        24,
        int,
        "OTTO_DONE_TODAY_WINDOW_HOURS",
        "Hours a finished session still counts toward 'Done today'",
    ),
    "otto.card_max_button_rows": (
        8,
        int,
        "OTTO_CARD_MAX_BUTTON_ROWS",
        "Max session buttons shown under the pinned card",
    ),
    "otto.session_id_display_chars": (
        8,
        int,
        "OTTO_SESSION_ID_DISPLAY_CHARS",
        "Chars of a session id shown in chat or plugin output",
    ),
    "otto.json_indent": (2, int, "OTTO_JSON_INDENT", "Indent width for otto.json"),
    "otto.card_header_template": (
        "\U0001f4cc ESTATE · {time}",
        str,
        "OTTO_CARD_HEADER_TEMPLATE",
        "Card header line; {time}",
    ),
    "otto.card_counts_template": (
        "Running {running} · Waiting on you {waiting} · Done today {done}",
        str,
        "OTTO_CARD_COUNTS_TEMPLATE",
        "Card counts line",
    ),
    "otto.line_running_template": (
        "▶ {repo} · {task} — step {step}",
        str,
        "OTTO_LINE_RUNNING_TEMPLATE",
        "Per-session line while running",
    ),
    "otto.line_waiting_template": (
        "⏸ {repo} · {task} — needs: {asking}",
        str,
        "OTTO_LINE_WAITING_TEMPLATE",
        "Per-session line while waiting",
    ),
    "otto.line_done_template": (
        "✔ {repo} · {task} · done {time}",
        str,
        "OTTO_LINE_DONE_TEMPLATE",
        "Collapsed line when a session finishes",
    ),
    "otto.line_stopped_template": (
        "\U0001f6d1 {repo} · {task} · stopped {time}",
        str,
        "OTTO_LINE_STOPPED_TEMPLATE",
        "Collapsed line when a session is stopped",
    ),
    "otto.line_denied_template": (
        "❌ {repo} · {task} · denied {time}",
        str,
        "OTTO_LINE_DENIED_TEMPLATE",
        "Collapsed line when a session is denied",
    ),
    "otto.line_failed_template": (
        "⚠️ {repo} · {task} · failed {time}",
        str,
        "OTTO_LINE_FAILED_TEMPLATE",
        "Collapsed line when a session fails",
    ),
    "otto.line_fallback_template": (
        "{repo} · {task}",
        str,
        "OTTO_LINE_FALLBACK_TEMPLATE",
        "Line used for an unrecognized status",
    ),
    "otto.footer_cockpit_url_template": (
        "Cockpit: {url}",
        str,
        "OTTO_FOOTER_COCKPIT_URL_TEMPLATE",
        "Card footer when ESTATE_PUBLIC_URL is set",
    ),
    "otto.footer_laptop_only": (
        "Cockpit: laptop only",
        str,
        "OTTO_FOOTER_LAPTOP_ONLY",
    ...
    "otto.cli_card_reset_help": (
        "Forget otto.json (does not unpin/delete in Telegram)",
        str,
        "OTTO_CLI_CARD_RESET_HELP",
        "--help text for `sb card-reset`",
    ),
    "otto.cli_install_plugin_help": (
        "Symlink the hermes-agent plugin into $HERMES_HOME/plugins",
        str,
        "OTTO_CLI_INSTALL_PLUGIN_HELP",
        "--help text for `sb install-plugin`",
    ),
    "otto.plugin_usage_id_template": (
        "usage: /{cmd} <id>",
        str,
        "OTTO_PLUGIN_USAGE_ID_TEMPLATE",
        "Usage message for id-only plugin commands (show/stop/approve/deny)",
    ),
    "otto.plugin_usage_steer": (
        "usage: /sb-steer <id> <text>",
        str,
        "OTTO_PLUGIN_USAGE_STEER",
