...
Feature: The cluster reports its own state and a runner grades it without a laptop
  ...
  Scenario: A fresh receipt with every node Ready grades ok
    Given a receipt "ok cluster-state at T nodes=2 ready=2 pods=40 pods_not_ready=1 flux=20 flux_not_ready=0 ds=3 ds_short=0 events_warning=0 monitoring_rules=1 alert_watchdog=1" written 5 minutes ago
