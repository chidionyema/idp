...
def _event_message():
    ...
    m = re.search(
        r"^( *)def event_message\(msg: str\) -> str:\n(?:\1 .*\n|\n)+", collect, re.M
    )
...
def test_a_long_oci_refusal_keeps_the_limit_name_at_its_tail():
    f = _event_message()
    msg = (
        "Failed adding 1 nodes to group ocid1.nodepool.oc1.region.aaaa due to OutOfResource.LimitExceeded; "
        "source errors: Error returned by LaunchInstance operation in Compute service."
        + "x" * 400
        + "(400, LimitExceeded) The following service limits were exceeded: standard-a1-core-count."
    )
    out = f(msg)
    assert out.endswith("standard-a1-core-count.") and out.startswith(
        "Failed adding 1 nodes"
    )
