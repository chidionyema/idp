...
def test_every_entity_has_a_description_and_the_interfaces_card_has_every_link(
    tmp_path,
):
...
def test_generator_refuses_an_entity_without_a_description():
    ...
    mod = importlib.util.module_from_spec(
        importlib.util.spec_from_loader("catalog_gen", loader)
    )
    loader.exec_module(mod)
    with pytest.raises(SystemExit) as e:
        mod.entity("Component", "mute", "Mute", "service", {}, [], description="  ")
    assert "mute" in str(e.value) and "describe" in str(e.value)
    assert "description: Speaks." in mod.entity(
        "Component", "loud", "Loud", "service", {}, [], description="Speaks."
    )
