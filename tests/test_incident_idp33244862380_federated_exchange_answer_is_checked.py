...
def _run_check(status, body):
    ...
    env = dict(os.environ, TAILSCALE_API_URL=base, TAILSCALE_FEDERATED_CLIENT_ID="fedid-test",
               ACTIONS_ID_TOKEN_REQUEST_URL=f"{base}/oidc?x=1", ACTIONS_ID_TOKEN_REQUEST_TOKEN="rt",
               GITHUB_REPOSITORY="chidionyema/idp", GITHUB_REF="refs/heads/main")
    try:
        return subprocess.run([str(SCRIPT), "--federated-check"], env=env, capture_output=True, text=True, timeout=30)
    finally:
        srv.shutdown()


def test_a_200_with_a_token_is_ok_and_the_token_is_not_printed():
    r = _run_check(200, {"access_token": "tskey-api-NEVER-PRINTED", "scope": "oauth_keys"})
    ...
    assert "client_id=fedid-test" in _FakeApi.seen[0][1] and "jwt=" in _FakeApi.seen[0][1]


def test_a_refusal_prints_the_full_body_and_exits_non_zero():
    r = _run_check(403, {"message": "subject claim does not match: repo:chidionyema/idp:ref:refs/heads/main"})
...
def test_there_is_no_fall_through_to_the_seed_road():
    text = SCRIPT.read_text()
    block = text[text.index('if [ -n "$fed_id" ]; then'):text.index("seed_tok=$(jq")]
    assert "using the seed" not in text
    assert block.index("federated_exchange") < block.index("else"), "with an identity configured the seed road is never reached"
