...
def test_the_collector_is_its_own_flux_row_and_needs_no_store():
    ...
    assert "observability" not in [d["name"] for d in col["spec"]["dependsOn"]], (
        "lean must stand without the store"
    )
    kust = yaml.safe_load(
        (ROOT / "platform/observability-collector/kustomization.yaml").read_text()
    )
    assert set(kust["resources"]) == {"helmrepository.yaml", "k8s-infra.yaml"}
    assert not (ROOT / "platform/observability/k8s-infra.yaml").exists(), (
        "k8s-infra must live in one row"
    )


def test_the_store_row_waits_for_the_collector_so_the_handoff_relabels_before_prune():
    rows = _rows()
    assert "observability-collector" in [
        d["name"] for d in rows["observability"]["spec"]["dependsOn"]
    ]
...
def test_each_row_owns_a_chart_source_in_its_own_namespace():
    col = next(
        d
        for d in _docs(ROOT / "platform/observability-collector/helmrepository.yaml")
        if d["kind"] == "HelmRepository"
    )
    store = next(
        d
        for d in _docs(ROOT / "platform/observability/helmrepository.yaml")
        if d["kind"] == "HelmRepository"
    )
    kust = yaml.safe_load(
        (ROOT / "platform/observability-collector/kustomization.yaml").read_text()
    ...
    hr = next(
        d
        for d in _docs(ROOT / "platform/observability-collector/k8s-infra.yaml")
