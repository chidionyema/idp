...
def test_k3s_job_is_a_second_distribution_on_the_runner_vm_not_docker() -> None:
    ...
    install = next(
        s for s in k3s["steps"] if "k3s on the runner VM" in s.get("name", "")
    )
    assert re.fullmatch(
        r"v\d+\.\d+\.\d+\+k3s\d+", install["env"]["INSTALL_K3S_VERSION"]
    ), "k3s version must be pinned"
    assert "--disable traefik" in install["env"]["INSTALL_K3S_EXEC"], (
        "the estate's traefik comes from the Flux tree"
    )
    ...
    cmds = "\n".join(
        ln for ln in install["run"].splitlines() if not ln.strip().startswith("#")
    )
    assert "get.k3s.io" in cmds and "k3d" not in cmds
    assert "docker" not in install["run"], (
        "CP2 is a second distribution, not k3d under another name"
    )
...
def test_both_providers_run_the_one_hydration_script_at_the_same_commit() -> None:
    ...
    assert (
        "flux install" in body
        and "flux create source git" in body
        and '--commit="$SHA"' in body
    )
    assert "clusters/oke/estate-config.yaml" in body and "clusters/oke/*.yaml" in body
    assert (
        "bin/idp-hydrate"
        in yaml.safe_load(WF.read_text())[True]["pull_request"]["paths"]
    )
...
def test_k3s_job_grades_the_same_floor_and_prints_wall_clock_and_cost() -> None:
    ...
    assert grade["env"]["PRIVATE"] == "${{ github.event.repository.private }}", (
        "cost is read from the repository, never assumed"
    )
