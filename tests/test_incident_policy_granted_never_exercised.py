...
FN = (
    'source <(sed -n "/^probe_state_write()/,/^}/p" bin/idp-oci-bootstrap); '
    f"probe_state_write estate-tofu-state {TOFU}"
)


def _run(
    tmp_path: Path, put_rc: int, profile_user: str | None = TOFU
    ...
    return subprocess.run(
        ["bash", "-c", FN], cwd=ROOT, env=env, capture_output=True, text=True
    )
