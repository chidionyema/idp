...
ALERTS = (
    ROOT / "platform" / "alerts-github"
)  # crew#406: the ledger has its own Kustomization
...
def test_every_flux_kustomization_event_reaches_a_readable_ledger() -> None:
    providers = {
        d["metadata"]["name"]: d
        for p in ALERTS.glob("provider*.yaml")
        for d in _docs(p)
        if d["kind"] == "Provider"
    }
    ...
    readable = [
        a
        for a in alerts
        if providers.get(a["spec"]["providerRef"]["name"], {})
        .get("spec", {})
        .get("type")
        == "githubdispatch"
        and a["spec"].get("eventSeverity") == "info"
        and {"kind": "Kustomization", "name": "*", "namespace": "flux-system"}
        in a["spec"]["eventSources"]
    ]
    assert readable, (
        "no info-severity Alert forwards every flux-system Kustomization to a githubdispatch Provider"
    )
...
def test_the_dispatch_has_a_workflow_that_prints_it() -> None:
    wf = yaml.safe_load(
        (ROOT / ".github" / "workflows" / "flux-events.yml").read_text()
    )
