...
def test_collector_rows_every_externalsecret_with_its_last_sync_and_the_role_can_read_them():
    ...
    assert "max(2 * secs, 7200)" in collect, (
        "stale rule is 2x refreshInterval with a 2h floor"
    )
...
def test_stale_externalsecret_row_fails_the_receipt_and_names_the_secret():
    row = {
        ...
        "ready": False,
        "last_sync": "2026-08-26T01:00:00Z",
    ...
    assert (
        "ExternalSecret llm/litellm-upstream" in out
        and "older than 2x refreshInterval" in out
    ), out
