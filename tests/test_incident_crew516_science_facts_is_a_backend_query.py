...
def test_incident_crew516_backend_query_grades_both_ways():
    mod = _collector()
    rows = [
        ("spend", "12", "2026-08-27 14:50:00"),
        ("ships", "3", "2026-08-27 09:00:00"),
    ]
    ...
    assert (
        head.startswith("FAIL science-facts")
        and "sources=0" in head
        and "science.source" in head
    ), head
...
def test_incident_crew516_query_is_the_backend_not_a_file():
    ...
    q = mod.QUERY
    assert (
        "signoz_logs.distributed_logs_v2" in q
        and "attributes_string['science.source']" in q
    ), q
...
def test_incident_crew516_receipt_is_wired_end_to_end():
    ...
    args = cron["spec"]["jobTemplate"]["spec"]["template"]["spec"]["containers"][0][
        "args"
    ][0]
    ...
    assert (
        kust["namespace"] == "observability"
        and "science-facts.yaml" in kust["resources"]
    )
