...
def unguarded_jobs(platform: pathlib.Path = PLATFORM) -> list[str]:
    ...
    for path in sorted(platform.rglob("*.yaml")):
        ...
        for d in _docs(path):
            ...
            if not any(
                str(c.get("image", "")).split(":")[0] in images for c in containers
            ):
                continue
...
def test_the_rule_refuses_the_incident_shape(tmp_path):
    ...
    (tmp_path / "job.yaml").write_text(
        job % "  annotations: {kustomize.toolkit.fluxcd.io/ssa: IfNotPresent}"
    )
...
def test_kini_finish_is_created_once_per_name(name):
    ...
    assert (
        d["metadata"]["annotations"]["kustomize.toolkit.fluxcd.io/ssa"]
        == "IfNotPresent"
    )
