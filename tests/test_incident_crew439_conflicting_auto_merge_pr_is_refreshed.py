...
def fake_gh(
    tmp_path: Path, mergeable: str, auto_merge: str = "false"
) -> tuple[Path, Path]:
...
def run(work: Path, bindir: Path) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [str(SCRIPT)], cwd=work, env=env, capture_output=True, text=True
    )
...
def test_a_conflicting_pr_gets_main_merged_and_pushed_before_it_is_armed(tmp_path):
    ...
    assert (
        "was CONFLICTING, merged main" in r.stdout
        and "pushed with the writer key" in r.stdout
    )
    assert r.stdout.strip().endswith("auto-merge armed (--squash)")
    pushed = git(
        tmp_path, "--git-dir", str(origin), "show", "flux/image-updates:k.yaml"
    )
    assert pushed == "newTag: main-783\n#\n#\n#\nother: 2", (
        pushed
    )  # the controller's line won, main's change came along
    calls = log.read_text()
    assert (
        "workflow run" not in calls
    )  # a GITHUB_TOKEN dispatch is not the path; the writer-key push fires ci
...
def test_a_mergeable_pr_is_armed_and_nothing_is_pushed(tmp_path):
    ...
    assert r.returncode == 0 and r.stdout.strip().endswith(
        "#5 auto-merge armed (--squash)"
    ), r.stdout + r.stderr
    assert (
        git(tmp_path, "--git-dir", str(origin), "rev-parse", "flux/image-updates")
        == before
    )
...
def test_a_repository_that_refuses_auto_merge_says_so_and_does_not_go_red(tmp_path):
    ...
    assert (
        "pr merge" not in log.read_text()
    )  # never call the thing the repository refuses
...
def test_an_unknown_mergeable_state_is_never_reported_ok(tmp_path):
    ...
    r = run(work, bindir)
    assert r.returncode == 1 and r.stdout.startswith(
        "FAIL    image-update-pr #5 mergeable=UNKNOWN"
    )
...
def test_the_oke_check_row_fails_on_a_stuck_armed_pr_and_is_ok_when_none_is(tmp_path):
    ...
    stuck.write_text(
        '[{"number": 298, "autoMergeRequest": {"enabledAt": "t"}, "mergeable": "CONFLICTING"},'
        ' {"number": 300, "autoMergeRequest": null, "mergeable": "CONFLICTING"},'
        ' {"number": 301, "autoMergeRequest": {"enabledAt": "t"}, "mergeable": "MERGEABLE"}]'
    )
    r = subprocess.run(
        [str(row), "--input", str(stuck)], capture_output=True, text=True
    )
    assert (
        r.returncode == 1
        and r.stdout.strip() == "FAIL    automerge-stuck  1 stuck: #298 CONFLICTING"
    ), r.stdout
    ...
    r = subprocess.run(
        [str(row), "--input", str(clean)], capture_output=True, text=True
    )
    assert (
        r.returncode == 0
        and r.stdout.strip() == "ok      automerge-stuck  0 stuck of 1 armed"
    ), r.stdout
    assert (
        "run: bin/idp-automerge-stuck"
        in (ROOT / ".github/workflows/oke-check.yml").read_text()
    )
