...
def test_the_three_lanes_are_on_from_git_under_a_stated_cap():
    e, _ = _estate()
    assert (
        e["features"]["watch"] is True
        and e["features"]["work"] is True
        and e["features"]["evolution"] is True
    )
    assert (
        e["limits"]["max_cost_usd_per_task"] <= 5
        and e["limits"]["budget_hard_stop"] is True
    )
...
def test_nothing_secret_sits_in_the_estate():
    ...
    for line in raw.splitlines():
        assert not re.search(r"(token|secret|password|api_key)\s*:\s*\S", line, re.I), (
            line
        )
...
def test_the_gateway_mounts_it_and_points_the_entrypoint_at_it():
    dep = [
        d
        for d in yaml.safe_load_all((DIR / "gateway.yaml").read_text())
