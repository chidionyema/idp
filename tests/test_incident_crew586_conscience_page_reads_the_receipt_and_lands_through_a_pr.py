...
def _render(tmp_path, tenets):
        ...
        json.dumps(
            {
                ...
                "host": "runner",
                "score": {
                    "green": sum(t["ok"] is True for t in tenets),
                    "total": len(tenets),
                },
                "blind": any(t["ok"] is None for t in tenets),
                "tenets": tenets,
            }
        )
    )
    env = {
        **os.environ,
        "CONSCIENCE_REPORT": str(rep),
        "CONSCIENCE_PAGE": str(tmp_path / "p.md"),
        "CONSCIENCE_HISTORY": str(tmp_path / "h.jsonl"),
    }
    return subprocess.run(
        [sys.executable, str(IDP / "bin" / "idp-conscience"), "--page"],
        env=env,
        capture_output=True,
        text=True,
    )
...
def test_page_puts_red_rows_first_and_reads_the_score_from_the_receipt(tmp_path):
    p = _render(
        ...
        [
            _row("aaa", True),
            _row("zzz", False, "exit 1"),
            _row("mmm", None, "exit 2 (BLIND)"),
        ],
    )
...
def test_page_without_a_receipt_is_blind(tmp_path):
    env = {
        **os.environ,
        "CONSCIENCE_REPORT": str(tmp_path / "none.json"),
        "CONSCIENCE_PAGE": str(tmp_path / "p.md"),
    }
    p = subprocess.run(
        [sys.executable, str(IDP / "bin" / "idp-conscience"), "--page"],
...
def test_daily_run_lands_the_page_through_a_pull_request_and_the_portal_lists_it():
    ...
    assert (
        ...
        and wf["permissions"]["pull-requests"] == "write"
    )
