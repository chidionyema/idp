...
def test_the_startup_budget_fits_inside_the_release_timeout():
    ...
    assert budget < timeout, (
        f"startup budget {budget}s >= the install timeout {timeout}s"
    )
    ...
    assert budget > 60, (
        f"startup budget {budget}s is inside the liveness window that killed it"
    )
...
def test_the_probes_the_chart_got_right_are_left_alone():
    ...
    assert "livenessProbe" not in api, (
        "the chart's liveness is correct; patching it invites drift"
    )
    assert "readinessProbe" not in api, (
        "the chart's readiness is correct; patching it invites drift"
    )
