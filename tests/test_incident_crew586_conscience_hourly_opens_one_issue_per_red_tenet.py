...
def test_green_closes_and_only_blind_fails():
    ...
    assert (
        '[ "$rc" -ne 2 ]' in text[text.rindex("- name:") :]
    )  # the last step, never the grade step (run 33200707064)
    ...
    )  # collector, issues, founder line, App token, page
...
def test_receipt_is_kept():
    ...
    assert any(
        "upload-artifact" in (s.get("uses") or "")
        and "conscience.json" in str(s.get("with"))
        for s in steps
    )
