...
def test_helmrelease_otel_endpoints_exist() -> None:
    ...
    assert _helmrelease_otel_endpoints(), (
        "no HelmRelease under platform/ declares spec.values.otelCollectorEndpoint"
    )


def test_deployment_otlp_env_endpoints_exist() -> None:
    assert _deployment_otlp_env_endpoints(), (
        "no Deployment under platform/ sets OTEL_EXPORTER_OTLP_ENDPOINT"
    )
