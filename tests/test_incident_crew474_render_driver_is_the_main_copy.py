...
_spec = importlib.util.spec_from_file_location(
    "catalog_render", _p, loader=SourceFileLoader("catalog_render", str(_p))
)
...
def test_incident_crew474_stale_driver_is_detected_and_current_driver_is_not(tmp_path):
    ...
    assert cr.driver_is_stale(stale, tmp_path / "missing") is False, (
        "no worktree copy: run what we have, never crash"
    )
...
def test_incident_crew474_only_the_scheduled_run_hands_over(tmp_path):
    ...
    assert cr.should_hand_over(branch, on_main, [], {}) is False, (
        "a PR editing the driver runs its own copy"
    )
    assert (
        cr.should_hand_over(
            branch, on_main, ["--scheduled"], {"CATALOG_RENDER_REEXEC": "1"}
        )
        is False
    ), "never loops"
