...
def _route(name: str, ns_file: Path) -> dict:
    ...
    return next(
        d for d in docs if d["kind"] == "HTTPRoute" and d["metadata"]["name"] == name
    )
...
def test_langfuse_public_api_bypasses_the_login_forward_auth() -> None:
    ...
    assert not any(r.get("filters") for r in api["spec"]["rules"]), (
        "the public API route must carry no ForwardAuth filter"
    )
    ...
    for r in ui["spec"]["rules"]:
        ...
        if "/oauth2/" in paths:
            continue
        assert any(
            x.get("extensionRef", {}).get("name") == "login-forward-auth"
            for x in r.get("filters", [])
        ), r
...
def test_trace_drill_is_catalogued_and_reads_keys_only_from_the_vault() -> None:
    ...
    assert not re.search(r"(sk-lf-|pk-lf-|sk-)[A-Za-z0-9]{8,}", script), (
        "a key literal in the drill"
    )
...
def test_github_app_installation_step_runs_even_when_a_drill_row_is_red() -> None:
    ...
    steps = [
        ...
        if "idp-github-app installation" in s.get("name", "")
    ]
...
def test_litellm_upstream_seed_step_runs_on_apply_and_never_echoes_a_value() -> None:
    ...
    steps = [
        ...
        for j in wf["jobs"].values()
        for s in j.get("steps", [])
        if "idp-vault-put litellm-upstream" in s.get("name", "")
    ]
    assert len(steps) == 1, steps
    step = steps[0]
    ...
    run = step["run"]
    assert (
        "bin/idp-vault-put litellm-upstream" in run
        and "LITELLM_MASTER_KEY=LITELLM_MASTER_KEY" in run
    )
    assert "exit 0" in run.split("\n")[0], "no seed -> n/a and exit 0"
    assert not re.search(
        r"echo[^\n]*\$\{?(MINIMAX|DEEPSEEK|OPENROUTER|GEMINI|LITELLM)_", run
    ), "a value echoed"
