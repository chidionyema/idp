...
def test_the_incident_creating_the_cluster_gets_a_second_attempt():
    ...
    assert first.get("continue-on-error") is True, (
        "the first attempt must not fail the job on its own"
    )
    assert first.get("id"), (
        "the first attempt needs an id for the second to be gated on"
    )
...
def test_every_curl_the_drill_makes_is_retried():
    bare = [
        ln for ln in _run_lines() if re.search(r"\bcurl\b", ln) and "--retry" not in ln
    ]
...
def test_every_image_and_chart_the_drill_pulls_is_retried():
    ...
    lines = _run_lines()
    for pattern, what in (
        (r"helm install .*oci://", "a chart pulled from a registry"),
        (r"^docker pull ", "an image pull"),
    ):
        hits = [ln for ln in lines if re.search(pattern, ln)]
        assert hits, (
            f"{what}: nothing matched {pattern!r}; this guard has stopped grading"
        )
        for ln in hits:
            assert "||" in ln or any(
                ln in other and "||" in other for other in lines
...
def test_docker_run_does_not_pull_implicitly():
    ...
    pulled = {
        m.group(1) for ln in lines for m in [re.match(r"docker pull (\S+)", ln)] if m
    }
    for ln in runs:
        images = re.findall(r"\b((?:[\w.-]+/)*[\w.-]+:[\w.+-]+)\b", ln)
        remote = [i for i in images if "/" in i]
        for img in remote:
            assert img in pulled, (
                f"{img} is pulled implicitly by `docker run` and never retried: {ln}"
            )
