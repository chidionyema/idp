...
platform/alerts-secret, in the same namespace, and the GitHub App one carries the three keys
...
def test_incident_crew325_every_alert_provider_secret_has_a_producer() -> None:
    providers = [d for d in _render("platform/alerts") if d["kind"] == "Provider"]
    assert providers
    targets = {
        (d["metadata"]["namespace"], d["spec"]["target"]["name"])
        for d in _render("platform/alerts-secret")
...
def test_incident_crew325_github_dispatch_secret_carries_app_auth_keys() -> None:
    es = next(d for d in _render("platform/alerts-secret") if d["metadata"]["name"] == "github-app")
