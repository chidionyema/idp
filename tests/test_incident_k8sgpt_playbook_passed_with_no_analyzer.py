...
def _run(tmp_path: Path, analyzer_present: bool, applied: str = "", github_sha: str = "") -> tuple[subprocess.CompletedProcess, list[str]]:
    ...
    env = {**os.environ, "PATH": f"{bin_dir}:{os.environ['PATH']}", "GITHUB_SHA": github_sha}
...
def test_incident_20260829_a_stale_flux_revision_is_blind_not_a_false_fail(tmp_path):
    ...
    p, calls = _run(tmp_path, analyzer_present=False, applied="main@sha1:aaaa", github_sha="bbbb")
    assert p.returncode == 0, p.stdout
    assert "BLIND k8sgpt-analyze  Flux has not applied this commit yet: healing-analyzer is at aaaa, this run is bbbb" in p.stdout, p.stdout
    assert "FAIL  k8sgpt-analyze" not in p.stdout
    p, _ = _run(tmp_path, analyzer_present=False, applied="main@sha1:bbbb", github_sha="bbbb")
    assert p.returncode != 0 and "FAIL  k8sgpt-analyze  no analyzer" in p.stdout, p.stdout
