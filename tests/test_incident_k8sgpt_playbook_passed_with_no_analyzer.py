...
def _run(
    tmp_path: Path, analyzer_present: bool, applied: str = "", github_sha: str = ""
    ...
    (bin_dir / "kubectl").write_text(
        ...
        f"  *'get kustomization healing-analyzer'*) printf '%s' '{applied}' ;;\n"
        f"  *'get kustomization healing-analyzer'*) printf '%s' '{applied}' ;;\n"
    ...
    p = subprocess.run(
        [str(PLAYBOOK), "k8sgpt-analyze"], capture_output=True, text=True, env=env
    )
...
def test_an_absent_analyzer_is_a_fail_with_the_operator_log_in_the_receipt(tmp_path):
    ...
    assert (
        "--- k8sgpt-operator-log" in p.stdout
        and 'secret "k8sgpt" not found' in p.stdout
    ), p.stdout
    ...
    assert "--- k8sgpt-describe" in p.stdout and "--- healing-events" in p.stdout, (
        p.stdout
    )
...
def test_flux_waits_for_the_analyzer_deployment_not_just_the_object():
    ...
    assert any(
        c.get("kind") == "Deployment"
        and c.get("name") == "estate"
        and c.get("namespace") == "healing"
        for c in checks
    ), checks


def test_incident_20260829_a_stale_flux_revision_is_blind_not_a_false_fail(tmp_path):
    """Runs 33237896443 and 33238265861 graded the object Flux had not applied yet and printed FAIL."""
    p, calls = _run(
        tmp_path, analyzer_present=False, applied="main@sha1:aaaa", github_sha="bbbb"
    )
    assert p.returncode == 0, p.stdout
    assert (
        "BLIND k8sgpt-analyze  Flux has not applied this commit yet: healing-analyzer is at aaaa, this run is bbbb"
        in p.stdout
    ), p.stdout
    assert "FAIL  k8sgpt-analyze" not in p.stdout
    p, _ = _run(
        tmp_path, analyzer_present=False, applied="main@sha1:bbbb", github_sha="bbbb"
    )
    assert p.returncode != 0 and "FAIL  k8sgpt-analyze  no analyzer" in p.stdout, (
        p.stdout
    )
