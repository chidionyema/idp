...
def _kubeconfig(tmp_path, env):
    return subprocess.run(
        [
            ...
            "cluster",
            "kubeconfig",
            "ocid1.cluster.oc1.uk-london-1.aaaa",
            "--file",
            str(tmp_path / "kc"),
        ],
        capture_output=True,
...
def test_a_token_shaped_run_of_characters_never_reaches_the_line(tmp_path, secret):
    ...
    r = _kubeconfig(
        tmp_path,
        _fake_oci(tmp_path, f"ServiceError: Authorization: Bearer {secret}", 1),
    )
...
def test_a_short_word_survives_so_the_redaction_does_not_eat_the_diagnosis(tmp_path):
    ...
    r = _kubeconfig(
        tmp_path,
        _fake_oci(tmp_path, "ServiceError: code NotAuthorizedOrNotFound status 404", 1),
    )
...
def test_a_failed_call_that_also_printed_to_stdout_leaks_none_of_it(tmp_path):
    ...
    env = _fake_oci(
        tmp_path, "ServiceError: code NotAuthorizedOrNotFound", 1, stdout=on_stdout
    )
...
def test_a_success_is_silent_and_writes_the_file(tmp_path):
    ...
    env = _fake_oci(
        tmp_path, "", 0, stdout="apiVersion: v1  # would be a token-bearing config"
    )
...
def test_idp_kube_tail_one_now_lands_on_the_cause(tmp_path):
    ...
    assert "tail -1" in KUBE.read_text(), (
        "the boundary this test is about still slices that way"
    )
