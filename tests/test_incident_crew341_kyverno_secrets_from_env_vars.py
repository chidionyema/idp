...
def test_t1_secretkeyref_in_env_is_refused():
    ...
    assert "no such overload" not in out, (
        f"CEL expression error, not a policy match: {out}"
    )
...
def test_t3_externalsecret_volume_mount_pattern_passes():
    ...
    assert _fail_count(out) == 0, (
        f"vault-backed volume-mount secrets must pass, got: {out}"
    )
...
def test_t4_pod_with_no_env_at_all_passes():
    ...
    assert _fail_count(out) == 0, (
        f"a pod with no env vars must not be refused, got: {out}"
    )
