...
def _gate(repo: Path) -> subprocess.CompletedProcess:
    return subprocess.run(
        ["bash", str(GATE), "main"], cwd=repo, capture_output=True, text=True
    )
...
def test_incident_touched_feature_is_not_an_executed_one(tmp_path: Path) -> None:
    ...
    assert (
        ok.returncode == 0
        and "ok    spec-gate 1 code file(s) changed with 1 spec" in ok.stdout
    ), ok.stdout + ok.stderr
    assert "residual spec-gate 1 feature file(s) named by no test" in ok.stdout, (
        ok.stdout
    )

    _commit(
        repo,
        {"docs/prose/prose.feature": "Feature: prose, not a spec\n"},
        "prose under docs",
    )
    docs = _gate(repo)
    assert docs.returncode == 0 and "residual spec-gate 1 feature" in docs.stdout, (
        docs.stdout
    )
    ...
    still_ok = _gate(repo)
    assert (
        still_ok.returncode == 0
        and "do not count as executable spec" in still_ok.stdout
        and "features/old.feature" in still_ok.stdout
    ), still_ok.stdout
    ...
    assert (
        fail.returncode == 1
        and "FAIL  spec-gate new *.feature" in fail.stdout
        and "features/new.feature" in fail.stdout
    ), fail.stdout
...
def test_code_change_with_only_an_unbound_feature_is_fail(tmp_path: Path) -> None:
    ...
    assert (
        r.returncode == 1
        and "FAIL  spec-gate 1 code file(s) changed and no executable spec" in r.stdout
    ), r.stdout


def test_incident_crew297_a_docstring_naming_the_feature_is_not_a_binding(
    tmp_path: Path,
) -> None:
    ...
    _commit(
        repo,
        {
            "features/cp1.feature": "Feature: cp1\n",
            "tests/test_cp1.py": '"""covers features/cp1.feature, in prose only"""\nx = 1\n',
        },
        "prose mention",
    )
    prose = _gate(repo)
    assert prose.returncode == 1 and "features/cp1.feature" in prose.stdout, (
        prose.stdout
    )
    assert "residual spec-gate 2 feature file(s) named by no test" in prose.stdout, (
        prose.stdout
    )

    _commit(
        repo,
        {"tests/test_cp1.py": 'scenarios("features/cp1.feature")\n'},
        "real binding",
    )
    real = _gate(repo)
    assert (
        real.returncode == 0
        and "residual spec-gate 1 feature file(s) named by no test" in real.stdout
    ), real.stdout
