...
def test_incident_crew516_render_reads_the_bucket_object_estate9_writes():
    fetch = next(
        s for s in _steps() if "inventory from the bucket" in s.get("name", "")
    )
    assert "state/inventory/latest.json" in fetch["run"]  # the key estate#9 publishes
    ran = " ".join(
        str(s.get("run", "")) + " ".join(map(str, s.get("env", {}).values()))
        for s in _steps()
    )
    assert (
        "~/.estate" not in ran and "/Users/" not in ran
    )  # no Mac path reaches a command, LAW 46
...
def test_incident_crew516_both_checkouts_sit_inside_the_workspace():
    ...
    assert (
        render["env"]["ESTATE_CODE"] == "${{ github.workspace }}"
    )  # the dir holding crew/
...
def test_incident_crew516_keys_reach_rclone_as_environment_never_argv():
    ...
    assert (
        "RCLONE_S3_ACCESS_KEY_ID" in fetch["env"]
        and "RCLONE_S3_SECRET_ACCESS_KEY" in fetch["env"]
    )
...
def test_incident_crew516_the_mac_row_is_retired_and_the_workflow_has_a_schedule():
    ...
    assert _wf()[True]["schedule"], (
        "the cloud render must run on a cron, not only on dispatch"
    )
...
def test_incident_crew516_the_runner_carries_flux_and_reads_the_feed_from_the_bucket():
    ...
    assert len(flux) == 1 and re.search(r"@[0-9a-f]{40}", flux[0]["uses"]), (
        "flux action missing or not sha-pinned"
    )
    ...
    assert "not in the bucket yet" in fetch["run"], (
        "a missing feed must be a named line, not a dead job"
    )
...
def test_incident_crew516_the_feed_receipt_is_the_file_and_the_push_needs_no_vault_beside_a_token():
    ...
    assert re.search(r'\[ -s "\$RUNNER_TEMP/feed\.md" \]', fetch["run"]), (
        "feed taken must be gated on a non-empty file"
    )
    assert _wf()["permissions"].get("packages") == "write"
    push = (ROOT / "bin" / "idp-catalog-push").read_text()
    assert "for t in flux kubectl git; do" in push, (
        "sops must not be a hard requirement"
    )
    code = "\n".join(l for l in push.splitlines() if not l.lstrip().startswith("#"))
    assert "sops" not in code, (
        "crew#227 CP3: the push reads no vault file; gh (GITHUB_TOKEN on Actions) is the only token"
    )
    assert re.search(
        r"gh auth token >/dev/null 2>&1 \|\| \{ echo \"BLIND gh holds no token", push
    ), "no token is BLIND, never a vault read"
