...
pytestmark = pytest.mark.skipif(
    shutil.which("envsubst") is None, reason="envsubst missing"
)


def run(root: str):
    return subprocess.run(
        [str(INSTALLER), "no-such-job"],
        env={**os.environ, "IDP_ROOT": root},
        capture_output=True,
        text=True,
    )
...
TEMP_ROOTS = [
    str(
        Path(tempfile.gettempdir()) / "claude-501" / "wt-idp-oke"
    ),  # the real incident path shape
...
def test_incident_crew280_accepts_the_permanent_checkout():
    ...
    assert (
        "loaded" not in r.stdout
    )  # filter matched no template, nothing was bootstrapped
