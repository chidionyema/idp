...
def _deployment():
    docs = yaml.safe_load_all(
        (
            ROOT / "platform" / "oci" / "autoscaler" / "cluster-autoscaler.yaml"
        ).read_text()
    )
...
def test_spot_pool_is_preemptible_from_zero_and_labelled():
    ...
    assert re.search(r"\bsize\s*=\s*0\b", spot) and re.search(
        r"\bautoscale\s*=\s*true", spot
    )
...
def test_autoscaler_grows_the_spot_pool_from_zero_to_the_founder_number():
    ...
    assert env["SPOT_NODEPOOL_ID"]["valueFrom"]["secretKeyRef"] == {
        "name": "oke-autoscaler",
        "key": "SPOT_NODEPOOL_ID",
    }
    seed = (ROOT / "bin" / "idp-autoscaler-seed").read_text()
    assert (
        "SPOT_NODEPOOL_ID=SPOT_NODEPOOL_ID" in seed
        and "a1-spot" in seed
        and not OCID.search(seed)
    )


def test_radio_room_is_kept_off_preemptible_and_the_rest_prefer_it():
    pol = yaml.safe_load(
        (ROOT / "platform" / "scheduling" / "capacity-affinity.yaml").read_text()
    )
    ...
    req = off["mutate"]["patchStrategicMerge"]["spec"]["affinity"]["nodeAffinity"][
        "requiredDuringSchedulingIgnoredDuringExecution"
    ]
    expr = req["nodeSelectorTerms"][0]["matchExpressions"][0]
    assert expr == {
        "key": "estate.io/capacity",
        "operator": "NotIn",
        "values": ["preemptible"],
    }, "In [on-demand] would strand the radio room on the unlabelled running node"
    pref = rules["everything-else-prefers-preemptible"]["mutate"][
        "patchStrategicMerge"
    ]["spec"]["affinity"]["nodeAffinity"][
        "preferredDuringSchedulingIgnoredDuringExecution"
    ]
    assert pref[0]["preference"]["matchExpressions"][0] == {
        "key": "estate.io/capacity",
        "operator": "In",
        "values": ["preemptible"],
    }
    assert (
        ...
        in (ROOT / "platform" / "scheduling" / "kustomization.yaml").read_text()
    )
...
def _conftest(fixture):
    return subprocess.run(
        [
            ...
            "json",
            "-p",
            str(ROOT / "policy" / "node_pool.rego"),
            str(fixture),
        ],
        capture_output=True,
        text=True,
...
def test_policy_prices_spot_under_the_same_cap_and_refuses_a_spot_price_at_on_demand(
    tmp_path,
):
    ...
    assert (
        under["capacity"]["spot"]["node_usd_hr"]
        == under["capacity"]["burst"]["node_usd_hr"] / 2
    )
