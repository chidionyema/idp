...
def test_run_33236900434_the_analyzer_container_drops_all_and_cannot_escalate():
    ...
    assert _cr()["spec"]["noCache"] is True, (
        "a read-only root filesystem has nowhere for the cache"
    )
...
def test_run_33236900434_only_probes_are_waived_and_only_for_the_analyzer():
    ...
    assert [e["policyName"] for e in ex["spec"]["exceptions"]] == [
        "require-pod-probes",
        "secrets-not-from-env-vars",
    ]  # 2026-08-29: run 33238265861
