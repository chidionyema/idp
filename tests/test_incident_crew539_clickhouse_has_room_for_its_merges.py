...
Guaranteed QoS (requests == limits), and it keeps at least one CPU so loading finishes inside the
probe window."""
...
VALUES = (
    Path(__file__).resolve().parents[1] / "platform" / "observability" / "values.yaml"
)
...
def test_clickhouse_memory_covers_the_measured_ceiling_plus_one_merge():
    ...
    assert _gib(r["limits"]["memory"]) >= 4, (
        "1.80 GiB ceiling + 1.39 GiB merge, measured in run 33140351385"
    )
    assert r["requests"]["memory"] == r["limits"]["memory"], (
        "Guaranteed QoS (crew#539 CP9)"
    )
    assert r["requests"]["cpu"] == r["limits"]["cpu"], "Guaranteed QoS (crew#539 CP9)"
    assert _cpu(r["limits"]["cpu"]) >= 1, (
        "table loading at 500m did not finish inside the startup probe window"
    )
