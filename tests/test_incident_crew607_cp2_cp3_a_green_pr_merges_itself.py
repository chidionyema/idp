...
def _load():
    loader = importlib.machinery.SourceFileLoader(
        "idp_pr_age", str(ROOT / "bin/idp-pr-age")
    )
...
class _Gh:
    def __init__(self, last_comment="", arm_says=""):
        self.calls, self.last_comment, self.arm_says, self.argv0 = (
            [],
            last_comment,
            arm_says,
            "",
...
def test_a_green_row_is_armed_through_the_arm_tool(monkeypatch):
    ...
    mod, rec = _wire(
        monkeypatch, arm_says="ok      pr-arm  #647 auto-merge armed (--merge)\n"
    )
...
def test_a_green_row_the_repository_will_not_arm_waits_on_the_founder(monkeypatch):
    ...
    mod, rec = _wire(
        monkeypatch, arm_says="waiting pr-arm  #647 is open and green-or-pending\n"
    )
...
def test_the_deploy_pull_request_is_never_drafted_or_closed(monkeypatch):
    ...
    assert (
        mod.act("idp", 1011, 25.0, "red:open", 4, mod.DEPLOY_BRANCH)
        == "deploy-untouched"
    )
    assert (
        mod.act("idp", 1011, 7.0, "conflict", 4, mod.DEPLOY_BRANCH)
        == "deploy-untouched"
    )
...
def test_a_row_over_the_bound_is_told_why_once(monkeypatch):
    ...
    assert not any(c[:2] == ["pr", "comment"] for c in rec.calls), (
        "nagged again for the same reason"
    )
    mod, rec = _wire(monkeypatch, last_comment=body)
    assert mod.act("crew", 605, 7.2, "conflict", 4) == "commented", (
        "a new reason is a new line"
    )
...
def test_the_red_clock_drafts_at_the_bound_and_closes_at_six(monkeypatch):
    mod, rec = _wire(monkeypatch)
    assert mod.act("crew", 605, 6.2, "red:review-gate", 4) == "commented"
    assert ["pr", "ready", "605", "-R", "chidionyema/crew", "--undo"] in rec.calls, (
        "drafted at the bound"
    )
    mod, rec = _wire(monkeypatch)
    assert mod.act("idp", 623, 25.0, "conflict", 4) == "closed"
    assert (
        rec.calls[0][:3] == ["pr", "close", "623"]
        and "Reopen when it is green" in rec.calls[0][-1]
    )
    mod, rec = _wire(monkeypatch)
    assert mod.act("idp", 641, 25.0, "blocked-by-policy", 4) == "commented", (
        "only red rows are on the clock"
    )
