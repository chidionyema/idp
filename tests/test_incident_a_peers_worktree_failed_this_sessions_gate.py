...
def test_the_incident_a_sibling_worktree_is_not_part_of_this_tree():
    ...
    try:
        ...
        r = subprocess.run(
            ["ruff", "check", "."], cwd=IDP, capture_output=True, text=True
        )
...
def test_the_pattern_is_anchored_so_it_cannot_swallow_real_directories():
    ...
    lines = [
        ln.strip()
        for ln in (IDP / ".gitignore").read_text().splitlines()
        if ln.strip() and not ln.strip().startswith("#")
    ]
    hits = [ln for ln in lines if re.search(r"\.wt-", ln)]
    assert hits, (
        ".gitignore no longer ignores the sibling worktrees; the incident is back"
    )
    for ln in hits:
        assert ln.startswith("/"), (
            f"{ln!r} is unanchored and would ignore .wt-* at any depth"
        )
