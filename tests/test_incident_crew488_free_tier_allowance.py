...
_loader = importlib.machinery.SourceFileLoader(
    "freetier", str(ROOT / "bin" / "idp-free-tier")
)
ft = importlib.util.module_from_spec(
    importlib.util.spec_from_loader("freetier", _loader)
)
...
def test_grade_both_ways():
    ...
    free = ft.grade(
        {"a1_cores": 2, "a1_memory_gb": 12, "block_gb": 200},
        {**MODULE, "worker_ocpus": 2, "worker_memory_gb": 12},
    )
...
def test_module_numbers_come_from_variables_tf():
    ...
    assert (
        m["free_ocpus"] <= m["worker_ocpus"]
        and m["free_memory_gb"] <= m["worker_memory_gb"]
    ), m
