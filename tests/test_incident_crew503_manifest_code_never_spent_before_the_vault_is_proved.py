...
def test_incident_crew503_a_failed_ci_preflight_stops_before_the_code_is_spent(
    tmp_path,
):
    log = tmp_path / "gh.log"
    ...
    assert "conversions" not in log.read_text(), (
        f"the code was spent although CI could not write: {log.read_text()}"
    )
...
def test_incident_crew503_a_failed_write_after_the_exchange_keeps_the_key(tmp_path):
    ...
    r = _run(
        tmp_path,
        'case "$*" in *"repo view"*) echo o/r;; *conversions*) echo \'' + APP + "';; "
...
def test_incident_crew503_a_good_convert_seeds_the_vault_from_ci_and_keeps_no_key_on_disk(
    tmp_path,
):
    log = tmp_path / "gh.log"
    r = _run(
    ...
    for k in (
        "SEED_GITHUB_APP_ID",
        "SEED_GITHUB_APP_SLUG",
        "SEED_GITHUB_APP_CLIENT_ID",
        "SEED_GITHUB_APP_PEM_B64",
    ):
    ...
    assert not list(tmp_path.glob("tmp.*")), (
        "a key file was left on disk after a good convert"
    )
