...
spec = importlib.util.spec_from_loader(
    "estate_showcase", importlib.machinery.SourceFileLoader("estate_showcase", str(BIN))
)
...
def synth(rng: random.Random) -> list:
    ...
    for i in range(rng.randint(1, 8)):
        ann = {}
        if rng.random() < 0.8:
            ann["estate/last-status"] = rng.choice(
                ["clean", "PASS", "FAIL", "NEVER RUN", "signal 1"]
            )
        docs.append(
            {
                "kind": "Resource",
                "metadata": {"name": f"job-{i}", "annotations": ann},
    ...
    for i in range(rng.randint(0, 4)):
        md = {
            "name": f"repo-{i}",
            "annotations": {
                "estate/dirty": str(rng.choice([0, 0, 3])),
                "estate/offsite": "True",
            },
        }
        if (
            rng.random() < 0.2
        ):  # REWORK 5437556292: a service nobody inventoried is BLIND, never Elite
    ...
    for i in range(rng.randint(0, 3)):
        ann = (
            {"estate/stale": rng.choice(["True", "False"]), "estate/age-h": "3"}
            if rng.random() < 0.8
            else {}
        )
        docs.append(
            {
                "kind": "Resource",
                "metadata": {"name": f"drill-{i}", "annotations": ann},
                "spec": {"type": "drill"},
            }
        )
    return docs
...
def test_properties() -> None:
    for seed in range(200):
        ...
        assert page == mod.render(
            "t", docs, [], None, Path("/x/STANDARDS.md"), Path("/x/S.md")
        ), seed
        for d in docs:
            assert page.count(f"`{d['metadata']['name']}`") == 1, (
                seed,
                d["metadata"]["name"],
            )
            g, _ = mod.grade(d)
        ...
        assert all(
            ...
            or "| BLIND |" in l
            or not l.startswith("| ")
            or l.startswith("| Grade")
            or l.startswith("| —")
            for l in lines[
                lines.index("## Gaps, loudest first") : lines.index(
...
def test_incident_crew474_a_failed_job_is_a_gap_row_and_check_is_proved_both_ways() -> (
    None
):
        ...
        docs = [
            {
                ...
                "metadata": {
                    "name": "nightly-backup",
                    "annotations": {"estate/last-status": "FAIL"},
                },
                "spec": {"type": "scheduled-job"},
            }
        ]
        cat.write_text(
            "# inventory taken: 2026-08-27T00:00:00Z\n" + yaml.safe_dump_all(docs)
        )
        args = [
            sys.executable,
            str(BIN),
            ...
            str(Path(td) / "none.md"),
            "--science",
            str(Path(td) / "none2.md"),
        ]
        assert (
            subprocess.run(args + ["--check"], capture_output=True).returncode == 1
        ), "no page: --check must fail"
        assert subprocess.run(args, capture_output=True).returncode == 0
        page = out.read_text()
        assert (
            "| GAP | scheduled-job | `nightly-backup` | estate/last-status=FAIL |"
            in page
        ), page
        assert "no table at" in page, (
            "missing standards table must be BLIND on the page, not omitted"
        )
        assert (
            subprocess.run(args + ["--check"], capture_output=True).returncode == 0
        ), "fresh page: --check must pass"
        out.write_text(page + "hand edit\n")
        assert (
            subprocess.run(args + ["--check"], capture_output=True).returncode == 1
        ), "drifted page: --check must fail"
...
def test_incident_crew474_guards_and_groups_are_graded_not_blind():
    ...
    spec = importlib.util.spec_from_file_location(
        "es_t", p, loader=SourceFileLoader("es_t", str(p))
    )
    ...
    guard = {
        "kind": "Resource",
        "spec": {"type": "guard"},
        "metadata": {"annotations": {"estate/last-status": "clean"}},
    }
    assert m.grade(guard)[0] == m.ELITE
    assert (
        m.grade({"kind": "Resource", "spec": {"type": "guard"}, "metadata": {}})[0]
        == m.BLIND
    )
    assert (
        m.grade({"kind": "Domain", "metadata": {"description": "the platform"}})[0]
        == m.ELITE
    )
...
def test_incident_5437556292_uninventoried_service_is_blind_not_elite():
    ...
    p = pathlib.Path(__file__).resolve().parents[1] / "bin" / "estate-showcase"
    spec = importlib.util.spec_from_file_location(
        "es_r", p, loader=SourceFileLoader("es_r", str(p))
    )
    m = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(m)
    described = {
        "kind": "Component",
        "spec": {"type": "service"},
        "metadata": {"description": "x", "annotations": {}},
    }
    ...
    inventoried = {
        ...
        "metadata": {
            "description": "x",
            "annotations": {"estate/dirty": "0", "estate/offsite": "True"},
        },
    }
