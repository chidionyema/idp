...
def _judgements() -> list[tuple[str, str]]:
    ...
    for f in _changed_policy_files():
        ...
        for key, rule in new_rules.items():
            ...
            body_new["validate"] = {
                k: v for k, v in rule["validate"].items() if k != "failureAction"
            }
            body_old = None
            if old:
                body_old = {k: v for k, v in old.items()}
                body_old["validate"] = {
                    k: v for k, v in old["validate"].items() if k != "failureAction"
                }
            if old is None or body_new != body_old:
                if action != "Audit":
                    defects.append(
                        (
                            key,
                            "new or changed validate rule must enter as failureAction: Audit",
                        )
                    )
                continue
            ...
            if old_action != "Enforce" and action == "Enforce":
                ...
                if not AUDIT_RECEIPT.search(line):
                    defects.append(
                        (
                            key,
                            "flip to Enforce must name the clean audit run: "
...
def test_the_judge_itself(monkeypatch, tmp_path, head, base, expect):
    ...
    me = sys.modules[
        __name__
    ]  # tests/ has no __init__.py: a dotted target patches a second copy of this module
    monkeypatch.setattr(me, "ROOT", tmp_path)
    monkeypatch.setattr(
        me, "_git", lambda *a: "platform/edge/p.yaml\n" if a[0] == "diff" else base_doc
    )
