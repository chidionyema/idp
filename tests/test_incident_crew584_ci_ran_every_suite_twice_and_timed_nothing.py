...
def _say_block() -> str:
    ...
    b = next(
        ...
        if lines[i] == "}"
        and lines[i - 1].startswith('  if [ -n "${GITHUB_STEP_SUMMARY')
    )
    return "\n".join(lines[a : b + 1]) + "\n"
...
def _pytest_rung() -> str:
    ...
    a = next(
        ...
        if l.startswith('if [ "${IDP_CI_SKIP_PYTEST:-0}" = 1 ]')
    )
    b = next(
        i
        for i in range(a, len(lines))
        if lines[i].startswith("fi  # IDP_CI_SKIP_PYTEST")
    )
    return "\n".join(lines[a : b + 1]) + "\n"
...
def test_in_ci_the_two_suites_run_once_and_the_rung_says_so(tmp_path):
    ...
    out = _bash(
        script,
        {"IDP_CI_SKIP_PYTEST": "1", "PATH": f"{fake}:{os.environ['PATH']}"},
        tmp_path,
    )
    assert out.returncode == 0, out.stderr
    assert (
        "skip  bdd+incident" in out.stdout and "IDP_CI_SKIP_PYTEST=1" in out.stdout
    ), out.stdout
...
def test_locally_the_rung_still_runs_pytest(tmp_path):
    ...
    assert "PYTEST-RAN" in out.stderr or "fail=1" in out.stdout, (
        out.stdout,
        out.stderr,
    )
...
def test_offline_gate_sets_the_skip_and_the_bdd_job_prints_durations():
    ...
    step = next(
        s for s in wf["jobs"]["offline-gate"]["steps"] if s.get("run") == "bin/idp-ci"
    )
    assert step["env"]["IDP_CI_SKIP_PYTEST"] == "1", step["env"]
    runs = [
        s["run"]
        for s in wf["jobs"]["bdd-suites"]["steps"]
...
"""
    ...
    assert (
        "ok    fast     nothing\n" in out.stdout and "nothing  (" not in out.stdout
    ), out.stdout
