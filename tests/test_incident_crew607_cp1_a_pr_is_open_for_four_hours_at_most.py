...
def _pr(repo, n, hours_old, **kw):
    created = (
        f"2026-08-28T{22 - hours_old:02d}:00:00Z"
        if hours_old < 22
...
def test_the_measured_day_is_red_and_every_reason_is_named(tmp_path):
    prs = [
        ...
        _pr(
            ...
            605,
            2,
            statusCheckRollup=[{"name": "review-gate", "conclusion": "FAILURE"}],
        ),
        _pr(
            "idp",
            648,
            1,
            statusCheckRollup=[
                {"name": "hydrate", "status": "IN_PROGRESS", "conclusion": None}
            ],
        ),
    ...
    for needle in (
        "647      1.0  green",
        "641      3.0  blocked-by-policy",
        "623      5.0  conflict",
        "605      2.0  red:review-gate",
        "648      1.0  pending:hydrate",
        "650      1.0  stacked-on:idp#648",
    ):
...
def test_no_authority_clock_is_blind_never_green(tmp_path, monkeypatch):
    ...
    p = subprocess.run(
        [sys.executable, str(BIN), "--json", str(f)],
        capture_output=True,
        text=True,
