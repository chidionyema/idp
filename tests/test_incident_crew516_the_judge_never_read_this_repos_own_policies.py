...
def test_this_repo_ships_cluster_policies_at_all():
    ...
    assert shipped, (
        "no ClusterPolicy under platform/: the tests below would pass on an empty set"
    )
...
def test_the_render_script_loads_them_before_it_judges_anything():
    ...
    assert call < apply_at, (
        "the own policies are loaded after the first render is judged"
    )
