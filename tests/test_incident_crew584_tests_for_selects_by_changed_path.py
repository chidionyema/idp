...
def _repo(tmp_path):
    ...
    (repo / "sovereign" / "tests" / "bdd" / "test_policy.py").write_text(
        "def test_z():\n    assert True\n"
    )
...
def _edit(repo):
    (repo / "platform" / "router.yaml").write_text(
        "a: 2\n"
...
def test_nothing_changed_is_ok_and_runs_nothing(tmp_path):
    repo = _repo(tmp_path)
    out = subprocess.run(
        [str(repo / "bin/idp-tests-for"), "--list", "--base", "HEAD"],
        text=True,
        capture_output=True,
    )
...
def test_yaml_edit_selects_the_test_naming_it_and_a_changed_test_selects_itself(
    tmp_path,
):
    repo = _repo(tmp_path)
    _edit(repo)
    ...
    assert sel == ["sovereign/tests/bdd/test_policy.py", "tests/test_router.py"], (
        out.stdout
    )
...
def test_sovereign_files_run_from_sovereign_and_root_files_from_root(tmp_path):
    ...
    fake.chmod(0o755)
    out = subprocess.run(
        [str(repo / "bin/idp-tests-for"), "--base", "HEAD"],
        text=True,
        capture_output=True,
...
def test_a_docs_only_diff_runs_nothing_and_says_so(tmp_path):
    ...
    (repo / "README.md").write_text(
        "platform/router.yaml is read by test_router\n"
    )  # names a test's path on purpose
    out = _run(repo)
    assert out.returncode == 0, out.stderr
    assert (
        "all docs or comment-only (SKIP_TESTS_ON_TRIVIAL), nothing to run" in out.stdout
    ), out.stdout
...
def test_a_comment_only_edit_is_trivial_but_a_value_edit_is_not(tmp_path):
    ...
    out = _run(repo)
    assert "nothing to run" in out.stdout and "test_router.py" not in out.stdout, (
        out.stdout
    )
    (repo / "platform" / "router.yaml").write_text("# a comment\na: 2\n")
    out = _run(repo)
