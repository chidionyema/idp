...
NO_SETUP_PYTHON = {
    "operating-model-gate.yml::operating-model-gate": "A reusable workflow: it checks the CALLER repo out to `.` and idp to `.idp`, so any "
    ...
    for path, name, steps in _jobs():
        ...
        for step in setups:
            if (step.get("with") or {}).get("cache") != "pip":
                uncached.append(
                    "%s::%s -> %s" % (path.name, name, step.get("name") or step["uses"])
                )
    assert uncached == [], (
...
def test_every_cache_key_hashes_at_least_one_file_that_exists(path):
    ...
    for name, job in (doc.get("jobs") or {}).items():
        ...
        for step in steps:
            ...
            assert dep, "%s::%s caches pip and names no cache-dependency-path" % (
                path.name,
                name,
            )
            ...
            for req in list(hashed):
                ...
                for pulled in _requires(req):
                    if pulled not in hashed:
                        missing.append(
                            "%s::%s hashes %s but not %s, which %s pulls in with -r"
