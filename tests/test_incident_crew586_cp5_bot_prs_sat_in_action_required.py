...
def test_repo_rulesets_reads_the_live_policy_and_repairs_it_from_the_spec():
    ...
    assert re.search(
        r'gh api -X PUT "/repos/\$OWNER/\$repo/actions/permissions/fork-pr-contributor-approval" --input "\$APPROVAL"',
        SCRIPT,
    )
    assert 'apply_pr_approval "$repo"' in SCRIPT, (
        "the rung runs for every repository, not only idp"
    )
