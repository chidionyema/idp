...
def _gen(tmp_path, repos):
    ...
    inv.write_text(
        json.dumps({"at": "2026-08-27T00:00:00Z", "findings": [], "rows": repos})
    )
    out = tmp_path / "out"
    out.mkdir()
    res = subprocess.run(
    ...
    for f in out.rglob("*.yaml"):
        for d in yaml.safe_load_all(f.read_text()):
            if (
                d
                and d.get("kind") == "Component"
                and d["metadata"]["name"] in ("with-docs", "no-docs", "local-docs")
            ):
                refs[d["metadata"]["name"]] = (
                    d["metadata"]
                    .get("annotations", {})
                    .get("backstage.io/techdocs-ref")
                )
    return refs
        ...
        [
            {
                "kind": "repo",
                "id": "with-docs",
                "path": str(tmp_path / "with-docs"),
                "remote": "git@github.com:example/with-docs.git",
                "_mkdocs": True,
            },
            {
                "kind": "repo",
                "id": "no-docs",
                "path": str(tmp_path / "no-docs"),
                "remote": "git@github.com:example/no-docs.git",
            },
            {
                "kind": "repo",
                "id": "local-docs",
                "path": str(tmp_path / "local-docs"),
                "remote": None,
                "_mkdocs": True,
            },
        ],
