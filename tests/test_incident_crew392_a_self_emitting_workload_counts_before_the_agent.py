...
def _collector() -> types.ModuleType:
    docs = [
        d
        for d in yaml.safe_load_all((OBS / "telemetry-coverage.yaml").read_text())
        if d
    ]
...
def test_incident_crew392_a_pod_seen_only_in_traces_counts_and_an_unseen_pod_still_fails():
    ...
    assert head.startswith("FAIL telemetry-coverage") and "seen=1 missing=1" in head, (
        head
    )
    assert body["missing"] == [{"ns": "mcp", "pod": "github-mcp-1"}], (
        "the pod in no table is still missing"
    )
    ...
    def both(sql):
        ...
        if "hubble" in sql:
            return {("4",)}
        return (
            {("mcp", "agentgateway-1"), ("mcp", "github-mcp-1")}
            if "signoz_traces" in sql
            else set()
        )
...
def test_incident_crew392_collector_stamps_the_pod_on_traces_and_logs():
    ...
    assert cfg["processors"]["k8sattributes"]["pod_association"] == [
    ...
    for name in ("traces", "logs"):
        ...
        assert "receivers" not in pipelines[name], (
            "receivers are the chart's; a list here would replace them"
        )
    assert "metrics" not in pipelines, (
        "metrics arrive through the agent and keep the shipped pipeline"
    )
...
def test_incident_crew392_rendered_chart_keeps_the_shipped_receivers():
    ...
    hr = next(
        d
        for d in yaml.safe_load_all((OBS / "signoz.yaml").read_text())
    ...
    assert (
        traces["processors"][0] == "k8sattributes" and "otlp" in traces["receivers"]
    ), traces
