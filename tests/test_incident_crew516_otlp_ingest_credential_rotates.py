...
USERS = re.compile(
    r'resource "oci_vault_secret" "otlp_ingest_users" \{(?P<body>.*?)\n\}', re.S
)
...
def rotation_ok(text: str) -> tuple[bool, str]:
    ...
    if (
        "ignore_changes" not in body
        or "secret_content" not in body.split("ignore_changes", 1)[1].split("]", 1)[0]
    ):
        return (
            False,
            "the users entry does not ignore its own bcrypt re-salt; every apply rewrites it",
        )
    if (
        "replace_triggered_by" not in body
        or "random_password.otlp_ingest"
        not in body.split("replace_triggered_by", 1)[1].split("]", 1)[0]
    ):
        return (
            False,
            "the users entry is not replaced when the password is; a rotation never reaches the vault",
        )
    if "bcrypt(random_password.otlp_ingest.result)" not in body:
        return (
            False,
            "the users line is not the bcrypt of the same password the sender reads",
        )
...
def test_the_committed_file_rotates() -> None:
    ...
    assert ok, why
    assert (
        "otlp-ingest-password" in TF.read_text()
        and "base64encode(random_password.otlp_ingest.result)" in TF.read_text()
    )
