...
@pytest.mark.parametrize(
    "path,policy,rule", RULES, ids=[f"{p}/{r['name']}" for _, p, r in RULES]
)
def test_every_validate_rule_names_operations_and_judges_a_delete_only_on_purpose(
    path, policy, rule
):
    for b in _blocks(rule):
        ops = b.get("operations")
        assert ops, (
            f"{path} {policy}/{rule['name']}: a match block names no operations, so it judges DELETE too"
        )
        if "DELETE" in ops:
            assert "delete" in rule["name"], (
                f"{path} {policy}/{rule['name']}: judges DELETE without saying so in its name"
            )
