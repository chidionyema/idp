...
def _value(flt, value):
    ...
    if "$contains" in flt:
        return isinstance(value, list) and any(
            _pred(flt["$contains"], v) for v in value
        )
    if "$exists" in flt:
...
def test_apis_tab_shows_only_where_an_api_relation_exists():
    ...
    with_api = dict(
        surfaces[0], relations=[{"type": "providesApi", "targetRef": "api:default/x"}]
    )
...
def test_the_port_is_pinned_to_the_installed_filter_predicates_version():
    ...
    if not PREDICATES_PKG.exists():
        ...
        pytest.skip(
            "backstage/node_modules not installed here; CI's backstage job has it"
        )
...
def test_the_port_matches_the_library_on_dotted_annotation_keys():
    ...
    assert _pred(
        {"metadata.annotations.backstage.io/techdocs-ref": {"$exists": True}}, entity
    )
    assert not _pred(
