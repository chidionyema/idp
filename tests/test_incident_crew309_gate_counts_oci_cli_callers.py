...
def test_callers_are_counted_and_comments_prose_and_clean_files_are_not(tmp_path):
    ...
    assert "cloud-agnostic-gate: 3 operator file(s) bound to the oci CLI" in r.stdout, (
        r.stdout
    )
...
def test_an_untracked_caller_in_a_checkout_is_not_counted(tmp_path):
    ...
    subprocess.run(
        ["git", "add", "bin/idp-clean", "bin/idp-py", ".github/workflows/drill.yml"],
        cwd=root,
        check=True,
    )
    r = _gate(root)
    assert "cloud-agnostic-gate: 2 operator file(s) bound to the oci CLI" in r.stdout, (
        r.stdout
    )
...
def test_the_live_number_is_the_one_the_audit_measured():
    ...
    assert n <= 20, (
        f"{n} operator files call the oci CLI; the measured ceiling is 20 — a new one needs its own reason"
    )
