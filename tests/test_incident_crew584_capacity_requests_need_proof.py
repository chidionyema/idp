...
CPU_BUDGET_CORES = 7.2  # 6.9 -> 7.2 on 2026-09-04 (crew#768): the otto-gateway and event-bus rows were merged dark because waking them moves their requests out of the `off` bucket and into this sum. Measured from origin/main: charged 6.985 with both rows dark, so the woken total measures 7.085 (the sum is recomputed, not added). Measured on the running cluster at 11:44Z: two nodes, 5808m allocatable each, 11616m total, 9522m requested across every namespace -- 2094m idle on nodes the estate already pays for, so waking the two rows buys no node and changes no bill. The ceiling moves to 7.2 and no further. Earlier: 5.4 -> 6.9 on 2026-08-29: langfuse web and worker at 250m still never booted (crew#626 CP15, diagnose run 33262424618: 10 restarts, liveness deadline exceeded; helm-retry 33260661133 stalled rollout); one core each, Guaranteed, labelled capacity-approved. Earlier: 5.0 measured 2026-08-29 after the trim (7.64 -> 5.00); +0.4 on the same day: langfuse web and worker at 50m never booted inside the liveness window (crew#626 CP15, run 33259031857: 164 restarts, exit 143), 250m each is the fence line. clickhouse 1.0 and prometheus 0.2 stay Guaranteed (crew#539 CP9), balloon 0.3 is the reserve; a ratchet: lowered on a measurement, raised only on an incident receipt
