...
def test_no_job_sleeps_the_full_600s_on_kubectl_wait():
    ...
    assert not re.search(r"kubectl wait kustomization --all", text), (
        "the fixed 600 s wait is back"
    )
    assert text.count("run: bin/idp-drill-wait") == 2, (
        "both hydrate and k3s must use the floor wait"
    )
...
def test_the_wait_settles_past_the_floor_so_a_run_can_still_beat_it(tmp_path):
    """crew#488, 2026-08-29: exiting the instant the floor is met broke the floor's own ratchet.

    drills/portability-floor.txt says "Only goes up: raise it in the PR that carries the green run
    URL that beat it." A wait that returns on the first poll at or above the floor makes that
    impossible -- the grade step reads the same cluster a fraction of a second later, so the run
    can never report more than the floor. Measured on the same tree: 11/41 with the old 600 s
    settle (run 33220112386), then exactly 9/42 twice with the floor at 9 (run 33225061822, jobs
    99026942901 and 99026943071). The number stopped being a measurement and became an echo of
    drills/portability-floor.txt.

    DRILL_WAIT_GRACE keeps polling for a bounded stretch after the floor is met. 60 s on a wait
    that now costs ~200 s, against the 600 s crew#584 removed.
    """
    n = _floor()
    items = ",".join(
        '{"metadata":{"namespace":"a","name":"k%d"},"status":{"conditions":[{"type":"Ready","status":"True"}]}}'
...
def test_the_grace_can_be_switched_off_for_a_cheap_run(tmp_path):
    ...
    assert out.returncode == 0, out.stderr
    assert "floor met" in out.stdout, out.stdout
    assert "settling" not in out.stdout, out.stdout
