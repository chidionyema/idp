...
def test_incident_crew480_github_provider_dependency_is_pinned():
    ...
    assert version, (
        "backend package.json must depend on the github catalog provider module"
    )
...
def test_incident_crew480_kubernetes_locator_reads_the_in_cluster_service_account():
    ...
    assert clusters, (
        "kubernetes.clusterLocatorMethods must declare at least one cluster"
    )
    for c in clusters:
...
def test_incident_crew480_a_reverted_literal_cluster_url_is_refused(tmp_path):
    ...
    text = APP_CONFIG_CONTAINER.read_text()
    assert "${K8S_CLUSTER_URL" in text, (
        "fixture assumption: the real file uses a substitution"
    )
    broken = text.replace(
    ...
    assert no_subst, (
        "reverting to a literal must be the thing the real-file test refuses"
    )
