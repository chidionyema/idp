...
def test_alert_pages_on_the_image_automation_kinds():
    alert = next(
        d
        for d in yaml.safe_load_all(ALERT.read_text())
        if d and d["metadata"]["name"] == "broken-workload"
    )
    ...
    for kind in (
        ...
        assert (kind, "flux-system") in kinds, (
            f"broken-workload Alert has no row for {kind}"
        )
