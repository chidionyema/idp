...
def test_keda_is_installed_once_pinned_and_emitting():
    releases = {
        ...
        and str(d["spec"]["chart"]["spec"].get("chart", "")).startswith("keda")
    }
    names = sorted(n for _, n in releases)
    assert names == ["keda", "keda-add-ons-http"], names
    for (path, name), d in releases.items():
        assert path.parent == PLATFORM / "keda", (
            f"{name} lives in {path}, not platform/keda"
        )
        version = str(d["spec"]["chart"]["spec"].get("version", ""))
        assert re.fullmatch(r"\d+\.\d+\.\d+", version), (
            f"{name} chart version not pinned: {version!r}"
        )
...
def test_no_second_autoscaler_under_platform():
    ...
    for path, d in _docs():
        ...
        if (
            kind in forbidden_kinds
            or api.startswith("serving.knative.dev")
            or api.startswith("operator.knative.dev")
        ):
        ...
        if (
            kind == "CronJob"
            and "scale" in yaml.safe_dump(d.get("spec", {}))
            and "replicas" in yaml.safe_dump(d.get("spec", {}))
        ):
