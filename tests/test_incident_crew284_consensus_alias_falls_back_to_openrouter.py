...
def test_incident_crew284_each_consensus_alias_falls_back_to_openrouter_first():
    for rel in ("platform/llm/config.yaml", "llm/config.yaml"):
        ...
        fallbacks = {
            k: v for e in cfg["router_settings"]["fallbacks"] for k, v in e.items()
        }
        for alias in CONSENSUS:
            ...
            assert first["api_key"] != f"os.environ/{alias.upper()}_API_KEY", (
                rel,
                alias,
                chain[0],
            )
            ...
            other = [
                h
                for h in chain
                if models[h]["api_key"] != models[alias]["api_key"]
                and not models[h]["model"].startswith("openrouter/")
            ]
