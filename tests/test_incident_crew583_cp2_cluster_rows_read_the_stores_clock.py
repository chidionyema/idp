...
ROWS = {
    "bin/idp-cluster-state": (
        "cluster-state",
        "CLUSTER_STATE_MAX_AGE_MIN",
        "ok cluster-state at 2026-08-29T02:00:03Z nodes=1 ready=1 pods=3 pods_not_ready=0 flux=1 "
        "flux_not_ready=0 ds=1 ds_short=0 deploy_short=0 events_warning=0 monitoring_rules=3 alert_watchdog=1 cpu_used_pct=30 mem_used_pct=25 "
        'cpu_req_pct=12 mem_req_pct=4 secret_stale_consumers=0\n{"capacity": [], "capacity_error": ""}\n',
    ),
    "bin/idp-telemetry-coverage": (
        "telemetry-coverage",
        "TELEMETRY_COVERAGE_MAX_AGE_MIN",
...
def test_a_receipt_stamped_ahead_of_the_stores_clock_is_never_ok(tmp_path, script):
    ...
    head = {
        "last-modified": "Sat, 03 Oct 2027 12:00:00 GMT",
...
@pytest.mark.parametrize("script", sorted(ROWS))
def test_a_fresh_receipt_by_the_stores_clock_is_ok_whatever_this_machine_says(
    tmp_path, script
):
    ...
    head = {
        ...
        "date": "Tue, 01 Jan 2019 12:05:00 GMT",
    }
    r = _run(tmp_path, script, head, ROWS[script][2])
