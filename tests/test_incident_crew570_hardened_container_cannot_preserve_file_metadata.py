...
PRIVILEGED_METADATA_FLAGS = (
    (
        re.compile(r"\bcp\s+(?:-\w*\s+)*-\w*a"),
        "cp -a is -dR --preserve=all: it utimensat()s and chown()s the destination",
    ),
    (
        re.compile(r"\bcp\s+(?:-\w*\s+)*-\w*p"),
        "cp -p preserves timestamps and ownership",
    ),
    (
        re.compile(r"\bcp\b[^|;&]*--preserve(?!=mode\b)(?!=links\b)"),
        "cp --preserve asks for attributes a dropped capability owns",
    ),
    (
        re.compile(r"\btar\s+(?:-\w*\s+)*-?\w*p"),
        "tar -p restores permissions and ownership from the archive",
    ),
    (
        re.compile(r"\brsync\s+(?:-\w*\s+)*-\w*[aot]"),
        "rsync -a/-o/-t preserve ownership and times",
    ),
)
...
def test_the_platform_actually_declares_containers_to_grade() -> None:
    ...
    assert len(containers) >= 10, (
        f"only {len(containers)} containers found under platform/ -- the walk is broken, not the estate"
    )
...
def test_the_robusta_init_container_is_in_scope() -> None:
    ...
    assert named, (
        "setup-venv was not found: the walk no longer descends into postRenderer patches"
    )
    assert any(_shell(c) for _, c in named), (
        "setup-venv was found but carries no command to grade"
    )
...
@pytest.mark.parametrize(
    "pattern,why",
    PRIVILEGED_METADATA_FLAGS,
    ids=lambda v: getattr(v, "pattern", "")[:24],
)
def test_no_hardened_container_asks_the_kernel_to_preserve_file_metadata(
    pattern, why
) -> None:
    offenders = []
    for path, container in _platform_containers():
        ...
        if shell and pattern.search(shell):
            offenders.append(
                f"{path}: container {container.get('name')!r}: {shell.strip()[:120]}"
            )
...
def test_the_fix_did_not_quietly_add_the_capability_back() -> None:
    ...
    for path, container in _platform_containers():
        ...
        if forbidden:
            offenders.append(
                f"{path}: container {container.get('name')!r} adds {sorted(forbidden)}"
            )
    assert not offenders, (
