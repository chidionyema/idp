...
def _receipt(ds_short: list, events: list, with_count: bool = True) -> str:
    ...
    return (
        ...
        + "\n"
        + json.dumps(
            {"flux_not_ready": [], "ds_short": ds_short, "events_warning": events}
        )
    )
...
def test_grader_fails_on_a_short_daemonset_and_prints_the_event_that_names_it():
    ...
    assert (
        "observability/k8s-infra-otel-agent ready=0/1" in out
        and "FailedCreate x12" in out
    ), out
