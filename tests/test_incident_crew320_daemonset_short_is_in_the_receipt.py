...
def _receipt(ds_short: list, events: list, with_count: bool = True) -> str:
    ...
    if with_count:
        head += f" ds=3 ds_short={len(ds_short)} deploy_short=0 events_warning={len(events)} monitoring_rules=1 alert_watchdog=1 cpu_used_pct=30 mem_used_pct=25 cpu_req_pct=12 mem_req_pct=4"
