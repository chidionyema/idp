...
@pytest.mark.parametrize(
    "path,workload,container,volumes",
    CASES,
    ids=[f"{p}::{n}/{c['name']}" for p, n, c, _ in CASES],
)
def test_readonly_root_container_mounts_a_writable_tmp(
    path, workload, container, volumes
):
...
def test_the_rule_sees_estate_mcp():
    assert any(
        n == "estate-mcp" and c["name"] == "estate-mcp" for _, n, c, _ in CASES
    ), "estate-mcp fell out of the sweep"
