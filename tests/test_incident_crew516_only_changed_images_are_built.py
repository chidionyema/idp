...
def test_unknown_ref_and_build_machinery_changes_build_everything(tmp_path):
    repo = _repo(tmp_path)
    assert _names(
        repo, "--changed-since", "0000000000000000000000000000000000000000"
    ) == ["alpha", "beta"]
    (repo / "bin" / "dockerfiles").write_text(
        (repo / "bin" / "dockerfiles").read_text() + "# touched\n"
    )
...
def test_workflow_discovers_against_the_base():
    ...
    assert (
        "origin/${{ github.base_ref }}" in text and "${{ github.event.before }}" in text
    )
