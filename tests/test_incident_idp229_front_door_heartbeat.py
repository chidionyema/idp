...
def test_pulse_is_a_restricted_cronjob_writing_the_object_the_reader_reads():
    cj = [
        d
        for d in _docs("platform/identity/front-door-heartbeat.yaml")
        if d["kind"] == "CronJob"
    ][0]
    assert (
        cj["spec"]["schedule"] == "*/5 * * * *"
    ...
    assert (
        env["DOOR_URL"] == "https://catalogue.${ESTATE_ZONE}/"
        and env["IDENTITY_URL"] == "${ESTATE_OIDC_DOMAIN_URL}"
    )
    assert (
        "front-door-heartbeat.yaml"
        in (ROOT / "platform/identity/kustomization.yaml").read_text()
    )
...
def test_incident_idp229_catalogue_names_the_workflow_and_its_cron_verbatim():
    for d in yaml.safe_load((ROOT / "drills/catalogue.yaml").read_text())["drills"]:
        ...
        crons = [
            s["cron"]
            for s in (
                yaml.safe_load(wf.read_text()).get(True)
                or yaml.safe_load(wf.read_text()).get("on")
            )["schedule"]
        ]
