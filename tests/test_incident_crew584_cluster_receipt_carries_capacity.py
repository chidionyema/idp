...
HEAD = json.dumps(
    {
        "last-modified": "Thu, 27 Aug 2026 19:00:03 GMT",
        "date": "Thu, 27 Aug 2026 19:00:05 GMT",
        "content-length": "10",
    }
)
MANIFEST = ROOT / "platform" / "state" / "cluster-state.yaml"
GREEN = (
    "nodes=1 ready=1 pods=3 pods_not_ready=0 flux=1 flux_not_ready=0 ds=1 ds_short=0 deploy_short=0 events_warning=0"
...
def test_rbac_grants_the_metrics_api_the_collector_reads():
    ...
    assert (
        rule
        and "nodes" in rule[0]["resources"]
        and {"get", "list"} <= set(rule[0]["verbs"])
    ), role["rules"]
    ...
    fake = {
        ...
        "/api/v1/pods": {
            "items": [
                ...
                {
                    "metadata": {"name": "done", "namespace": "a"},
                    "spec": {
                        "nodeName": "n1",
                        "containers": [
                            {
        ...
        "/apis/metrics.k8s.io/v1beta1/nodes": {
            "items": [
                {
                    "metadata": {"name": "n1"},
                    "usage": {"cpu": "1200m", "memory": "6291456Ki"},
                }
            ]
        },
    }
    start = src.index("def qty(v):")
    end = src.index('head = (f"ok cluster-state')
    block = src[start:end]
    ns = {
        "get": lambda p: fake[p],
        "nodes": fake["/api/v1/nodes"]["items"],
        "pods": fake["/api/v1/pods"]["items"],
    }
    exec(block, ns)
    row = ns["capacity"][0]
    assert row == {
        ...
        "cpu_used_pct": 30,
        "memory_used_pct": 25,
        "cpu_requested_pct": 12,
        "memory_requested_pct": 4,
    }, row
    assert (
        ns["cpu_used_pct"],
        ns["mem_used_pct"],
...
def test_reader_prints_the_capacity_row_in_plain_words(tmp_path):
    ...
    assert (
        "ok      capacity       cpu used 30% requested 12%, memory used 25% requested 4% of what is paid for"
        in r.stdout
    ), r.stdout
...
def test_reader_fails_a_receipt_without_capacity_or_with_unreadable_metrics(tmp_path):
    ...
    assert (
        r.returncode == 1
        and "FAIL    capacity       receipt carries no cpu_used_pct" in r.stdout
    ), r.stdout + r.stderr
    blind = (
        f"ok cluster-state at 2026-08-29T02:00:03Z {GREEN} cpu_used_pct=-1 mem_used_pct=-1 cpu_req_pct=12 mem_req_pct=4 secret_stale_consumers=0\n"
        + json.dumps(
            {"capacity": [], "capacity_error": "metrics.k8s.io nodes: 403 forbidden"}
        )
        + "\n"
    )
    ...
    assert (
        r.returncode == 1
        and "FAIL    capacity       usage unreadable: metrics.k8s.io nodes: 403 forbidden"
        in r.stdout
    ), r.stdout + r.stderr
