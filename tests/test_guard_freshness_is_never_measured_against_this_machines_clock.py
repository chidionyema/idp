...
LOCAL_CLOCK = re.compile(
    r"datetime\.now\(|\btime\.time\(\)|\bdate \+%s\b|dt\.datetime\.now\("
)
...
def _offenders() -> dict[str, list[str]]:
    ...
    for p in sorted(ROOT.joinpath("bin").rglob("*")):
        ...
        if LOCAL_CLOCK.search(src) and FOREIGN_STAMP.search(src):
            ...
            out[rel] = [l.strip() for l in src.splitlines() if LOCAL_CLOCK.search(l)][
                :3
            ]
    return out
...
def test_the_known_list_only_ever_shrinks(path):
    ...
    assert ROOT.joinpath(path).is_file(), (
        "%s no longer exists -- delete it from NOT_YET_MIGRATED" % path
    )
...
def test_the_one_place_the_local_clock_is_allowed_near_an_age_does_not_use_it():
    ...
    src = _code_only(
        ROOT.joinpath("bin/lib/receipt_age.py").read_text(encoding="utf-8")
    )
    assert not LOCAL_CLOCK.search(src), (
        "receipt_age.py asks this machine what time it is"
    )
...
def test_the_authority_stamps_both_ends_and_grades_neither():
    ...
    for path in sorted(THE_AUTHORITY):
        ...
        assert "total_seconds()" not in src, (
            "%s now measures an interval; it cannot be exempt" % path
        )
        assert "max_age" not in src.lower(), (
            "%s now grades an age; it cannot be exempt" % path
        )
