...
def test_incident_crew495_signoz_root_admin_comes_from_the_vault() -> None:
    ...
    assert "SIGNOZ_USER_ROOT_PASSWORD" not in env, (
        "the password must come from the vault, not git"
    )
    ...
    assert targets["signoz.env.SIGNOZ_USER_ROOT_PASSWORD"] == (
        "signoz-root",
        "password",
    )
...
def test_incident_crew495_the_chain_breaks_when_a_link_is_missing() -> None:
    ...
    assert not broken <= written, (
        "a key the ExternalSecret never writes must be refused"
    )
