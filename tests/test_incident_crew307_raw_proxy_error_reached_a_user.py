...
def test_every_application_rule_answers_errors_with_the_status_page_and_sets_edge_headers():
    ...
    for f, name, rule in _app_rules():
        names = {
            ...
            if fl.get("type") == "ExtensionRef"
            and fl["extensionRef"]["kind"] == "Middleware"
        }
        assert {"friendly-errors", "edge-headers"} <= names, (
            f"{f}: rule of {name} lacks {names}"
        )
        seen += 1
    assert seen >= 10, seen
...
def test_every_route_namespace_carries_both_middlewares_and_wires_them():
    for f in ROUTE_FILES:
        ns = next(
            d["metadata"]["namespace"] for d in _docs(f) if d.get("kind") == "HTTPRoute"
        )
        mw = pathlib.Path(f).with_name("edge-manners.yaml")
        assert mw.exists(), mw
        kinds = {(d["kind"], d["metadata"]["name"]) for d in _docs(mw)}
        assert {
            ("Middleware", "friendly-errors"),
            ("Middleware", "edge-headers"),
        } <= kinds, mw
        assert all(d["metadata"]["namespace"] == ns for d in _docs(mw)), mw
        assert "edge-manners.yaml" in (mw.parent / "kustomization.yaml").read_text(), (
            mw.parent
        )
...
def test_the_status_page_serves_every_status_the_middleware_rewrites():
    ...
    mw = next(
        d
        for d in _docs(ROOT / "platform/guacamole/edge-manners.yaml")
        if d["metadata"]["name"] == "friendly-errors"
    )
    ...
    assert (
        dep["spec"]["template"]["spec"].get("priorityClassName")
...
def test_traefik_can_reach_the_status_page_across_namespaces_and_writes_an_access_log():
    values = next(
        d
        for d in _docs(ROOT / "platform/edge/traefik.yaml")
