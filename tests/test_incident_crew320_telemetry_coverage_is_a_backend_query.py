...
def test_incident_crew320_backend_query_grades_both_ways():
    ...
    seen = {"logs": {("kini", "kini-worker-1")}, "metrics": {("llm", "litellm-router-1")}}
    head, body = mod.main(kube=kube, clickhouse=lambda sql: seen["logs" if "logs" in sql else "metrics"], now=NOW)
    assert head.startswith("ok telemetry-coverage") and "pods=2 seen=2 missing=0" in head, head
    assert body["missing"] == [] and body["backend_errors"] == {}

    # FAIL: neither table has heard from the router in the window, and the receipt names it.
    head, body = mod.main(kube=kube, clickhouse=lambda sql: {("kini", "kini-worker-1")}, now=NOW)
