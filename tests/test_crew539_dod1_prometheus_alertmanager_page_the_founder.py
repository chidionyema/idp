...
def test_flux_row_substitutes_the_zone_and_waits_on_both_releases():
    ...
    assert set(kz["resources"]) == {"namespace.yaml", "alertmanager-config.yaml", "kube-prometheus-stack.yaml", "blackbox.yaml"}
...
def test_kube_prometheus_stack_ships_the_upstream_rules_and_honours_every_rule_and_probe():
    hr = one("platform/monitoring/kube-prometheus-stack.yaml", "HelmRelease", "kube-prometheus-stack")
    ...
    for must_stay in ("general", "kubernetesApps", "kubernetesSystem", "kubernetesStorage", "kubeStateMetrics", "alertmanager", "prometheus"):
...
def test_alertmanager_config_comes_from_the_vault_and_routes_telegram_then_robusta():
    es = one("platform/monitoring/alertmanager-config.yaml", "ExternalSecret", "alertmanager-telegram")
    ...
    cfg = yaml.safe_load(raw.replace("{{ .token }}", "TOKEN").replace("{{ .channel }}", "-1001"))
    assert cfg["route"]["receiver"] == "telegram"
    names = {r["name"] for r in cfg["receivers"]}
    assert names == {"null", "telegram", "robusta"}
    ...
    assert rb["webhook_configs"][0]["url"] == "http://robusta-runner.robusta.svc.cluster.local/api/alerts"
...
def test_probe_targets_are_exactly_the_founder_surfaces_and_the_module_accepts_401_and_405():
    probe = one("platform/monitoring/rules/founder-surfaces-probe.yaml", "Probe", "founder-surfaces")
    ...
    assert all("${ESTATE_ZONE}" in t or t.startswith(("https://cloud.oracle.com/", "https://t.me/")) for t in targets), targets
    assert probe["spec"]["module"] == "founder" and probe["spec"]["jobName"] == "founder-surfaces"
    ...
    assert probe["spec"]["prober"]["url"].startswith(hr["spec"]["values"]["fullnameOverride"] + ".monitoring.svc")
...
def test_estate_rules_carry_founder_surface_down_and_the_cp14_pvc_alert():
    ...
    assert "kubelet_volume_stats_available_bytes / kubelet_volume_stats_capacity_bytes < 0.10" in pvc["expr"]
    assert pvc["labels"]["severity"] == "warning"  # the severity Alertmanager forwards to Robusta
...
def test_collector_reads_rules_and_the_alertmanager_proxy_and_the_grader_fails_on_zero():
    ...
    role = one("platform/state/cluster-state.yaml", "ClusterRole", "cluster-state-reader")
    grants = {(g, r, v) for rule in role["rules"] for g in rule["apiGroups"] for r in rule["resources"] for v in rule["verbs"]}
    ...
    assert 'int(kv["monitoring_rules"]) == 0 or int(kv["alert_watchdog"]) == 0' in grader
...
def test_grader_passes_with_rules_and_watchdog_and_fails_without():
    ...
    rc, out = grade(BASE + " monitoring_rules=0 alert_watchdog=0", '{"monitoring": {"error": "alertmanager: HTTP 503"}}')
