...
def _blind():
    if not shutil.which("kyverno"):
        pytest.skip(
            "BLIND: kyverno CLI not installed; the estate policy set cannot be evaluated"
        )
        ...
        if d and d.get("kind") in ("Deployment", "StatefulSet"):
            ...
            yield {
                ...
                "metadata": {
                    ...
                    "namespace": d["metadata"].get("namespace", "backstage"),
...
def _apply(tmp_path, pods):
    ...
    resource = (
        tmp_path / "pods.yaml"
    )  # a leading dot hides the file from the CLI: 0 rules applied
