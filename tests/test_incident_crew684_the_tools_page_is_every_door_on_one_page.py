...
def test_the_page_names_no_tool_host_or_group_list_of_its_own():
    ...
    assert "founder-surface" not in src or "FOUNDER_SURFACE_TYPE" in _code(APP / "home" / "useDoors.ts")
    assert not re.search(r"https?://", src), "a URL in the page: the catalogue is the list (LAW 46)"
...
def test_tools_is_a_founder_surface_the_gate_and_the_drill_carry():
    ...
    assert re.search(r'\(\s*"tools"\s*,\s*"text=', DRILL.read_text()), "the login drill grades /tools on its own content"
...
def test_no_two_source_files_differ_only_in_case():
    ...
    for f in src.rglob("*"):
        ...
        if key in seen and seen[key] != f.name:
            raise AssertionError(f"{seen[key]} and {f.name} differ only in case (case-insensitive disks merge them)")
        seen[key] = f.name
