...
def test_no_flux_row_carries_a_custom_resource_its_ordering_has_not_installed_yet():
    ...
    for name, (path, _) in sorted(rows.items()):
        ...
        assert bad == [], (
            ...
            f"CRD exists; its closure is {sorted(_closure(name, rows))}: "
            + ", ".join(bad)
        )
    assert seen > 25, (
        f"only {seen} rows rendered ({skipped} skipped) -- the sweep is not covering the tree"
    )
...
def test_every_group_the_tree_uses_is_either_built_in_or_owned_by_a_named_row():
    ...
    assert unowned == {}, (
        f"groups used by no known provider row: { {k: sorted(v) for k, v in unowned.items()} }"
    )
...
def test_a_group_no_row_claims_is_an_offender_and_never_waved_through():
    ...
    docs = [
        {
            "apiVersion": "brand.new.io/v1alpha1",
            "kind": "Widget",
            "metadata": {"name": "w"},
        }
    ]
    assert offenders(docs, set(), row="anywhere") == [
        "Widget/w (group 'brand.new.io' is provided by no row)"
    ]
...
def test_no_flux_row_names_a_priority_class_its_ordering_has_not_created_yet():
    ...
    assert len(providers) >= 2, (
        f"the tree renders {providers} -- the sweep found no PriorityClass to check against"
    )
    offences, seen, asked = _sweep(
        rows,
        lambda name, docs: priority_class_offenders(
            docs, providers, _closure(name, rows), row=name
        ),
...
def test_a_priority_class_reference_nested_in_a_helm_release_is_read_too():
    ...
    assert priority_class_offenders(
        docs,
        {"infrastructure-critical": "priority-classes"},
        set(),
        row="observability",
    ) == [
...
def test_no_flux_row_reaches_into_a_namespace_its_ordering_has_not_created_yet():
    ...
    assert len(providers) >= 15, (
        f"only {len(providers)} namespace(s) traced to a row -- the sweep is not reading the tree"
    )
    offences, seen, asked = _sweep(
        rows,
        lambda name, docs: namespace_offenders(
            docs, providers, _closure(name, rows), row=name
        ),
...
def test_a_helm_release_installing_somewhere_else_is_read_at_its_target_namespace():
    ...
    assert namespace_offenders(
        docs, {"monitoring": "monitoring"}, set(), row="alerts"
    ) == [
    ...
    docs = [
        {
            "apiVersion": "v1",
            "kind": "ConfigMap",
            "metadata": {"name": "c", "namespace": "invented"},
        }
    ]
    assert namespace_offenders(
        docs, {"backstage": "backstage"}, {"backstage"}, row="x"
