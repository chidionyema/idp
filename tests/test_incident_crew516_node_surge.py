...
then deletes the old one with the size decremented. Proved with `oci`/`tofu` shims that record
...
def _tree(tmp_path, nodes):
    ...
    env = {**os.environ, "PATH": f"{shim}:{os.environ['PATH']}", "SHIM_LOG": str(tmp_path / "log"),
           "SHIM_NODES": str(tmp_path / "nodes.json"), "SURGE_SETTLE_SECONDS": "0", "SURGE_POLL_SECONDS": "0"}
