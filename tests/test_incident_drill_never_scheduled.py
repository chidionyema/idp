...
def _run(tmp: Path, age_min: int | None, rc: int = 0) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        ["bash", str(GUARD), "login-drill.yml", "20"],
        env=env,
        capture_output=True,
        text=True,
    )
...
def test_incident_stale_or_absent_drill_is_fail_fresh_is_ok(tmp_path: Path) -> None:
    ...
    assert ok.returncode == 0 and ok.stdout.startswith(
        "ok      heartbeat  login-drill.yml  last success 4 min ago (run 999)"
    ), ok.stdout + ok.stderr
    stale = _run(tmp_path, 41)
    assert (
        stale.returncode == 1
        and "FAIL    heartbeat  login-drill.yml  last success 41 min ago > 20 (run 999)"
        in stale.stdout
    ), stale.stdout
    assert "gh workflow run login-drill.yml -R o/r" in stale.stdout
    none = _run(tmp_path, None)
    assert none.returncode == 1 and "no successful run on record" in none.stdout, (
        none.stdout
    )
