...
def test_incident_crew401_render_pushes_the_catalogue_after_generating_it():
    ...
    assert gen < push < unchanged, (
        "the push must follow catalog-gen and run even when the page is unchanged"
    )
    assert re.search(r'"BLIND ".*catalogue push', src), (
        "a machine that cannot push must say BLIND"
    )
