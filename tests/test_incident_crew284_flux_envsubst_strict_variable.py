    ...
    for f in glob.glob(str(ROOT / "clusters" / "*" / "*.yaml")):
        for d in yaml.safe_load_all(pathlib.Path(f).read_text()):
            if d and d.get("kind") == "ConfigMap":
                keys.setdefault(d["metadata"]["name"], set()).update(
                    (d.get("data") or {}).keys()
                )
    boot = (ROOT / "bin" / "idp-flux-bootstrap").read_text()
    for m in re.finditer(
        r"create configmap (\S+)((?:\s+--from-literal=[A-Z_]+=\S+)+)", boot
    ):
        keys.setdefault(m.group(1), set()).update(
            re.findall(r"--from-literal=([A-Z_]+)=", m.group(2))
        )
    ...
    for f in glob.glob(str(ROOT / "platform" / "**" / "*.yaml"), recursive=True):
        ...
        for d in docs:
            if d and d.get("kind") == "ExternalSecret":
                ...
                names = (tgt.get("template", {}).get("data") or {}).keys() or [
                    x["secretKey"] for x in d["spec"].get("data", [])
                ]
                keys.setdefault(tgt.get("name") or d["metadata"]["name"], set()).update(
                    names
                )
    return keys
...
def test_every_flux_variable_has_a_source():
    ...
    for ks in kustomizations():
        ...
        for f in sorted(
            glob.glob(str(ROOT / ks["spec"]["path"] / "**" / "*.yaml"), recursive=True)
        ):
...
def test_guard_refuses_the_incident_and_permits_shell_forms():
    ...
    assert undefined_variables(
        'export X="pg://u:${LITELLM_DB_PASSWORD}@h"', allowed
    ) == {"LITELLM_DB_PASSWORD"}
    assert (
        undefined_variables(
            'host: a.${ESTATE_ZONE}\nexport "$(basename "$f")=$(cat "$f")"', allowed
        )
        == set()
    )
    # crew#483: a lowercase name with a shell modifier is substituted too; `$$` is the escape
    assert undefined_variables(
        "code=${out%% *}; loc=${out#* } ${lochost:-x}", allowed
    ) == {"out", "lochost"}
    assert (
        undefined_variables("code=$${out%% *}; loc=$${out#* } ${ESTATE_ZONE}", allowed)
        == set()
    )
