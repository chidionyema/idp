...
def _run(tmp_path, psql_output):
    ...
    env = dict(
        ...
        FAKE_PSQL=psql_output,
        LANGFUSE_ENV_FILE=str(envf),
    )
    return subprocess.run(
        [str(VERIFY), "--migrations-only"],
        env=env,
        capture_output=True,
        text=True,
        cwd=ROOT,
...
def test_incident_crew340_failed_migration_is_named_and_fails(tmp_path):
    r = _run(
        tmp_path,
        "20260701_v4_step_3_backfill_events_full_from_observations ClickHouse events_full table does not exist\n",
    )
