...
def _task_container():
    docs = list(
        yaml.safe_load_all(
            open(ROOT / "platform" / "chaos" / "backstage-pod-kill.yaml")
        )
    )
    sched = next(d for d in docs if d and d.get("kind") == "Schedule")
    tpl = next(
        t
        for t in sched["spec"]["workflow"]["templates"]
        if t.get("templateType") == "Task"
    )
...
def _judge(pod, tmp_path):
    ...
    r = subprocess.run(
        ["kubectl", "kustomize", str(POLICIES)], capture_output=True, text=True
    )
    ...
    out = subprocess.run(
        ["kyverno", "apply", str(pol), "--resource", str(res)],
        capture_output=True,
        text=True,
    )
...
def test_workflow_task_pod_is_admitted(tmp_path):
    ...
    assert (
        "fail: 0" in out.replace(",", "").lower()
        or "0 failed" in out.lower()
        or "fail: 0" in out
    ), out
