...
def test_every_enabled_plugin_has_a_pluginconfig_entry():
    profiles = _profiles()
    assert "RemoveDuplicates" in profiles[0]["plugins"]["balance"]["enabled"], (
        "the incident plugin is still enabled"
    )
...
def test_the_incident_shape_is_named():
    broken = [
        {
            "name": "rebalance-after-node-swap",
            "pluginConfig": [
                {"name": "DefaultEvictor"},
                {"name": "LowNodeUtilization"},
            ],
            "plugins": {
                "balance": {"enabled": ["RemoveDuplicates", "LowNodeUtilization"]}
            },
        }
    ]
    assert missing_config(broken) == [
        "rebalance-after-node-swap/balance/RemoveDuplicates"
    ]
