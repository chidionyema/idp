...
def test_every_tenet_names_a_rule_that_exists():
    policy = (
        "".join(f.read_text() for f in (IDP / "policy").glob("*.rego"))
        + (IDP / "bin" / "pr-report").read_text()
    )
    for r in yaml.safe_load((IDP / "conscience" / "tenets.yaml").read_text())["tenets"]:
        name = (
            "no_secret_added"
            if r["pr_rule"] == "no_static_secret_in_diff"
            else r["pr_rule"]
        )
        assert f"rule={name}" in policy, (
            f"tenet {r['name']} names rule {name} which no policy defines"
        )
