...
def test_router_reports_every_call_to_langfuse():
    ...
    assert "langfuse" in s.get("success_callback", []) and "langfuse" in s.get(
        "failure_callback", []
    )
    ...
    init = _one(
        "platform/observability/langfuse.yaml", "ExternalSecret", "langfuse-init"
    )
    init_remote = {d["remoteRef"]["key"] for d in init["spec"]["data"]}
    assert remote <= init_remote, (
        "the router must read the keys Langfuse was initialised with, not a second pair"
    )
...
def test_observability_route_can_reach_the_front_door():
    ...
    grant = _one(
        "platform/identity/httproute.yaml", "ReferenceGrant", "front-door-oauth2-path"
    )
    assert {f["namespace"] for f in grant["spec"]["from"]} >= {
        "backstage",
        "observability",
    }
