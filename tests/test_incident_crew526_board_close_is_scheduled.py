...
def test_board_close_is_a_nightly_dead_man_wrapped_turn_in_the_crew_checkout():
    ...
    assert (
        job["command"][0].endswith("hc-wrap.sh") and job["command"][1] == "board-close"
    )
    assert job["command"][-2:] == ["~/.claude/scripts/estate_board.py", "close"]
    assert "--dry-run" not in job["command"]
    assert (
        job["cron"].split()[2:] == ["*", "*", "*"] and job["cron"].split()[1] != "*"
    )  # once a day
    assert (
        job["ok_exit"] == [3]
        and job["env"]["HC_FINDINGS_EXIT"] == "3"
        and 2 not in job["ok_exit"]
    )
    assert (
        job["cwd"] == "$CODE/crew"
        and job["env"]["ESTATE_CLOSER_LOG"] == "science/closer.jsonl"
    )
...
def test_the_picker_on_this_machine_has_the_close_verb():
    ...
    out = subprocess.run(
        [sys.executable, str(picker), "--help"],
        capture_output=True,
        text=True,
        check=False,
    )
