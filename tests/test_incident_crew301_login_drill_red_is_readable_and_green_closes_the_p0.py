...
def test_the_fail_line_keeps_the_call_log():
    ...
    err = (
        "APIRequestContext.post: self-signed certificate; "
        + "x" * 160
        + " Call log:\n  - → POST https://catalogue.example/api/auth/oauth2Proxy/refresh\n  - ← 302"
    )
    out = ns["fail"]("identity", err)
    assert (
        "POST https://catalogue.example/api/auth/oauth2Proxy/refresh" in out
        and "\n" not in out
    )
...
def test_a_green_run_closes_the_open_p0_and_a_red_one_still_opens_it():
    ...
    steps = (
        wf["jobs"]["login-drill"]["steps"]
        if "login-drill" in wf["jobs"]
        else next(iter(wf["jobs"].values()))["steps"]
    )
    ...
    assert (
        green["if"] == "success()"
        and "gh issue close" in green["run"]
        and "--reason completed" in green["run"]
    )
    assert 'in:title "P0: login drill failed"' in green["run"], (
        "it must find the same issue the red step opens"
    )
