...
def test_no_secret_sits_in_a_value_and_the_chart_reads_the_one_the_vault_writes():
    ...
    for word in ("password", "api_key", "apikey", "token"):
        assert f'"{word}"' not in flat and not re.search(
            rf'{word}[^"]*":\s*"[^"$]', flat
        ), (word, flat)
    assert values["existingSecret"] == "hindsight-env"
    assert values["postgresql"]["enabled"] is False, (
        "the chart's bundled ankane/pgvector:latest runs as root"
    )
...
def test_the_kyverno_waiver_covers_the_api_pods_only():
    docs = [
        ...
        for d in yaml.safe_load_all(
            (ROOT / "platform/edge/hindsight-exception.yaml").read_text()
        )
        if d
    ]
    (pe,) = [d for d in docs if d["kind"] == "PolicyException"]
    assert pe["metadata"]["namespace"] == "kyverno"
    assert [e["policyName"] for e in pe["spec"]["exceptions"]] == [
...
def test_the_flux_row_waits_on_the_vault_and_the_router_and_the_gateway_points_at_it():
    ...
    gw = [
        d
        for d in yaml.safe_load_all(
            (ROOT / "platform/hermes-agent/gateway.yaml").read_text()
        )
    ...
    assert (
        "SEED_HINDSIGHT_DB_PASSWORD" not in seed
        and "hindsight|" in seed
        and "never seeded by hand" in seed
    )
    ...
    assert "hindsight" in opts, (
        "the dispatch choice list lost `hindsight` in the #430 merge (2026-08-27)"
    )
...
def test_incident_crew524_an_oci_repository_is_addressed_as_a_chart_ref_and_the_preamble_is_dropped(
    tmp_path,
):
    args, render = _run_renderer(tmp_path, "oci://ghcr.io/vectorize-io/charts")
    assert (
        "oci://ghcr.io/vectorize-io/charts/hindsight" in args and "--repo" not in args
    ), args
    assert render.startswith("---\n") and [
        d["kind"] for d in yaml.safe_load_all(render) if d
    ] == ["ConfigMap"]
...
def test_incident_crew524_an_http_repository_still_goes_through_repo(tmp_path):
    ...
    assert (
        args[args.index("--repo") + 1] == "https://charts.example.org"
        and "hindsight" in args
    )
