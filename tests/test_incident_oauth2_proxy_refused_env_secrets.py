...
def _render(tmp_path, tag, override):
    docs = [
        d for d in yaml.safe_load_all((IDENTITY / "oauth2-proxy.yaml").read_text()) if d
    ]
...
def test_shipped_values_are_admitted_and_env_var_wiring_is_refused(tmp_path):
    ...
    assert rendered.count("--config=") == 1 and "kind: ConfigMap" not in rendered, (
        "the chart still writes a config of its own"
    )
    ...
    bad = _render(
        ...
        "envvars",
        {
            ...
            "config": {"existingSecret": "oauth2-proxy", "configFile": ""},
        },
    )
...
def test_the_secret_reaches_the_pod_as_one_mounted_file():
    ...
    hr = next(
        d
        for d in yaml.safe_load_all((IDENTITY / "oauth2-proxy.yaml").read_text())
    ...
    es = next(
        d
        for d in yaml.safe_load_all((IDENTITY / "external-secret.yaml").read_text())
        if d
    )
    assert (
        v["extraArgs"]["config"].rsplit("/", 1)[1]
        in es["spec"]["target"]["template"]["data"]
    )
    assert (
        next(x for x in v["extraVolumes"] if x["name"] == "config")["secret"][
            "secretName"
        ]
        == es["spec"]["target"]["name"]
    )
