    ...
    for f in sorted(root.rglob("*.yaml")):
        for d in _docs(f):
            if d.get("kind") == "Namespace":
                out[d["metadata"]["name"]] = (d["metadata"].get("labels") or {}).get(
                    ENFORCE, "privileged"
                )
    return out
    ...
    for f in sorted(root.rglob("*.yaml")):
        for d in _docs(f):
            ...
            if not waived & HOSTPATH_POLICIES:
                continue
            for block in d["spec"]["match"].get("any", []) + d["spec"]["match"].get(
                "all", []
            ):
...
def violations(root=ROOT / "platform"):
    ...
    return [
        f"{f}: PolicyException {name} waives hostPath into namespace {ns}, whose PSA enforce level is "
        ...
        for f, name, ns in hostpath_waivers(root)
        if levels.get(ns, "privileged") != "privileged"
    ]


def test_every_hostpath_waiver_lands_in_a_privileged_namespace():
    assert list(hostpath_waivers()), (
        "no hostPath waiver found: the k8s-infra exception moved or lost its policies"
    )
    assert violations() == []
...
def test_a_waiver_into_a_restricted_namespace_is_refused(tmp_path):
    ...
    exc = yaml.safe_load(
        (ROOT / "platform" / "edge" / "k8s-infra-exception.yaml").read_text()
    )
