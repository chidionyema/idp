...
def _flux_message():
    ...
    fn = next(
        ...
        if isinstance(n, ast.FunctionDef) and n.name == "flux_message"
    )
    ns: dict = {}
...
def test_incident_crew483_a_kyverno_denial_keeps_its_policy_name():
    fm = _flux_message()
    head = (
        "Helm install failed for release observability-agent/observability-agent-k8s-infra with chart "
    ...
    assert policy in fm(head + ("x" * 1800) + policy), (
        "past 2000 the tail with the policy survives"
    )
