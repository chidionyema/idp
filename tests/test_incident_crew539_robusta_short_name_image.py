...
def test_image_registry_is_fully_qualified_not_a_short_name() -> None:
    v = _robusta_values()
    assert "image" in v, (
        "no image.registry override: the chart's short unqualified default (robustadev) ships"
    )
    registry = v["image"]["registry"]
    assert "/" in registry, (
        f"image.registry {registry!r} has no host — this is the short name the runtime refused"
    )
    assert (
        registry.split("/")[0].count(".") >= 1 or registry.split("/")[0] == "localhost"
...
def test_registry_override_is_not_the_bare_chart_default() -> None:
    v = _robusta_values()
    assert v["image"]["registry"] != "robustadev", (
        "still the chart default: robustadev/<image> is an unqualified short name"
    )
