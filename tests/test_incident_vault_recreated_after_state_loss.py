...
def _run(tmp: Path, creates: bool, live: bool) -> subprocess.CompletedProcess:
    b, m = _tree(tmp)
    show = (
        '  # oci_kms_vault.estate will be created\n      + display_name = "estate-secrets"\nPlan: 1 to add.'
        if creates
        else "No changes."
    )
    _fake(
    ...
    rows = ["decoy-vault ocid1.vault.oc1..decoy"] + (
        [f"estate-secrets {LIVE}"] if live else []
    )
    _fake(
        b,
        "idp-cloud",
...
def test_incident_vault_recreated_after_state_loss(tmp_path: Path) -> None:
    ...
    assert (
        r.returncode == 1
        and "REFUSE" in r.stdout
        and f"tofu import oci_kms_vault.estate {LIVE}" in r.stdout
    )
    assert "decoy" not in r.stdout, (
        "the guard took a row that is not the display_name it asked for"
    )
    ...
    assert (
        r.returncode == 0
        and "creates nothing" in r.stdout
        and not (tmp_path / "cloud.calls").exists()
    )
...
def test_incident_bucket_recreated_after_state_loss(tmp_path: Path) -> None:
    ...
    assert (
        "tofu import oci_objectstorage_bucket.drill_receipts n/<namespace>/b/estate-drill-receipts"
        in r.stdout
    )
