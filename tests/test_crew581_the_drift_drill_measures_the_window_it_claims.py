...
def _row():
    ...
    assert len(matching) == 1, (
        f"expected one catalogue row for {WORKFLOW.name}, got {len(matching)}"
    )
    return matching[0]
...
def test_a_target_must_be_a_configmap_flux_applied_that_holds_data():
    applied = {
        "metadata": {
            "name": "cm",
            "namespace": "ns",
            "labels": {drill.FLUX_NAME_LABEL: "k"},
        },
        "data": {"b": "2", "a": "1"},
    }
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {
            "metadata": {"name": "cm", "namespace": "ns", "labels": {}},
            "data": {"a": "1"},
        },
...
def test_the_row_is_not_marked_pending_because_the_job_ships_with_it():
    assert "pending" not in _row(), (
        "the workflow is in this same commit; a pending row would never FAIL"
    )
...
def _deployment(labelled=True, containers=("api",)):
    return {
        ...
        "spec": {
            "template": {"spec": {"containers": [{"name": c} for c in containers]}}
        },
    }
