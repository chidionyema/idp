...
MODULE = (
    ROOT / "backstage" / "plugins" / "catalog-backend-module-dagster-entity-provider"
)
...
def test_the_module_and_its_pure_mapping_and_jest_test_exist():
    for rel in (
        "package.json",
        "src/index.ts",
        "src/DagsterEntityProvider.ts",
        "src/mapping.ts",
        "src/mapping.test.ts",
        "src/dagsterClient.ts",
        "src/types.ts",
    ):
...
def test_the_fixture_is_a_recorded_dagster_graphql_response_with_lineage_and_owners():
    ...
    assert len(asset_nodes) >= 2, (
        "fixture needs at least one dependency edge to prove lineage"
    )
    dependents = [a for a in asset_nodes if a["dependencyKeys"]]
    assert dependents, (
        "fixture must carry at least one asset with dependencyKeys (lineage)"
    )
    owners = {tuple(a["owners"]) for a in asset_nodes}
    assert any(o and o[0].startswith("team:") for o in owners), (
        "no team owner in fixture"
    )
    assert any(o and "@" in o[0] for o in owners), "no user-email owner in fixture"
    assert any(len(o) == 0 for o in owners), (
        "no owner-less asset in fixture (fallback path)"
    )
...
def test_the_backend_registers_the_module():
    ...
    assert (
        pkg["dependencies"]["catalog-backend-module-dagster-entity-provider"]
        == "workspace:*"
    )
