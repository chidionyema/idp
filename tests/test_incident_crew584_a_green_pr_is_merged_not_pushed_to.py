...
def _run(repo: Path, env: dict) -> subprocess.CompletedProcess:
    return subprocess.run(
        [str(SCRIPT)], cwd=repo, env=env, capture_output=True, text=True
    )


def _pr(
    head: str, conclusions: list[str], state: str = "OPEN", merge: str = "CLEAN"
) -> dict:
    ...
    return {
        ...
        "statusCheckRollup": [
            {"name": f"c{i}", "conclusion": c} for i, c in enumerate(conclusions)
        ],
    }
...
def test_no_pr_or_nothing_new_or_blind_is_never_refused(tmp_path):
    ...
    head2 = subprocess.run(
        ["git", "rev-parse", "HEAD"], cwd=repo2, capture_output=True, text=True
    ).stdout.strip()
    (tmp_path / "same" / "bin" / "gh").write_text(
...
def test_the_pre_push_hook_runs_the_rung_first():
    rungs = [
        l
        for l in (ROOT / ".githooks" / "pre-push").read_text().splitlines()
        if l.startswith('"$IDP/bin/')
    ]
