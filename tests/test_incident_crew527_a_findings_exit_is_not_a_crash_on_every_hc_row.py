...
def test_the_graphify_sweep_findings_exit_is_declared_on_its_row():
    ...
    assert job["ok_exit"] == [1], (
        "exit 1 is VERDICT-with-findings, measured by hand 2026-08-28"
    )
...
def test_every_hc_wrapped_row_that_accepts_a_findings_exit_tells_hc_wrap_the_same_codes():
    ...
    for name, job in JOBS.items():
        ...
        if declared is None:
            wrong.append(
                f"{name}: ok_exit={ok} but no HC_FINDINGS_EXIT (its liveness check fails on every finding)"
            )
        elif sorted(str(c) for c in ok) != sorted(
            str(declared).replace(",", " ").split()
        ):
