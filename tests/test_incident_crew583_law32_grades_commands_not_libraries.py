...
def _gate(*added: str) -> subprocess.CompletedProcess:
    return subprocess.run(
        [sys.executable, str(GATE), "--added", *added], capture_output=True, text=True
    )
...
def test_the_same_file_with_the_executable_bit_is_a_command_again(tmp_path):
    ...
    r = subprocess.run(
        [
            sys.executable,
            str(tmp_path / "bin/law32-gate"),
            "--added",
            "bin/lib/receipt_age.py",
        ],
        capture_output=True,
        text=True,
    )
    ...
    assert r.returncode == 1 and "receipt_age.py: no docs/tutorials/demo" in r.stdout, (
        r.stdout + r.stderr
    )
...
def test_a_name_with_no_file_behind_it_is_still_graded():
    ...
    assert (
        r.returncode == 1
        and "feature-with-no-pages: no docs/tutorials/demo" in r.stdout
    ), r.stdout + r.stderr


def test_a_real_command_with_its_pages_still_passes():
    assert (ROOT / "bin/supply-chain").exists() and os.access(
        ROOT / "bin/supply-chain", os.X_OK
    )
