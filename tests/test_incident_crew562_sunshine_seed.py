...
def _run(tmp_path, *argv):
    return subprocess.run(
        [str(BIN), *argv], env=_env(tmp_path), capture_output=True, text=True
    )
...
def test_mint_writes_username_password_and_the_derived_basic_header_and_prints_no_value(
    tmp_path,
):
    r = _run(tmp_path)
    ...
    assert (
        e["authorization"]
        == "Basic " + base64.b64encode(f"estate:{e['password']}".encode()).decode()
    )
    assert (
        e["password"] not in r.stdout + r.stderr
        and e["authorization"] not in r.stdout + r.stderr
    )
...
def test_adopt_refuses_without_the_mounted_pair(tmp_path):
    r = subprocess.run(
        [str(BIN), "--adopt"],
        env={**_env(tmp_path), "SUNSHINE_CREDS_DIR": str(tmp_path / "none")},
        capture_output=True,
        text=True,
    )
...
def test_oke_check_apply_runs_the_bootstrapper():
    ...
    steps = [
        ...
        if "idp-bootstrap-sunshine" in str(s.get("run", ""))
    ]
