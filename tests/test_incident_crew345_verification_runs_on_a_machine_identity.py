...
def test_the_drill_script_refuses_a_person_and_a_browser_login_as_the_subject() -> None:
    ...
    assert "oci iam user get --user-id" in script, (
        "the subject is named through the API, not assumed"
    )
    assert '[ "$ttype" != te ]' in script, (
        "a browser login (ttype=login) must be a red identity row"
    )
...
def test_no_scheduled_workflow_asks_a_person_to_log_in() -> None:
    ...
    for f in sorted(WF.glob("*.yml")):
        ...
        assert TOKEN_EXCHANGE in text, (
            f"{f.name} is scheduled, talks to OCI, and exchanges no OIDC token"
        )
        # graded on what runs, not on comments: oke-check.yml names the command it replaces (idp#267)
        code = "\n".join(
            line for line in text.splitlines() if not line.lstrip().startswith("#")
        )
        assert "oci session authenticate" not in code, (
            f"{f.name} asks for a browser login"
        )
