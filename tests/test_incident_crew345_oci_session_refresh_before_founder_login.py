...
def test_t1_script_calls_oci_session_refresh_before_giving_up():
    ...
    assert "oci session validate" in text, (
        "must still check for an already-valid session first"
    )
...
def test_t2_refresh_is_attempted_per_profile_not_just_first():
    ...
    assert (
        'for d in "$SESSIONS_DIR"' in preceding or 'for d in "$SESSIONS_DIR"' in text
    ), (
