...
def test_incident_crew325_cert_manager_components_carry_limits() -> None:
    hr = next(
        d
        for d in _docs("platform/edge/cert-manager.yaml")
...
def test_incident_crew325_every_policy_exception_lives_where_kyverno_reads() -> None:
    ...
    kyverno = next(
        d for d in _docs("platform/kyverno/kyverno.yaml") if d["kind"] == "HelmRelease"
    )
    ...
    paths = sorted(
        {
            ...
            and d["spec"].get("path", "").startswith("./platform/")
        }
    )
    assert paths
    found = 0
    for k in paths:
        ...
        for d in yaml.safe_load_all(out):
            if d and d.get("kind") == "PolicyException":
                found += 1
                assert d["metadata"].get("namespace") == "kyverno", (
                    k,
                    d["metadata"]["name"],
                )
    assert found >= 3
...
def test_incident_crew325_emptydir_over_an_image_conf_dir_is_seeded_first() -> None:
    ...
    hr = next(
        d
        for d in _docs("platform/observability/signoz.yaml")
        if d["kind"] == "HelmRelease"
    )
    for renderer in hr["spec"]["postRenderers"]:
        for patch in renderer["kustomize"]["patches"]:
            ...
            if not isinstance(
                body, dict
            ):  # JSON-patch lists (ClickHouseInstallation) carry no pod spec
                continue
