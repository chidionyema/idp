...
def test_incident_crew340_profiler_and_metric_logs_are_removed():
    ...
    for table in (
        ...
        assert table in removed, (
            f"{table} is not removed in {XML.name}; it burnt the 2 vCPU host (crew#340)"
        )
    assert "query_log" not in removed, (
        "query_log is the instrument this incident was read from; keep it"
    )
