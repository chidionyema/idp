...
RECEIPT = {
    "measured_at": "2026-08-28T07:23:00+00:00",
    "host": "runner",
    "score": {"green": 6, "total": 7},
    "blind": False,
...
def _run(tmp_path, env):
    rep = tmp_path / "r.json"
    rep.write_text(json.dumps(RECEIPT))
    return subprocess.run(
        [sys.executable, str(EMIT), str(rep)],
        env={**os.environ, **env},
        capture_output=True,
        text=True,
    )
...
def test_the_receipt_lands_as_one_record_with_the_source_attribute_and_the_auth_pair(
    tmp_path,
):
    srv = _serve()
    _Collector.seen.clear()
    _Collector.status = 200
    p = _run(
        tmp_path,
        {
            "OTEL_EXPORTER_OTLP_ENDPOINT": f"http://127.0.0.1:{srv.server_port}",
            "OTLP_INGEST_USER": "science",
            "OTLP_INGEST_PASSWORD": "pw",
        },
    )
    assert (
        p.returncode == 0
        and "ok   conscience emit" in p.stdout
        and "HTTP 200" in p.stdout
    ), p.stdout + p.stderr
    path, auth, body = _Collector.seen[0]
    assert (
        path == "/v1/logs"
        and auth == "Basic " + base64.b64encode(b"science:pw").decode()
    )
    rec = body["resourceLogs"][0]["scopeLogs"][0]["logRecords"][0]
    attrs = {a["key"]: list(a["value"].values())[0] for a in rec["attributes"]}
    assert attrs == {
        "science.source": "conscience",
        "conscience.green": "6",
        "conscience.total": "7",
        "conscience.red": "secure",
    }
...
def test_a_refused_post_is_fail_not_ok(tmp_path):
    ...
    assert (
        p.returncode == 1
        and "FAIL conscience emit" in p.stdout
        and "HTTP 401" in p.stdout
    )
