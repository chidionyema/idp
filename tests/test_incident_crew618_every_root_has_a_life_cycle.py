...
def test_every_secret_a_workflow_reads_has_a_life_cycle_row_or_is_declared_configuration():
    ...
    config = set(
        re.findall(
            r"`(SEED_[A-Z0-9_]+)`",
            POLICY[POLICY.index("## Configuration that is not a credential") :],
        )
    )
    missing = sorted(
        n for n in _names_in_workflows() if n not in rows and n not in config
    )
    assert not missing, (
        f"no life-cycle row on credential-lifecycle.md for {missing} (crew#618)"
    )
