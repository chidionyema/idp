...
spec = importlib.util.spec_from_loader(
    "estate_diagram", importlib.machinery.SourceFileLoader("estate_diagram", str(BIN))
)
...
def synth(rng: random.Random) -> list:
    ...
    docs = [
        {
            ...
            "metadata": {
                "name": c,
                "annotations": {
                    "estate/path": f"checkout-{c}",
                    "estate/coupling": rng.choice(["none", "anthropic", "openai"]),
                },
            },
            "spec": {"type": "service"},
        }
    ...
    for t in ("scheduled-job", "guard", "ledger", "port", "drill"):
        for i in range(rng.randint(0, 5)):
            ann = {
                "estate/interval-s": str(rng.choice([30, 60, 3600])),
                "estate/last-status": rng.choice(["0", "1"]),
                "estate/loaded": "yes",
            }
            if t == "port":
                ann = {
                    "estate/port": str(1000 + i),
                    "estate/owner": "python",
                    "estate/bind": "127.0.0.1",
                    "estate/command": f"checkout-{rng.choice(comps)}/run",
                }
            docs.append(
                {
                    "kind": "Resource",
                    "metadata": {"name": f"{t}-{i}", "annotations": ann},
                    "spec": {
                        "type": t,
                        "dependsOn": []
                        if t == "port"
                        else [f"component:default/{rng.choice(comps)}"],
                    },
                }
            )
    return docs
...
def prop_every_entity_present_and_counts_match(n=200) -> int:
    ...
    for seed in range(n):
        ...
        if page != mod.render("2026-01-01T00:00:00Z", docs):
            fails += 1
            print(f"FAIL seed={seed}: not deterministic")
            continue
        for d in docs:
            ...
            if d["kind"] == "Component" or t in ("port", "scheduled-job"):
                if d["metadata"]["name"] not in page:
                    fails += 1
                    print(f"FAIL seed={seed}: {d['metadata']['name']} missing")
                    break
        for c in (d for d in docs if d["kind"] == "Component"):
            ...
            want = {
                t: sum(
                    ...
                    if d["kind"] == "Resource"
                    and d["spec"]["type"] == t
                    and f"component:default/{name}" in d["spec"]["dependsOn"]
                )
            ...
            if label not in page:
                fails += 1
                print(f"FAIL seed={seed}: counts wrong for {name}")
                break
    return fails
...
def incident_r29_hand_drawn_page_is_refused() -> int:
    with tempfile.TemporaryDirectory() as td:
        ...
        cat.write_text(
            "# inventory taken: 2026-01-01T00:00:00Z\n"
            + yaml.safe_dump_all(synth(random.Random(7)))
        )
        out.write_text("# Live estate\nhand drawn\n")
        bad = subprocess.run(
            [sys.executable, str(BIN), "--catalog", cat, "--out", out, "--check"],
            capture_output=True,
            text=True,
        )
        subprocess.run(
            [sys.executable, str(BIN), "--catalog", cat, "--out", out],
            check=True,
            capture_output=True,
        )
        good = subprocess.run(
    ...
    print(
        f"{'ok  ' if ok else 'FAIL'}  incident r29: drifted rc={bad.returncode}, rendered rc={good.returncode}, no catalogue rc={missing.returncode}"
    )
...
if __name__ == "__main__":
    ...
    print(
        f"{'ok  ' if not f else 'FAIL'}  property: 200 synthetic catalogues, {f} failures"
    )
