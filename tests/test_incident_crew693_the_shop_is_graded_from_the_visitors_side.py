...
def _module():
    loader = importlib.machinery.SourceFileLoader("storefront_drill_crew693", str(DRILL))
...
def _run(module, monkeypatch, home: str, picture_status: int = 200) -> tuple[int, str]:
    def fetch(url: str):
        if url.endswith(".jpg"):
            return picture_status, "image/jpeg", b"\xff\xd8\xff" if picture_status == 200 else b""
...
def test_the_build_that_was_being_shipped_makes_every_row_green(drill, monkeypatch):
    ...
    assert code == 0, f"a drill that can only ever be red is an alarm, not a drill:\n{out}"
    assert re.search(r"^ok +pack-pictures", out, re.M), out


def test_a_picture_the_page_names_and_the_shop_does_not_serve_is_a_fail(drill, monkeypatch):
...
def test_the_shop_host_is_read_from_the_estate_config_never_typed():
    ...
    body = source.split('"""', 2)[2]  # the docstring quotes the incident and names the host once
    assert "mumchimp" not in body, "LAW 46: the zone is read from clusters/oke/estate-config.yaml"
    assert "ESTATE_ZONE" in body


def test_the_drill_grades_features_and_never_layout():
    source = DRILL.read_text().lower()
    for word in ("data-testid", "queryselector", "css", "pixel", "font", "colour", "stylesheet"):
...
def test_the_catalogue_entry_names_this_workflow_and_its_cron_verbatim():
    row = [d for d in yaml.safe_load(CATALOGUE.read_text())["drills"] if d["name"] == "storefront-smoke"]
    assert row, "the drill is not in drills/catalogue.yaml, so bin/idp-verify never grades its freshness"
    entry = row[0]
    workflow = ROOT / ".github" / "workflows" / entry["workflow"]
    cron = re.findall(r"- cron: \"([^\"]+)\"", workflow.read_text())
    assert entry["schedule"] in cron, f"{entry['schedule']} is not a cron line in {entry['workflow']}"
