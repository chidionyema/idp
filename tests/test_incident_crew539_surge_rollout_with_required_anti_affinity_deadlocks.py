...
OVERLAYS = sorted(
    p.parent for p in (ROOT / "platform").rglob("overlays/*/kustomization.yaml")
)
...
def _render(overlay: pathlib.Path):
    ...
    if r.returncode != 0:
        ...
        pytest.skip(
            f"{overlay.relative_to(ROOT)} does not render in a clean checkout: {r.stderr.strip()[-120:]}"
        )
    return [d for d in yaml.safe_load_all(r.stdout) if isinstance(d, dict)]


def _requires_one_per_host(spec: dict) -> bool:
    aff = ((spec.get("affinity") or {}).get("podAntiAffinity") or {}).get(
...
def surging_pinned_deployments(docs) -> list[str]:
    ...
    for d in docs:
        ...
        if str(surge) not in ("0", "0%"):
            bad.append(
                f"{(d.get('metadata') or {}).get('namespace')}/{(d.get('metadata') or {}).get('name')}: maxSurge={surge}"
            )
    return bad
...
@pytest.mark.parametrize(
    "overlay", OVERLAYS, ids=[str(o.relative_to(ROOT)) for o in OVERLAYS]
)
