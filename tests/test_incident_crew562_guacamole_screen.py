...
def test_no_password_exists_for_the_app():
    sql = _seed_sql()
    assert re.search(r"DELETE FROM guacamole_entity WHERE name = 'guacadmin'", sql), (
        "the default admin must go"
    )
    assert "'password'" not in sql and "'username'" not in sql, (
        "the VNC login is typed at connect time, never seeded"
    )
    env = next(d for d in _docs("env.yaml") if d["kind"] == "ConfigMap")["data"]
    assert (
        env["HEADER_ENABLED"] == "true"
        and env["HTTP_AUTH_HEADER"] == "X-Auth-Request-Email"
    )
    ...
    for c in (
        ...
        for e in c.get("env", []):
            assert "secretKeyRef" not in str(e.get("valueFrom", {})), (
                f"{c['name']}: secrets are mounted files, never env"
            )
...
def test_the_mac_is_reached_only_over_the_tailnet_egress():
    ...
    assert (
        ...
        and svc["spec"]["ports"][0]["port"] == 5900
    )
    sql = _seed_sql()
    assert "('hostname', 'founder-mac-vnc')" in sql and "('port', '5900')" in sql
    assert not re.search(
        r"\b100\.\d+\.\d+\.\d+\b", (DIR / "guacamole.yaml").read_text() + sql
    ), "no tailnet IP literal (LAW 46)"
...
def test_the_vault_entry_is_born_by_a_bootstrapper():
    ...
    assert re.search(
        r"^guacamole\s+postgres-password\s+hex32$",
        (ROOT / "bin" / "idp-estate-seed").read_text(),
        re.M,
    )
    register = (ROOT / "docs" / "reference" / "policy" / "root-trust.md").read_text()
    assert re.search(
        r"^\| `guacamole` \(`postgres-password`\) \|.*\| MEETS \| `bin/idp-estate-seed` \|$",
        register,
        re.M,
    )
...
def test_the_surface_is_behind_the_one_login_listed_and_probed():
    ...
    assert (
        surfaces
        and surfaces[0]["metadata"]["links"][0]["url"]
        == "https://catalogue.${ESTATE_ZONE}/screen/"
    )
    probe = (
        ROOT / "platform" / "monitoring" / "rules" / "founder-surfaces-probe.yaml"
    ).read_text()
...
def test_a_first_time_user_is_told_the_one_prerequisite_and_the_estate_measures_it():
    ...
    assert probe["spec"]["targets"]["staticConfig"]["static"] == [
        "founder-mac-vnc.guacamole.svc.cluster.local:5900"
    ]
    ...
    assert "Sharing > Screen Sharing on" in rules, (
        "the alert names the one switch in plain English"
    )
    assert (
        "founder-mac-screen-sharing-probe.yaml"
        in (
            ROOT / "platform" / "monitoring" / "rules" / "kustomization.yaml"
        ).read_text()
    )
