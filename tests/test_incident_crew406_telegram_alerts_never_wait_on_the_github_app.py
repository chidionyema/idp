    ...
    for f in files:
        for d in yaml.safe_load_all(f.read_text()):
            if (
                ...
                and d.get("apiVersion", "").startswith("kustomize.toolkit")
            ):
    ...
    for f in (root / path).glob("*.yaml"):
        for d in yaml.safe_load_all(f.read_text()):
            if d and d.get("kind") == "ExternalSecret":
                keys = [
                    x.get("extract", {}).get("key")
                    for x in d["spec"].get("dataFrom", [])
                ]
                keys += [
                    x.get("remoteRef", {}).get("key") for x in d["spec"].get("data", [])
                ]
...
def test_the_telegram_alerts_kustomization_never_waits_on_the_github_app() -> None:
    ...
    assert _renders_github_app(ROOT, ks["alerts-github"]["spec"]["path"]), (
        "the ledger's own path holds the App secret"
    )
...
def test_the_rule_refuses_the_chain_that_bit(tmp_path: Path) -> None:
    ...
    ks = {
        ...
        "alerts-secret": {
            "spec": {"path": "./s", "dependsOn": [{"name": "secret-store"}]}
        },
    }
