...
def test_incident_crew284_cluster_router_has_its_database():
    ...
    assert (
        'export DATABASE_URL="postgresql://litellm:$(cat /run/secrets/litellm/upstream/LITELLM_DB_PASSWORD)@litellm-db'
        in script
    )
    assert "${LITELLM_DB_PASSWORD}" not in script
    assert "DATABASE_URL" not in {e["name"] for e in c["env"]}, (
        "the URL holds the password; it is composed, not declared"
    )
    ...
    assert (
        ...
        == "/run/secrets/litellm/upstream/LITELLM_DB_PASSWORD"
    )
    vols = {v["name"]: v for v in sts["spec"]["template"]["spec"]["volumes"]}
    assert vols["upstream"]["secret"] == {"secretName": "litellm-upstream"}
    assert (
        "postgres.yaml"
        in yaml.safe_load((LLM / "kustomization.yaml").read_text())["resources"]
    )
