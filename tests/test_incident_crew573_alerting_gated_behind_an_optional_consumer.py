...
def test_the_graph_is_actually_read():
    ...
    forward, _ = _graph()
    assert CLUSTERS, (
        "no clusters/*/platform.yaml found; the guard is grading an empty set"
    )
    assert len(forward) >= 20, (
        f"only {len(forward)} Kustomizations parsed; the parser is broken"
    )
    missing = ALERTING - set(forward)
    assert not missing, (
        f"the alerting rows this guard exists for are absent: {sorted(missing)}"
    )
...
def test_a_pager_that_is_always_reachable_is_the_catch_all_receiver():
    ...
    doc = next(
        ...
        and "alertmanager.yaml"
        in yaml.safe_dump(d.get("spec", {}).get("target", {}).get("template", {}))
    )
