...
AUTOMATION = yaml.safe_load_all(
    (ROOT / "platform/image-automation/backstage.yaml").read_text()
)
...
def _build_tag_template() -> str:
    ...
    m = re.search(
        r'-t "\$image:(main-\$\{\{ github\.run_number \}\}-\$\{\{ github\.sha \}\})"',
        wf,
    )
...
def test_policy_orders_the_tag_the_build_writes_and_refuses_the_bare_sha():
    tag = (
        _build_tag_template()
        .replace("${{ github.run_number }}", "4071")
...
def test_kustomization_carries_the_policy_marker():
    ...
    m = re.search(
        r'newTag: (\S+) # \{"\$imagepolicy": "flux-system:backstage:tag"\}', text
    )
    assert m, (
        "newTag line has no $imagepolicy marker; image-automation-controller cannot find it"
    )
    ...
    assert "platform/backstage/overlays/oke".startswith(
        update["path"].removeprefix("./")
    ), update
...
def test_controllers_are_installed_and_the_writer_authenticates_as_the_app():
    ...
    deployments = {
        d["metadata"]["name"]
        for d in yaml.safe_load_all(gotk)
    ...
    assert (
        writer["spec"]["url"] == "https://github.com/chidionyema/idp"
        and writer["spec"]["provider"] == "github"
    )
    assert writer["spec"]["secretRef"] == {"name": "flux-writer"}
    es = yaml.safe_load(
        (ROOT / "platform/image-automation/flux-writer.yaml").read_text()
    )
    assert set(es["spec"]["target"]["template"]["data"]) == {
        "githubAppID",
    ...
    assert "put flux-writer" not in seed and "SEED_FLUX_WRITER" not in seed, (
        "the pasted deploy key path is gone"
    )
    assert not (ROOT / "platform/image-automation/github-app.yaml").exists(), (
        "one App entry, github-app, not a second"
    )
    assert (
        BY_KIND["ImageUpdateAutomation"]["spec"]["sourceRef"]["name"]
...
def test_the_automation_branch_is_graded_and_becomes_a_pull_request():
    ...
    assert branch in (ci.get("on") or ci.get(True))["push"]["branches"], (
        "required checks would never run on the automation push"
    )
    ...
    last = run.splitlines()[
        -1
    ].strip()  # crew#439: the step ends by running bin/idp-image-update-pr; the rule binds the script
    ...
    script = (
        ROOT / last.split("origin/main:", 1)[1].rstrip(")")
        if "origin/main:" in last
        else ROOT / last
    )
