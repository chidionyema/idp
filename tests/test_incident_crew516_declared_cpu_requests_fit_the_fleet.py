...
def test_incident_crew516_the_budget_is_read_from_the_files_that_set_it():
    ...
    assert len(rows) >= 50, (
        "the manifest walk found fewer containers than the 67 measured on 2026-08-27"
    )
    assert any("gotk-components" in f for _, f, _ in rows)  # clusters/ is in the walk
    assert any(
        "/spec/values" in p for _, _, p in rows
    )  # HelmRelease values are in the walk
...
def test_incident_crew516_a_synthetic_overspend_fails(tmp_path):
    ...
    (tmp_path / "platform/oci/variables.tf").write_text(
        'variable "worker_ocpus" {\n  default = 1\n}\n'
    )
    (tmp_path / "platform/oci/main.tf").write_text(
        "node_pools = {\n      size             = 1\n}\n"
    )
    ...
    assert sum(c for c, _, _ in declared_cpu_requests(tmp_path)) > 1 * (
        1 - SYSTEM_RESERVE
    )
    (tmp_path / "platform/a.yaml").write_text(
        "kind: Deployment\nspec:\n  template:\n    spec:\n      containers:\n"
...
@pytest.mark.parametrize(
    "v,cores", [("100m", 0.1), ("1", 1.0), ("1500m", 1.5), (2, 2.0)]
)
