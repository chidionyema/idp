...
def test_session_then_vault_checkout_then_grade():
    n = _names()
    assert (
        n.index("OCI session token from the GitHub OIDC token")
        < n.index("vault checkout for the secure row")
        < n.index("bin/idp-conscience")
    )
    for s in _steps():
        if s.get("name") in (
            "oci cli",
            "OCI session token from the GitHub OIDC token",
            "vault checkout for the secure row",
        ):
...
def test_a_refused_mint_or_clone_is_named_blind_and_the_token_is_masked():
    step = next(
        s for s in _steps() if s.get("name") == "vault checkout for the secure row"
    )
    run = step["run"]
    ...
    assert (
        "exit 1" not in run
    )  # the row goes BLIND; the run is not killed before it grades
