...
spec = importlib.util.spec_from_file_location(
    "wake", BIN, loader=SourceFileLoader("wake", BIN)
)
...
def test_cli_offline_dry_run(tmp_path):
    ...
    assert (
        "wake-blocked: 3 sleeping, 1 woke, 0 capped, 0 errors, open=3 cap=10"
        in r.stdout
    )
...
def test_one_failing_wake_does_not_skip_the_rest(monkeypatch, tmp_path, capsys):
    ...
    two = [
        {"number": 1, "merged_at": None, "body": "Blocked-by: idp#400"},
    ...
    monkeypatch.setattr(
        wake,
        "gh",
        lambda *a: json.dumps([two]) if "--paginate" in a else json.dumps([]),
    )
