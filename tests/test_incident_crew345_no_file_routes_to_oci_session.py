...
def test_every_mention_of_the_login_names_the_receipt_path() -> None:
    for f in FILES:
        ...
        if "oci session authenticate" in text:
            assert RECEIPT.search(text), (
                f"{f.relative_to(ROOT)} mentions the login without naming the receipt path"
            )


def test_the_guard_refuses_the_instance_that_bit() -> None:
    assert ROUTES.search(
        "BLIND   oci-whoami: no $SESSIONS_DIR; run oci session authenticate first"
    )
    assert ROUTES.search("run: oci session authenticate --region <region>")
    assert not ROUTES.search(
        "sessions read state through bin/idp-cluster-state (crew#345), no login"
    )
