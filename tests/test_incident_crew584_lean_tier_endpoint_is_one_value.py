...
def test_the_agent_takes_its_endpoint_from_estate_config():
    ...
    cfg = next(
        d
        for d in _docs(ROOT / "clusters/oke/estate-config.yaml")
        if d["kind"] == "ConfigMap"
    )
...
def test_no_platform_file_names_the_store_endpoint_for_the_agent():
    ...
    hits = [
        p
        for p in (ROOT / "platform/observability-collector").rglob("*.yaml")
        if "signoz-otel-collector.observability.svc" in json.dumps(_docs(p))
    ]  # values, not comments
    assert hits == [], hits
