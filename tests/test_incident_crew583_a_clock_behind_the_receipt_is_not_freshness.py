...
CASES = [
    ...
    (
        "idp-kini-state",
        "kini-finish",
        "ok status=COMPLETED red=0",
        {"KINI_STATE_MAX_AGE_MIN": "60"},
    ...
    (
        ...
        "science-facts",
        "ok sources=3\n{}",
        {"SCIENCE_FACTS_MAX_AGE_MIN": "120"},
        timedelta(minutes=400),
    ),
    (
        "idp-chaos-drill",
        "chaos-drill",
        "ok backstage pod killed, healthcheck stayed 200",
        {"CHAOS_MAX_AGE_HOURS": "194"},
        timedelta(hours=400),
    ),
]
...
def _head(served: datetime, age: timedelta) -> dict:
    ...
    return {
        "last-modified": format_datetime(served - age),
...
def _run(tmp: Path, b: Path, script: str, env: dict) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [str(b / script)], env=e, capture_output=True, text=True, timeout=60
    )
...
@pytest.mark.parametrize(
    "skew",
    [-FAR, timedelta(0), FAR],
    ids=["clock_400d_behind", "clock_correct", "clock_400d_ahead"],
)
...
@pytest.mark.parametrize("script,row,body,env,stale", CASES, ids=IDS)
def test_a_receipt_stamped_after_the_read_that_found_it_is_blind(
    tmp_path, script, row, body, env, stale
):
...
@pytest.mark.parametrize("script,row,body,env,stale", CASES, ids=IDS)
def test_the_store_is_named_as_the_broken_thing_not_the_drill(
    tmp_path, script, row, body, env, stale
):
...
@pytest.mark.parametrize("script,row,body,env,stale", CASES, ids=IDS)
def test_a_stamp_seconds_after_the_read_is_still_fresh(
    tmp_path, script, row, body, env, stale
):
...
@pytest.mark.parametrize("script,row,body,env,stale", CASES, ids=IDS)
def test_an_unreadable_timestamp_is_blind_not_zero(
    tmp_path, script, row, body, env, stale
):
...
def test_every_backend_of_idp_cloud_carries_its_own_clock(tmp_path):
    ...
    delta = (
        parsedate_to_datetime(head["date"])
        - parsedate_to_datetime(head["last-modified"])
    ).total_seconds()
...
def test_the_rule_is_one_file_the_four_import_not_four_copies():
    ...
    for script, _, _, _, _ in CASES:
        ...
        assert "parsedate_to_datetime" not in src, (
            script,
            "still parsing the stamp on its own",
        )
        assert "datetime.now" not in src, (
            script,
            "still asking this machine what time it is",
        )
