...
def test_the_operator_exposes_its_metrics_to_prometheus():
    hr = next(
        d for d in _docs("platform/healing/k8sgpt.yaml") if d["kind"] == "HelmRelease"
    )
...
def test_findings_and_blindness_both_page():
    rule = next(
        d
        for d in _docs("platform/monitoring/rules/k8sgpt.yaml")
    ...
    assert (
        "k8sgpt.yaml"
        in (ROOT / "platform/monitoring/rules/kustomization.yaml").read_text()
    )
...
def test_every_analyser_installed_has_a_rule_that_reads_it():
    ...
    crs = [
        ...
        if any(d.get("kind") == "K8sGPT" for d in _docs(f))
    ]
    assert crs, "no analyser found"
    rules = "".join(
        f.read_text() for f in (ROOT / "platform/monitoring/rules").glob("*.yaml")
    )
