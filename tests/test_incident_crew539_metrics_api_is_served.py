...
def test_metrics_server_is_the_upstream_chart_pinned_and_admissible():
    ...
    assert chart["chart"] == "metrics-server" and chart["version"] == "3.14.0", (
        "pinned upstream chart"
    )
    assert docs["HelmRepository"]["spec"]["url"].startswith(
        "https://kubernetes-sigs.github.io/metrics-server"
    )
    values = hr["spec"]["values"]
    assert values["priorityClassName"] == "system-cluster-critical", (
        "require-priority-class; infrastructure-critical is the radio room only"
    )
    assert (
        values["resources"]["limits"]["cpu"] and values["resources"]["limits"]["memory"]
    ), "require-pod-requests-limits"
    assert docs["Namespace"]["metadata"]["labels"]["app.kubernetes.io/part-of"] == "idp"
    assert docs["Namespace"]["metadata"]["name"] == "metrics-server", (
...
def test_cluster_reconciles_metrics_server_after_scheduling_with_a_health_check():
    rows = {
        d["metadata"]["name"]: d
        for d in _docs(PLATFORM)
        if d.get("kind") == "Kustomization"
    }
    row = rows["metrics-server"]
    assert row["spec"]["path"] == "./platform/metrics-server"
    assert {"name": "scheduling"} in row["spec"]["dependsOn"], (
        "the PriorityClass exists before a pod names it"
    )
    assert row["spec"]["wait"] is True
    hc = row["spec"]["healthChecks"][0]
    assert (hc["kind"], hc["name"], hc["namespace"]) == (
        "Deployment",
        "metrics-server",
        "metrics-server",
    )
