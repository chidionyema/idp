...
def test_a_one_word_what_becomes_a_hint_on_a_measured_sentence():
    d = _mod().describe({"kind": "data", "what": "database", "path": "~/.estate/temporal/dev.db",
                         "mb": 0.84, "referenced": True})
    assert d.startswith("Database file at ~/.estate/temporal/dev.db, 0.84 MB, read by something in the estate"), d
    assert d != "database"
    led = _mod().describe({"kind": "ledger", "what": "the board of record", "path": "~/x.jsonl",
                           "rows": 3, "referenced": False})
    assert "referenced by nothing; the board of record." in led, led


def test_a_full_sentence_what_still_wins():
    d = _mod().describe({"kind": "data", "what": "every session verbatim: his words, every tool call",
                         "path": "~/.claude/projects", "mb": 6376.7, "referenced": True})
...
def test_only_a_shebang_is_skipped_before_the_comment_block(tmp_path):
    ...
    y.write_text("# External Secrets: syncs the vault into\n# Kubernetes Secrets. Second sentence.\napiVersion: v1\n")
    assert _mod()._docline(str(y)) == "External Secrets: syncs the vault into Kubernetes Secrets."
    sh = tmp_path / "job.sh"
    ...
    lab.write_text("# Edge row. Gateway API and DNS for every public door.\nresources: []\n")
    assert _mod()._docline(str(lab)) == "Edge row. Gateway API and DNS for every public door."
...
def test_every_generated_description_is_a_sentence(tmp_path):
    ...
    r = subprocess.run([sys.executable, str(GEN)],
                       env={**os.environ, "INV": str(FIX), "OUT": str(out), "ESTATE_ENV": "dev",
                            "CATALOG_GEN_ROOT": str(ROOT)}, capture_output=True, text=True)
    ...
    for d in docs:
        ...
        if len(desc.split()) < 3 or desc.lower().rstrip(".") in STUBS or desc.lower() == md.get("name", "").lower():
