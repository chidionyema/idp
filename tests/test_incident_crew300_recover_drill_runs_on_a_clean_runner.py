...
def test_recover_drill_is_weekly_on_the_exchanged_oidc_session_with_no_static_credential() -> (
    None
):
    wf = yaml.safe_load(WF.read_text())
    assert [s["cron"] for s in wf[True]["schedule"]] == ["41 4 * * 0"]
    assert (
        "workflow_dispatch" in wf[True]
        and SCRIPT.relative_to(ROOT).as_posix() in wf[True]["pull_request"]["paths"]
    )
    assert wf["permissions"] == {"id-token": "write", "contents": "read"}
    steps = wf["jobs"]["recover"]["steps"]
    assert any(TOKEN_EXCHANGE in s.get("uses", "") for s in steps), (
    ...
    assert "GH_TOKEN" not in drill["env"], (
        "the clones must use the App lane, not the job's own token"
    )
    ...
    assert (
        'GH_TOKEN="${RECOVER_BOOT_GH_TOKEN:-}" "$WORK/idp/bin/idp-verify-drill"'
        in script
    )
    assert script.count("RECOVER_BOOT_GH_TOKEN") == 1, (
        "the boot token must reach exactly one call"
    )
    assert "grep -E '^(FAIL|BLIND) ' \"$OUT/verify-drill.txt\"" in script, (
        "a red boot must name its rows in the log"
    )
    text = WF.read_text()
    assert not re.search(
        r"OCI_(API|PRIVATE)_KEY|FINGERPRINT|PASSWORD|R2_|RCLONE_", text
    ), "a static credential on the recovery path"
    assert any(
        s.get("name") == "recover-receipt" for s in (st.get("with", {}) for st in steps)
    ), "no receipt artifact"
...
def test_recover_drill_is_catalogued_with_its_own_cron_verbatim() -> None:
    drills = {
        d["name"]: d
        for d in yaml.safe_load((ROOT / "drills" / "catalogue.yaml").read_text())[
            "drills"
        ]
    }
    ...
    assert 168 < row["max_age_hours"] <= 194, (
        "a weekly drill: seven days plus GitHub's scheduling slack"
    )


def test_the_recovery_lane_can_only_read_and_the_vault_is_found_by_name_not_tofu_state() -> (
    None
):
    lanes = json.loads((ROOT / "platform" / "github-app" / "lanes.json").read_text())
    assert lanes["recovery"] == {"metadata": "read", "contents": "read"}, lanes[
        "recovery"
    ]
    ...
    assert app.count('"$IDP/bin/idp-cloud" secret get "$SECRET"') == 2, (
        "both token paths read the key through bin/idp-cloud"
    )
    assert "tofu output" not in app and "oci vault secret list" not in app
    cloud = (ROOT / "bin" / "idp-cloud").read_text()
    assert cloud.count("V=${ESTATE_VAULT_OCID:-$(cd") >= 2, (
        "idp-cloud vault lookups must accept the override a runner without tofu state needs"
    )
    script = SCRIPT.read_text()
    ...
    assert not re.search(r"ocid1\.[a-z]+\.oc1\.[a-z0-9.-]*\.[a-z0-9]{20,}", script), (
        "an OCID literal (LAW 46)"
    )
...
def _run(env: dict, tmp: Path) -> subprocess.CompletedProcess:
    ...
    full = {
        ...
        "HOME": str(tmp),
        "STUB_LOG": str(tmp / "calls"),
        "RECOVER_WORK": str(tmp / "work"),
        "RECOVER_RECEIPT_DIR": str(tmp / "receipt"),
        **env,
    }
    return subprocess.run(
        [str(SCRIPT)], env=full, capture_output=True, text=True, timeout=60
    )


def test_without_an_exchanged_session_the_drill_is_blind_and_touches_no_network(
    tmp_path: Path,
) -> None:
    ...
    assert not (tmp_path / "calls").exists(), (
        "a blind drill must not call oci or rclone"
    )
    assert (tmp_path / "receipt" / "rows.txt").read_text() == r.stdout, (
        "the receipt is the rows, verbatim"
    )


def test_when_the_vault_cannot_be_listed_the_drill_is_blind_not_red(
    tmp_path: Path,
) -> None:
    r = _run(
        {
    ...
    assert (tmp_path / "calls").read_text().count("oci") == 1, (
        "one vault listing, nothing after a blind vault"
    )


def test_the_incremental_bundle_check_matches_what_git_actually_prints(
    tmp_path: Path,
) -> None:
    ...
    pat = re.search(
        r"grep -q '([^']+)' \"\$OUT/bundle-\$d.txt\"", SCRIPT.read_text()
    ).group(1)
    src = tmp_path / "src"
    src.mkdir()
    g = lambda *a, cwd=src: subprocess.run(
    ...
    g("init", "-q", cwd=empty)
    r = subprocess.run(
        ["git", "bundle", "verify", str(tmp_path / "inc.bundle")],
        cwd=empty,
        capture_output=True,
        text=True,
    )
    assert r.returncode != 0
    assert re.search(pat, r.stdout + r.stderr), (
        f"script pattern {pat!r} does not match git's wording: {r.stderr!r}"
    )


def test_a_token_is_graded_by_its_shape_in_both_scripts_never_by_being_non_empty() -> (
    None
):
    ...
    assert re.search(shape, drill), (
        "bin/idp-recover-drill does not grade the token by shape"
    )
    ...
    assert re.search(r"^\s*[^#\n]*\bghs_[A-Za-z0-9]{10,}", drill + app, re.M) is None, (
        "a literal token in a script"
    )
    for good in ("ghs_" + "a" * 36, "ghs_" + "a" * 200 + "_b.c-d"):
        assert re.fullmatch(r"ghs_[A-Za-z0-9_.-]+", good)
    assert (
        re.fullmatch(r"ghs_[A-Za-z0-9_.-]+", "BLIND   github-app no installation token")
        is None
    )
...
def test_an_app_jwt_is_sent_as_bearer_never_through_gh_token() -> None:
    ...
    for path in ("/app/installations", "/app/installations/$inst/access_tokens"):
        assert f'app_api "$jwt" {path}' in app or f'app_api "$jwt" "{path}"' in app, (
            path
        )


def test_a_prefix_with_no_latest_bundle_is_named_withdrawn_never_graded_by_rclone_exit_code(
    tmp_path: Path,
) -> None:
    ...
    assert re.search(
        r'rclone copyto ":s3:\$B/bundles/\$d/latest\.bundle" "\$f" 2>"\$OUT/copy-bundle-\$d\.err" && \[ -s "\$f" \]',
        script,
    ), "a copy is graded by the file it left, not by rclone's exit code"
