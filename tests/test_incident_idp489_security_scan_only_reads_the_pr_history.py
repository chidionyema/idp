...
SCRIPT = Path(
    os.environ.get(
        "ESTATE_SECURITY_SCAN",
        Path(__file__).resolve().parents[1] / "bin" / "estate-security-scan",
    )
)
pytestmark = pytest.mark.skipif(
    shutil.which("gitleaks") is None, reason="gitleaks not on PATH"
)
...
def _scan(repo: Path) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [str(SCRIPT), "--quiet", "--source", str(repo)], capture_output=True, text=True
    )
...
def test_a_leak_on_another_branch_does_not_fail_this_checkout(
    repo_with_leak_on_side_branch: Path,
) -> None:
...
def test_the_branch_that_carries_the_leak_still_fails(
    repo_with_leak_on_side_branch: Path,
) -> None:
