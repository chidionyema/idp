...
def _tree(tmp):
    ...
    (g / "a-guard.py").write_text(
        '"""Refuse a session that starts stale (LAW 7).\n\nmore."""\n'
    )
        ...
        json.dumps(
            {
                "hooks": {
                    "SessionStart": [
                        {
                            "hooks": [
                                {"command": f"python3 {g}/hook-run.py {g}/a-guard.py"}
                            ]
                        }
                    ],
                    "UserPromptSubmit": [],
                    "PreToolUse": [
                        {
                            "matcher": "Bash",
                            "hooks": [
                                {"command": f"python3 {g}/hook-run.py {g}/b-guard.py"}
                            ],
                        }
                    ],
                    "Stop": [],
...
def test_page_is_rendered_from_the_tree_and_check_sees_staleness(tmp_path):
    ...
    try:
        ...
        assert (
            "| 1 | `a-guard.py` | Refuse a session that starts stale (LAW 7) | 7 | every tool |"
            in text
        ), text
        assert "| 1 | `b-guard.py` | Refuse git add -A | — | Bash |" in text, text
        assert "## UserPromptSubmit" not in text, (
            "an event with no guards is not a section"
        )
        assert (
            subprocess.run([sys.executable, str(GEN), "--check"], env=env).returncode
            == 0
        )
