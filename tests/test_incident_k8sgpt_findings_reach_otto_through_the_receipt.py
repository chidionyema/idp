...
def test_the_receipt_carries_the_findings_and_the_head_counts_them():
    ...
    assert "list failed" in src.split("core.k8sgpt.ai/v1alpha1/results")[1][:600], (
        "a failed list must be a row, never a zero"
    )
...
def test_otto_may_run_the_reader():
    ...
    assert (
        "Bash(bin/idp-cluster-state*)"
        in (ROOT / "platform/hermes-agent/estate.yaml").read_text()
    )
