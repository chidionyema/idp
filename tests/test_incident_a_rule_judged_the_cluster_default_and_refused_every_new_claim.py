...
def test_no_admission_rule_judges_a_claim_s_storage_class():
    for name, rule in _rules().items():
        kinds = [
            k
            for m in rule.get("match", {}).get("any", [])
            for k in m.get("resources", {}).get("kinds", [])
        ]
...
def test_the_static_gate_refuses_a_provider_storage_class_in_the_platform_tree():
    ...
    assert rx.search("storageClassName: oci-bv"), (
        "the static gate must catch a provider class where it is written"
    )
