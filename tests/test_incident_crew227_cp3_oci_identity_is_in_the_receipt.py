...
def _collect_src():
    ...
    return collect.split("spiffe = spiffe_rows()", 1)[1].split("at = datetime.now", 1)[
        0
    ]
...
PODS = [
            ...
            "containers": [
                {
                    ...
                    "command": ["/bin/sh", "-c"],
                    "args": [
                        "oci --auth instance_principal os object put --bucket-name b"
                    ],
                    "env": [],
                }
            ],
            ...
            "containers": [
                {
                    ...
                    "image": "ghcr.io/oracle/oci-cli:20260826",
                    "command": ["/bin/sh", "-c"],
                    "args": ["oci os object put"],
                    "env": [],
                    "volumeMounts": [{"name": "key", "mountPath": "/k"}],
                }
            ],
            ...
            "containers": [
                {
                    "name": "app",
                    "image": "estate/app:1",
                    "env": [{"name": "OCI_CLI_AUTH", "value": "config_file"}],
                }
            ],
            "volumes": [],
        },
    ...
    # not an OCI caller at all: must not appear in the row.
    {
        ...
        "spec": {
            "containers": [{"name": "web", "image": "estate/web:1", "env": []}],
            "volumes": [],
        },
    },
]
...
def test_receipt_names_oci_callers_their_identity_and_any_static_key():
    ...
    assert "backstage/unrelated" not in by_pod, (
        "a container that never calls OCI must not be in the row"
    )
    assert by_pod["backstage/cluster-state-clean"] == {
        "pod": "backstage/cluster-state-clean",
        "container": "receipt",
        "identity": "instance_principal",
        "static_key": [],
    }
    ...
    assert got["static_key_pods"] == [
        "backstage/cluster-state-static-env",
        "backstage/cluster-state-static-vol",
    ]
    assert got["unknown_identity_pods"] == [
        "backstage/cluster-state-static-vol",
        "llm/sdk-caller",
    ]
...
def test_a_clean_cluster_has_no_static_key_and_no_unknown_identity():
    ...
    assert (
        len(got["oci_pods"]) == 1
        and got["oci_pods"][0]["identity"] == "instance_principal"
    )
