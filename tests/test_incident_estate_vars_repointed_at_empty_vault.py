...
def _run(tmp: Path, count: str) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [
            "bash",
            "-c",
            f'IDP="{tmp}/idp"; VAULT_ID=ocid1.vault.new\n{BLOCK}\necho "VAULT_ID=$VAULT_ID"',
        ],
        env=env,
        capture_output=True,
        text=True,
...
def test_incident_estate_vars_repointed_at_empty_vault(tmp_path: Path) -> None:
    r = _run(tmp_path, "10")
    assert r.returncode == 3 and "REFUSE" in r.stdout
    r = _run(tmp_path, "0")
    assert (
        r.returncode == 0
        and "switching" in r.stdout
        and "VAULT_ID=ocid1.vault.new" in r.stdout
    )
    r = _run(tmp_path, "BLIND")
    assert r.returncode == 2 and "BLIND" in r.stdout and "flux" in r.stdout
