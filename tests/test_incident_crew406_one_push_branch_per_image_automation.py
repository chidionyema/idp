...
def shared_push_branches(automations: list[dict]) -> list[str]:
    ...
    c = Counter(
        f"{a['spec']['sourceRef']['name']}/{a['spec']['git']['push']['branch']}"
...
def test_the_rule_refuses_the_pair_that_collided() -> None:
    def auto(name, branch):
        return {
            ...
            "metadata": {"name": name},
            "spec": {
                "sourceRef": {"name": "idp-writer"},
                "git": {"push": {"branch": branch}},
            },
        }

    assert shared_push_branches(
        [
            auto("backstage", "flux/image-updates"),
            auto("sovereign-worker", "flux/image-updates"),
        ]
    ) == ["idp-writer/flux/image-updates"]
