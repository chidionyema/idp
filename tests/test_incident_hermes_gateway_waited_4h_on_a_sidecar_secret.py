...
def _required_sidecar_secret_volumes(volumes: list[dict]) -> list[str]:
    return [
        v["name"]
        for v in volumes
        if v["name"] not in GATEWAY_OWN
        and "secret" in v
        and not v["secret"].get("optional", False)
    ]
...
def test_the_tailscale_volume_is_still_mounted_and_optional():
    volumes = {
        v["name"]: v for v in _deployment()["spec"]["template"]["spec"]["volumes"]
    }
    assert volumes["tailscale-authkey"]["secret"] == {
        "secretName": "hermes-agent-tailscale",
        "optional": True,
    }
    ...
    bad = [
        {
            ...
            "secret": {"secretName": "hermes-agent-tailscale"},
        }
    ]
