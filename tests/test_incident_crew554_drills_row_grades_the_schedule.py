...
def _load():
    ...
    helpers = text[
        text.index("def _field(") : text.index("now = datetime.now(timezone.utc)")
    ]
    ns: dict = {}
...
def test_expected_firings_match_the_cron_promise() -> None:
    ...
    assert (
        ns["expected_firings"]("0 6 * * 1", end) == 0
    )  # a Friday window holds no Monday
...
def _fake_gh(b: Path, runs_spec: list, now: datetime) -> None:
    ...
    runs = [
        {"created_at": ts, "event": ev, "triggering_actor": {"login": who}}
        for ev, who in runs_spec
    ]
    (b / "gh").write_text(
        "#!/usr/bin/env python3\n"
        "import sys, json\n"
...
def _estate(
    tmp: Path, sched_runs: int, dispatched: list | None = None
) -> tuple[Path, Path]:
    now = datetime.now(timezone.utc)
    b = tmp / "bin"
    b.mkdir()
    _fake_gh(
        b, [("schedule", "github-actions[bot]")] * sched_runs + (dispatched or []), now
    )
...
def _run(tmp: Path, b: Path, cat: Path) -> subprocess.CompletedProcess:
    env = {
        "PATH": f"{b}:{os.path.dirname(os.sys.executable)}:/usr/bin:/bin",
        "HOME": str(tmp),
    }
...
def test_a_cron_that_fired_twice_in_24h_is_a_red_schedule_row(tmp_path: Path) -> None:
    ...
    assert (
        "FAIL      drills    schedule               login-drill.yml fired 2 of 24 promised"
        in r.stdout
    ), r.stdout
...
def test_a_cron_that_fired_every_hour_is_green(tmp_path: Path) -> None:
    ...
    r = _run(tmp_path, b, cat)
    assert (
        "ok        drills    schedule               login-drill.yml fired 24 of 24 promised"
        in r.stdout
    ), r.stdout + r.stderr
...
def test_a_persons_dispatches_are_a_hand_not_a_clock(tmp_path: Path) -> None:
    b, cat = _estate(
        tmp_path,
        2,
        [("workflow_dispatch", "chidionyema")] * 22 + [("push", "chidionyema")] * 5,
    )
    r = _run(tmp_path, b, cat)
    assert (
        "FAIL      drills    schedule               login-drill.yml fired 2 of 24 promised"
