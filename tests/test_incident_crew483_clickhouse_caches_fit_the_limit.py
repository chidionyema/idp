...
def test_clickhouse_caches_and_merges_leave_inserts_more_than_half_the_ceiling() -> (
    None
):
    ch = _clickhouse()
    ...
    assert "mark_cache_size" in settings, (
        "mark_cache_size unset: ClickHouse's default is 5 GiB"
    )
    marks = int(settings["mark_cache_size"]) + int(
        settings.get("index_mark_cache_size", 0)
    )
    merges = (
        float(settings.get("merges_mutations_memory_usage_to_ram_ratio", 0.5)) * ceiling
    )
