...
def test_kyverno_honours_exceptions_from_the_namespace_the_row_uses():
    ...
    hr = next(
        d
        for d in _docs(ROOT / "platform" / "kyverno" / "kyverno.yaml")
...
def test_the_exception_waives_exactly_the_policies_the_chart_fails(tmp_path):
    ...
    excepted = {
        e["policyName"] for e in _docs(MESH / "exception.yaml")[0]["spec"]["exceptions"]
    }
    assert excepted == failing, {
        "waived for nothing": sorted(excepted - failing),
        "missing": sorted(failing - excepted),
    }
