...
def run(args, env, tenets=None, tmp_path=None):
    if tenets is not None:
        env = {
            **env,
            "CONSCIENCE_TENETS": str(tenets),
            "CONSCIENCE_REPORT": str(tmp_path / "r.json"),
        }
    return subprocess.run(
        [sys.executable, str(CONSCIENCE), *args],
        capture_output=True,
        text=True,
        env=env,
        cwd=IDP,
...
def test_a_red_row_files_one_issue_on_the_board_and_a_green_row_files_nothing(tmp_path):
    ...
    assert p.returncode == 1, (
        p.stdout + p.stderr
    )  # still red: --act never launders the grade
    calls = pathlib.Path(env["GH_CALLS"]).read_text().splitlines()
    creates = [c for c in calls if c.startswith("issue create")]
    assert (
        len(creates) == 1
        and "conscience: b is red" in creates[0]
        and "--repo someone/crew" in creates[0]
    )
    assert not any("conscience: a is red" in c for c in calls), (
        "the green row must file nothing"
    )
...
def test_ci_minutes_reads_the_latest_green_run_and_flakes_count_reruns(tmp_path):
    ...
    runs = [
        {
            "createdAt": "2026-08-28T17:41:48Z",
            "updatedAt": "2026-08-28T17:49:16Z",
            "conclusion": "success",
            "attempt": 1,
            "databaseId": 1,
        },
        {
            "createdAt": "2026-08-28T17:31:45Z",
            "updatedAt": "2026-08-28T17:40:55Z",
            "conclusion": "success",
            "attempt": 2,
            "databaseId": 2,
        },
    ...
    assert p.returncode == 0 and p.stdout.strip().splitlines()[-1] == "8", (
        p.stdout + p.stderr
    )  # 7.5 min rounds up
...
def test_pr_age_counts_only_old_open_prs(tmp_path):
    env = shim(tmp_path)
    (tmp_path / "prs.json").write_text(
        json.dumps(
            [
                {"number": 5, "createdAt": "2000-01-01T00:00:00Z"},
                {"number": 6, "createdAt": "2999-01-01T00:00:00Z"},
            ]
        )
    )
    ...
    assert (
        p.returncode == 0
        and p.stdout.strip().splitlines()[-1] == "1"
        and "#5" in p.stdout
    )
...
def test_the_engineering_rows_are_in_the_same_file_as_the_ethos():
    rows = {
        r["name"]: r
        for r in yaml.safe_load((IDP / "conscience" / "tenets.yaml").read_text())[
            "tenets"
        ]
    }
    for name in (
