...
def test_an_arrow_without_numbers_is_refused(tmp_path):
    assert "rule=optimised_plan" in _rules(
        _with_body(tmp_path, "Optimised: slow -> fast; cut: things\n")
    )
...
def test_incident_an_existing_generated_body_gains_the_line(path):
    ...
    assert refresh, (
        f"{path} writes the line on create but never refreshes an existing body"
    )
