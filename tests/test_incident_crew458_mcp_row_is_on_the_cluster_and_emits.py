...
def row_docs():
    ...
    for f in sorted(ROW.glob("*.yaml")):
        if f.name == "agentgateway.yaml":  # the gateway's own config file, not a k8s object
            continue
        out.extend(docs(f))
    return out
...
def test_incident_crew458_the_mcp_row_is_a_flux_row_behind_the_edge():
    ...
    assert (parent["name"], parent["namespace"], parent["sectionName"]) == ("prospector-edge", "prospector", "https-mcp")
