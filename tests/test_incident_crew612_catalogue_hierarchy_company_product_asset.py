...
def test_every_asset_is_reachable_from_a_company_through_a_product(tmp_path):
    ...
    repos = [
        d["metadata"]["name"]
        for d in docs
        if d["kind"] == "Component" and d["spec"]["type"] == "service"
    ]  # catalog-gen MAP: repo -> Component/service
    ...
    for d in assets:
        inside = [
            t.split("/")[-1]
            for t in d["spec"].get("dependsOn", [])
            if t.startswith("component:default/")
        ]
        inside = [t for t in inside if t in repo_set]
        if d["metadata"]["name"] in repo_set:
            assert (
                d["spec"]["system"] == f"system:default/product-{d['metadata']['name']}"
            ), d["metadata"]["name"]
        elif inside:
            assert d["spec"]["system"] == f"system:default/product-{inside[0]}", (
                d["metadata"]["name"],
                d["spec"]["system"],
            )
    assert any(
        d["spec"].get("dependsOn")
        for d in assets
        if d["metadata"]["name"] not in repo_set
    ), "fixture proves nothing: no asset inside a repo"
...
def test_every_company_product_and_group_carries_a_link_a_person_can_open(tmp_path):
    ...
    inv["rows"] += [
        ...
        },
        {
            ...
            "kind": "ledger",
            "root": "a",
            "path": str(repo / "untracked.sqlite"),
            "coupling": None,
        },
    ]
    ...
    links = {
        d["metadata"]["name"]: [l["url"] for l in d["metadata"].get("links") or []]
        for d in docs
        if d["kind"] == "Resource"
    }
    tracked = [
        n for n in links if n.endswith("tracked.sqlite") and "untracked" not in n
    ]
    untracked = [n for n in links if n.endswith("untracked.sqlite")]
    assert tracked and untracked, list(links)
    assert links[tracked[0]] == [
        "https://github.com/example/realrepo/tree/HEAD/tracked.sqlite"
    ], links[tracked[0]]
    assert links[untracked[0]] == [], (
        "an untracked path would 404; it must carry no code link"
    )
