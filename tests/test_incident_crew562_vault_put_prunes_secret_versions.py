...
def _versions(n: int, pending_deletion: int = 0):
    ...
    for i in range(1, n + 1):
        stages = (
            ["CURRENT", "LATEST"]
            if i == n
            else (["PREVIOUS"] if i == n - 1 else ["DEPRECATED"])
        )
...
def _put(tmp_path, versions, keep=None):
    ...
    env = dict(
        ...
        OCI_CLI_PROFILE="fake",
        OCI_COMPARTMENT_OCID="ocid1.compartment.oc1..fake",
        FAKE_LOG=str(log),
        FAKE_VERSIONS=str(tmp_path / "versions.json"),
        FAKE_BUNDLE=str(tmp_path / "bundle"),
    )
    if keep is not None:
...
def test_at_the_cap_every_deprecated_version_but_the_newest_two_is_scheduled_before_the_update(
    tmp_path,
):
    ...
    nums = sorted(
        int(ln.split("--secret-version-number ")[1].split()[0]) for ln in sched
    )
    # 30 versions: CURRENT=30 stays; 29 (PREVIOUS) and 28 are the two kept; 1..27 go
    assert nums == list(range(1, 28)), nums
    assert all(
        "--time-of-deletion 2026-" in ln or "--time-of-deletion 20" in ln
        for ln in sched
    )
    assert lines.index(sched[-1]) < lines.index(
        next(ln for ln in lines if ln.startswith("update "))
...
def test_versions_already_pending_deletion_are_not_scheduled_twice(tmp_path):
    ...
    assert r.returncode == 0, r.stdout + r.stderr
    nums = sorted(
        int(ln.split("--secret-version-number ")[1].split()[0])
        for ln in lines
        if "schedule-deletion" in ln
    )
