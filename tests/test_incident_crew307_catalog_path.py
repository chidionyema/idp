...
def test_incident_crew307_catalog_stays_at_catalog():
    ...
    assert not re.search(r"^\s*- page:catalog:", cfg, re.M), (
        "page:catalog override moves /catalog; crew#307"
    )
...
def test_incident_crew307_broken_path_row_says_what_rendered():
    ...
    src = (
        Path(__file__).resolve().parents[1].joinpath("bin/idp-login-drill").read_text()
    )
