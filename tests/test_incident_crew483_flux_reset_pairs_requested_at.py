    ...
    for f in sorted(PLATFORM.rglob("*.yaml")):
        for d in yaml.safe_load_all(f.read_text()):
            ...
            if any(t in ann for t in TRIGGERS):
                out.append(
                    (
                        f"{f.relative_to(PLATFORM)}:{d['kind']}/{d['metadata']['name']}",
                        ann,
                    )
                )
    return out
...
def test_every_flux_reset_or_force_annotation_pairs_a_matching_requested_at() -> None:
    found = _annotated()
    assert found, (
        "the k8s-infra reset this test was written for is gone; delete the test with it"
    )
    for where, ann in found:
        requested = ann.get("reconcile.fluxcd.io/requestedAt")
        for t in TRIGGERS:
            if t in ann:
                assert ann[t] == requested, (
                    f"{where}: {t}={ann[t]!r} requestedAt={requested!r}"
                )
