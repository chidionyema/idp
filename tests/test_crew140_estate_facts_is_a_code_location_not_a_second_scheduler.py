...
def _locations() -> dict[str, dict]:
    ...
    return {
        e["python_file"]["location_name"]: e["python_file"] for e in doc["load_from"]
    }
...
def test_workspace_names_both_locations_by_relative_path():
    ...
    assert (
        facts["relative_path"] == "../../crew/science/scheduler/estate_dagster/facts.py"
    )
...
def test_facts_location_loads_on_this_interpreter_when_the_crew_checkout_is_beside_idp():
    ...
    r = subprocess.run(
        [
            sys.executable,
            "-c",
            "from estate_dagster.facts import defs, SPECS; print(len(SPECS))",
        ],
        cwd=wd,
        capture_output=True,
        text=True,
