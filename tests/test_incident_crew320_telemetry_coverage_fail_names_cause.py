...
def test_fail_receipt_names_backend_error_and_missing_pods() -> None:
    body = "FAIL telemetry-coverage at X pods=45 seen=0 missing=45\n" + json.dumps(
        {
            "backend_errors": {
                "logs": "Code: 60. Table signoz_logs.distributed_logs_v2 does not exist"
            },
...
def test_ok_receipt_still_passes_with_no_cause_lines() -> None:
    rc, out = _run(
        "ok telemetry-coverage at X pods=45 seen=45 missing=0 hubble_radio_flows=12\n{}"
    )
    assert rc == 0
...
def test_receipt_without_the_hubble_row_is_a_miss_and_zero_names_it() -> None:
    ...
    rc, out = _run(
        'ok telemetry-coverage at X pods=45 seen=45 missing=0 hubble_radio_flows=0\n{"hubble_radio_flows": 0}'
    )
    assert (
        rc == 1
        and "hubble_radio_flows=0" in out
        and "named a radio-room workload" in out
    ), out
