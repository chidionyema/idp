...
def test_receipt_names_spiffe_ids_and_csi_workloads_and_the_role_allows_the_list():
    ...
    assert any(
        "spire.spiffe.io" in r["apiGroups"] and "clusterspiffeids" in r["resources"]
        for r in role["rules"]
    )
    ...
    pods = [
        {
            "metadata": {"namespace": "a", "name": "with-svid"},
            "status": {"phase": "Succeeded"},
            "spec": {"containers": [{"name": "fetch"}], "volumes": csi},
        },
        {
            "metadata": {"namespace": "a", "name": "log-broken"},
            "status": {"phase": "Succeeded"},
            "spec": {"containers": [{"name": "fetch"}], "volumes": csi},
        },
        {
            "metadata": {"namespace": "a", "name": "still-running"},
            "status": {"phase": "Running"},
            "spec": {"containers": [{"name": "fetch"}], "volumes": csi},
        },
        {
            "metadata": {"namespace": "a", "name": "plain"},
            "spec": {"volumes": [{"name": "d", "emptyDir": {}}]},
        },
        {
            "metadata": {
                "namespace": "s",
        ...
        },
        {
            ...
            "status": {"phase": "Running"},
            "spec": {"containers": [{"name": "spire-server"}], "volumes": []},
        },
    ]
    agent_log = (
        'time=1 level=info msg="Node attestation was successful"\ntime=2 level=info msg=quiet\n'
        'time=3 level=error msg="Failed to collect all selectors for PID" error="dial tcp 127.0.0.1:10250"\n'
    )
    LONG = (
        'time=4 level=info msg="Created entry" spiffe_id=spiffe://estate/ns/a/sa/proof parent_id=spiffe://estate/spire/agent/k8s_psat/estate/n1'
        + " selectors="
        + "k8s:pod-label:x," * 40
    )
    ...
    svids = [
        {
            "pod": "a/with-svid",
            "container": "fetch",
            "spiffe_ids": ["spiffe://estate/ns/a/sa/proof"],
            "error": "",
        },
        {
            "pod": "a/log-broken",
        ...
        "items": [
            {
                ...
                "status": {"stats": {"podsSelected": 3, "entriesToRender": 3}},
            }
        ]
    }
    prog = (
    ...
    assert got == {
        "clusterspiffeids": [
            {"name": "agents", "podsSelected": 3, "entriesToRender": 3}
        ],
        "error": "",
        "csi_workloads": csi_workloads,
        "svids": svids,
        "agents": agents,
        "server": server,
    }
    ...
    got = json.loads(
        subprocess.run(
            [sys.executable, "-c", failing], capture_output=True, text=True, check=True
        ).stdout
    )
    assert (
        got["clusterspiffeids"] is None
        and "403" in got["error"]
        and got["csi_workloads"] == csi_workloads
        and got["svids"] == svids
        and got["agents"] == agents
        and got["server"] == server
    )
