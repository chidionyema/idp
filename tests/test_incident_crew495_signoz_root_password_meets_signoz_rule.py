...
BLOCK = re.compile(
    r'resource "random_password" "signoz_root" \{(?P<body>.*?)\n\}', re.S
)
...
def _check(text: str) -> None:
    b = _block(text)
    assert b.get("special") == "true", (
        "SigNoz wants a symbol; special = false can never carry one"
    )
    for k in ("min_upper", "min_lower", "min_numeric", "min_special"):
        assert int(b.get(k, "0")) >= 1, (
            f"{k} must pin at least one character of its class"
        )
    assert int(b.get("length", "0")) >= 8, "SigNoz's minimum length"
    assert b.get("override_special"), (
        "override_special must be set, or the provider's default set includes $ and quotes"
    )
    bad = UNSAFE & set(b["override_special"])
    assert not bad, (
        f"override_special carries characters the env/URL path would misread: {sorted(bad)}"
    )


def test_incident_crew495_the_signoz_root_password_meets_the_rule_signoz_enforces() -> (
    None
):
    _check(TF.read_text())
...
def test_incident_crew495_the_rule_refuses_the_shapes_that_crashed_or_would(
    broken: str,
) -> None:
