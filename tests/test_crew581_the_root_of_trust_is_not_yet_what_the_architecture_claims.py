...
def _shell_scripts():
    return sorted(
        p for p in BIN.iterdir() if p.is_file() and not p.name.endswith((".py", ".md"))
    )


def _manifests():
    return sorted(
        p for d in ("platform", "clusters") for p in (ROOT / d).rglob("*.yaml")
    )
...
def test_the_vault_terraform_is_read_and_declares_a_key():
    text = _vault_tf()
    assert "oci_kms_key" in text, (
        f"{VAULT_TF} declares no KMS key; this test is grading nothing"
    )
    scalars = _hcl_scalars(text)
    for key in HSM_TARGET:
        assert key in scalars, (
            f"{key} is not declared in {VAULT_TF}; the parser would pass blindly"
        )
...
def test_the_bin_directory_is_read_and_the_home_key_pattern_matches_what_it_describes():
    ...
    assert len(scripts) > 50, (
        f"only {len(scripts)} scripts found under {BIN}; wrong path"
    )
...
def test_the_manifest_sweep_finds_the_spiffe_proof_harness():
    mounting = {
        str(p.relative_to(ROOT)) for p in _manifests() if SPIFFE_CSI in p.read_text()
    }
    assert mounting, (
        f"no manifest mounts {SPIFFE_CSI}; the sweep is reading the wrong tree"
    )
...
def test_no_script_roots_the_estate_in_a_key_file_under_a_home_directory():
    offenders = [
        p.name for p in _shell_scripts() if HOME_KEY_DEFAULT.search(p.read_text())
    ]
    assert offenders == []
