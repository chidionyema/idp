...
def test_window_is_24h_and_hourly():
    ...
    assert isinstance(w["days-before-pr-close"], int), (
        "actions/stale parseInt()s the close window"
    )
    assert w["days-before-issue-stale"] == -1 and w["days-before-issue-close"] == -1
    assert wf[True]["schedule"][0]["cron"].split()[1] == "*", (
        "must run hourly, not daily"
    )
...
def test_close_comment_names_both_ways_back():
    ...
    assert (
        "Blocked-by:" in w["close-pr-message"]
        and "wake-blocked" in w["close-pr-message"]
    )
