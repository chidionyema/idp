...
def _estate(
    tmp: Path, entries: list, last_green: dict, firings: dict, names: dict | None = None
):
...
def _run(
    tmp: Path, b: Path, cat: Path, env: dict | None = None
) -> subprocess.CompletedProcess:
    e = {
        "PATH": "%s:%s:/usr/bin:/bin" % (b, os.path.dirname(sys.executable)),
        "HOME": str(tmp),
    }
    e.update(env or {})
    return subprocess.run(
        [str(SCRIPT), str(cat), str(tmp)],
        env=e,
        capture_output=True,
        text=True,
        timeout=60,
...
def _in_ci(
    ref: str = "chidionyema/idp/.github/workflows/verify-drill.yml@refs/heads/main",
) -> dict:
    return {"GITHUB_RUN_ID": RUN_IN_FLIGHT, "GITHUB_WORKFLOW_REF": ref}


def test_the_workflow_the_row_runs_inside_is_not_graded_by_its_own_history(
    tmp_path: Path,
) -> None:
    ...
    b, cat = _estate(
        tmp_path,
        [("verify-drill.yml", 3)],
        {"verify-drill.yml": None},
        {"verify-drill.yml": _hourly(24, now)},
    )
    r = _run(tmp_path, b, cat, _in_ci())
    assert (
        "no successful run of verify-drill.yml has ever been recorded" not in r.stdout
    ), r.stdout
    assert (
        "verify-drill.yml is the run computing this row (run %s)" % RUN_IN_FLIGHT
        in r.stdout
    ), r.stdout + r.stderr
    assert r.returncode == 0, r.stdout + r.stderr


def test_off_a_runner_the_same_workflow_with_no_green_run_is_still_red(
    tmp_path: Path,
) -> None:
    ...
    assert "no successful run of verify-drill.yml has ever been recorded" in r.stdout, (
        r.stdout + r.stderr
    )
    assert r.returncode == 1, r.stdout


def test_only_the_workflow_in_flight_is_exempt_never_the_rest_of_the_catalogue(
    tmp_path: Path,
) -> None:
    ...
    b, cat = _estate(
        tmp_path,
        [("verify-drill.yml", 3), ("login-drill.yml", 3)],
        {"verify-drill.yml": None, "login-drill.yml": None},
        {"verify-drill.yml": _hourly(24, now), "login-drill.yml": _hourly(24, now)},
    )
    r = _run(tmp_path, b, cat, _in_ci())
    assert "verify-drill.yml is the run computing this row" in r.stdout, (
        r.stdout + r.stderr
    )
    assert "no successful run of login-drill.yml has ever been recorded" in r.stdout, (
        r.stdout
    )
...
def test_the_workflow_name_identifies_the_run_when_the_ref_is_absent(
    tmp_path: Path,
) -> None:
    ...
    b, cat = _estate(
        ...
        {"verify-drill.yml": None},
        {"verify-drill.yml": _hourly(24, now)},
        names={"verify-drill.yml": "verify-drill"},
    )
    r = _run(
        tmp_path,
        b,
        cat,
        {"GITHUB_RUN_ID": RUN_IN_FLIGHT, "GITHUB_WORKFLOW": "verify-drill"},
    )
    assert "verify-drill.yml is the run computing this row" in r.stdout, (
        r.stdout + r.stderr
    )
...
def test_freshness_is_the_age_of_a_completed_green_run(tmp_path: Path) -> None:
    ...
    b, cat = _estate(
        tmp_path,
        [("login-drill.yml", 3)],
        {"login-drill.yml": green},
        {"login-drill.yml": _hourly(24, now)},
    )
    r = _run(tmp_path, b, cat, _in_ci())
    assert "login-drill.yml last green 1.0h ago (max 3h)" in r.stdout, (
        r.stdout + r.stderr
    )
    assert r.returncode == 0, r.stdout
    ...
    b2, cat2 = _estate(
        tmp_path / "b",
        [("login-drill.yml", 3)],
        {"login-drill.yml": stale_green},
        {"login-drill.yml": _hourly(24, now)},
    )
    r2 = _run(tmp_path / "b", b2, cat2, _in_ci())
    assert "login-drill.yml last green 9.0h ago, older than 3h" in r2.stdout, (
        r2.stdout + r2.stderr
    )
    assert r2.returncode == 1, r2.stdout


def test_a_green_run_dated_after_this_clock_is_blind_never_fresh(
    tmp_path: Path,
) -> None:
    ...
    b, cat = _estate(
        tmp_path,
        [("login-drill.yml", 3)],
        {"login-drill.yml": ahead},
        {"login-drill.yml": _hourly(24, now)},
    )
    r = _run(tmp_path, b, cat)
    assert "last green %s is 9.0h in the future" % ahead in r.stdout, (
        r.stdout + r.stderr
    )
...
def test_a_run_concluding_seconds_after_the_clock_was_read_is_still_fresh(
    tmp_path: Path,
) -> None:
...
def test_one_skewed_clock_stops_the_whole_report_not_just_its_own_row(
    tmp_path: Path,
) -> None:
    ...
    b, cat = _estate(
        tmp_path,
        [("login-drill.yml", 3), ("verify-drill.yml", 3)],
