...
def _inputs(tmp_path, rows=3):
    ...
    cat.write_text(
        "# inventory taken: 2026-08-27T13:01:19Z\napiVersion: backstage.io/v1alpha1\nkind: Component\n"
    )
...
@pytest.mark.skipif(
    not (os.path.exists(SU) and shutil.which("flux") and shutil.which("jq")),
...
def test_dry_run_builds_the_three_inputs_and_pushes_nothing(tmp_path):
    ...
    assert (
        "dry-run estate-db: 3 assets, would push oci://ghcr.io/chidionyema/idp/estate-db:latest"
        in out.stdout
    )
...
@pytest.mark.skipif(
    not (shutil.which("flux") and shutil.which("jq")), reason="needs flux and jq"
)
def test_a_missing_input_is_blind_not_a_partial_artifact(tmp_path):
    ...
    assert out.returncode == 2 and "BLIND" in out.stdout and "STATE.md" in out.stdout, (
