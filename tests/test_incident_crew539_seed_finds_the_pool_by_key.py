...
def _run(tmp_path, pools, *keys):
    ...
    r = subprocess.run(
        [str(b / "idp-autoscaler-seed"), *keys], env=env, capture_output=True, text=True
    )
    return r, out
...
def test_a_cluster_prefixed_name_is_still_found_and_the_newest_wins_after_a_surge(
    tmp_path,
):
    pools = [
...
def test_the_spot_key_never_matches_the_base_pool(tmp_path):
    ...
    r, _ = _run(
        tmp_path,
        [{"name": "a1", "id": "ocid1.nodepool.a1", "at": "2026-08-27T00:00:00Z"}],
    )
