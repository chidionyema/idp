...
def test_radio_room_set_and_nothing_else_carries_infrastructure_critical():
    ...
    carriers = {
        ...
        and p.relative_to(IDP).as_posix()
        != "platform/scheduling/require-priority-class.yaml"
    }
...
def test_ping_lives_outside_the_cluster_and_pages_the_existing_bot():
    ...
    assert (
        "secrets.SEED_HERMES_TELEGRAM_BOT_TOKEN" in text
        and "secrets.SEED_HERMES_TELEGRAM_HOME_CHANNEL" in text
    )
    assert "backstage/founder/catalog-info.yaml" in text, "one list, the portal's"
    assert "-o /dev/null" in text and "curl -sI" not in text and " -I " not in text, (
        "GET, never HEAD"
    )
    assert not re.search(r"bot\d{6,}:", text), "no bot token literal"
    assert not re.search(r"chat_id=-?\d{5,}", text), "no chat id literal"
    assert "steps.prev.outputs.conclusion == 'failure'" in text, (
        "recovery message after a red tick"
    )
