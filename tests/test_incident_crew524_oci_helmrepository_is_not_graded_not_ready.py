...
REPOS = [
    {
        ...
        "spec": {"type": "oci", "url": "oci://x"},
    },
    {
        "metadata": {"name": "broken-oci", "namespace": "a"},
        "spec": {"type": "oci", "url": "oci://y"},
        "status": {
            "conditions": [
                {"type": "Ready", "status": "False", "message": "pull denied"}
            ]
        },
    },
    {
        "metadata": {"name": "plain", "namespace": "b"},
        "spec": {"url": "https://charts.example"},
    },
]
...
def _run(tmp_path: Path) -> dict:
    ...
    src = collect.replace(
        'SA = "/var/run/secrets/kubernetes.io/serviceaccount"', f'SA = "{sa}"', 1
    )
    assert src != collect, "SA constant found"
    r = subprocess.run(
        [sys.executable, "-c", PREAMBLE % json.dumps(REPOS) + src],
        capture_output=True,
        text=True,
        timeout=60,
    )
...
def test_oci_repo_without_a_condition_is_ready_and_says_why(tmp_path):
    body = _run(tmp_path)
    rows = {
        (r["ns"], r["name"]): r for r in body["flux"] if r["kind"] == "HelmRepository"
    }
    assert rows[("hindsight", "hindsight")]["ready"] is True
    assert "no Ready condition" in rows[("hindsight", "hindsight")]["message"]
    assert not any(
        r["kind"] == "HelmRepository" and r["name"] == "hindsight"
        for r in body["flux_not_ready"]
    )


def test_a_failing_oci_repo_and_a_plain_repo_without_a_condition_stay_not_ready(
    tmp_path,
):
    body = _run(tmp_path)
    rows = {
    ...
    assert (
        rows[("a", "broken-oci")]["ready"] is False
        and "pull denied" in rows[("a", "broken-oci")]["message"]
    )
    assert (
        rows[("b", "plain")]["ready"] is False
        and rows[("b", "plain")]["message"] == "no Ready condition yet"
    )
