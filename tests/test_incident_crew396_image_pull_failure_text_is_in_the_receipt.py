...
def test_a_waiting_container_names_its_reason_and_a_starting_one_is_silent():
    ...
    assert got == {
        "init": 'ImagePullBackOff: Back-off pulling image "ghcr.io/x/y:main-727"',
    ...
    r = subprocess.run(
        [sys.executable, "-c", empty], capture_output=True, text=True, check=True
    )
...
def test_a_failed_job_pod_that_never_restarted_carries_its_current_log():
    ...
    got = json.loads(r.stdout)
    assert (
        set(got["out"]) == {"step", "looping"}
        and got["out"]["step"] == "Traceback: boom\n"
    )
    assert [u.split("container=")[1] for u in got["asked"]] == [
        "step&previous=false&tailLines=8",
        "looping&previous=true&tailLines=8",
    ]
...
def test_third_receipt_a_killed_job_pod_names_its_pod_reason_and_terminated_reason():
    ...
    quiet = {
        "phase": "Running",
        "containerStatuses": [
            {"name": "web", "restartCount": 0, "state": {"running": {}}}
        ],
    }
    prog = (
        "def failed"
