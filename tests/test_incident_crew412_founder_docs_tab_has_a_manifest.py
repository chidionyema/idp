...
def test_the_crew_board_has_a_docs_tab():
    assert any(name == "founder-crew-board" for name, _ in _refs()), (
        "founder-crew-board carries no techdocs-ref"
    )
...
def test_every_founder_techdocs_ref_names_a_tree_with_a_manifest(name: str, ref: str):
    ...
    assert m, (
        f"{name}: techdocs-ref {ref!r} is not url:https://github.com/<owner>/<repo>/tree/<ref>/"
    )
    ...
    try:
        ...
        assert e.code != 404, (
            f"{name}: {owner}/{repo}@{gitref} has no mkdocs.yml at its root (HTTP 404)"
        )
        pytest.skip(f"BLIND: GitHub answered HTTP {e.code} for {url}")
    except (urllib.error.URLError, TimeoutError) as e:
        pytest.skip(f"BLIND: GitHub unreachable: {e}")
    assert "docs_dir" in body or "site_name" in body, (
        f"{name}: {url} is not a mkdocs manifest"
    )
