...
def test_nodes_flag_min_is_one_and_max_is_estate_defaults_max_nodes():
    ...
    assert nodes, (
        cmd
    )  # crew#539 CP10 adds a second line for the preemptible pool; the first is the on-demand pool
    lo, hi, pool = nodes[0].split("=", 1)[1].split(":")
    assert lo == "1" and int(hi) == defaults["max_nodes"] and defaults["max_nodes"] >= 2
    assert pool == "$(NODEPOOL_ID)", (
        "the pool id comes from the vault-fed Secret, never a literal"
    )
...
def test_row_is_wired_and_names_no_ocid():
    for f in [
        *ROW.glob("*.yaml"),
        ROOT / "platform" / "oci" / "autoscaler.tf",
        ROOT / "bin" / "idp-autoscaler-seed",
    ]:
    ...
    env = {
        e["name"]: e
        for e in _deployment()["spec"]["template"]["spec"]["containers"][0]["env"]
    }
    assert env["NODEPOOL_ID"]["valueFrom"]["secretKeyRef"] == {
        "name": "oke-autoscaler",
        "key": "NODEPOOL_ID",
    }
    ...
    rows = [
        ...
        if d and d["metadata"]["name"] == "autoscaler"
    ]
...
def test_terraform_hands_size_over_and_moves_the_pool_instead_of_recreating_it():
    ...
    assert re.search(r"ignore_initial_pool_size\s*=\s*true", pool) and re.search(
        r"\bautoscale\s*=\s*true", pool
    )
    auto = (ROOT / "platform" / "oci" / "autoscaler.tf").read_text()
    assert re.search(
        r'moved\s*\{\s*from\s*=\s*module\.oke\.module\.workers\[0\]\.oci_containerengine_node_pool\.tfscaled_workers\["a1"\]\s*to\s*=\s*module\.oke\.module\.workers\[0\]\.oci_containerengine_node_pool\.autoscaled_workers\["a1"\]',
        auto,
    )
    for verb in [
