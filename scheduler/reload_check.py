...
def verdict(reply: str) -> tuple[int, list[str]]:
    ...
    if doc.get("errors"):
        return 1, [
            "reloadWorkspace refused: "
            + "; ".join(str(e.get("message", e))[:200] for e in doc["errors"])
        ]
    w = (doc.get("data") or {}).get("reloadWorkspace") or {}
    if w.get("__typename") != "Workspace":
        return 1, [
            f"reloadWorkspace returned {w.get('__typename') or 'nothing'}: {str(w.get('message', ''))[:200]}"
        ]
    bad = [
        e
        for e in w.get("locationEntries", [])
        if (e.get("locationOrLoadError") or {}).get("__typename") == "PythonError"
    ]
    lines = [
        f"code location {e['name']} does not load: "
        f"{e['locationOrLoadError'].get('message', '').splitlines()[0][:200]}"
        for e in bad
    ]
