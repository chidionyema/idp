...
def find_waits(text: str) -> list[tuple[float, str]]:
    ...
    for m in re.finditer(r'\bsleep\s+("?)([^\s;&|)]+)\1', text):
        ...
        if secs is not None:
            ...
            out.append(
                (float(CEILING_SEC + 1), f"sleep {m.group(2)} (unreadable duration)")
            )

    for m in re.finditer(
        r'\btimeout\s+(?:-k\s*\S+\s+)?("?)(\d+(?:\.\d+)?(?:ms|s|m|h|d)?)\1', text
    ):
        secs = to_seconds(m.group(2))
        if secs is not None:
    ...
    for m in re.finditer(r"\b(for|while|until)\b[\s\S]*?\bdone\b", text):
        ...
        bounded = re.search(
            r"\{\d+\.\.\d+\}|-lt\s+\d+|-le\s+\d+|-eq\s+\d+|seq\s+\d+\s+\d+|\b1\.\.\d+",
            body,
        )
        ...
        if forever or (external and not bounded):
            what = (
                "an unbounded loop with no terminating condition"
                if forever
                else (
...
class EstateExecutionDecree(CustomLogger):
    ...
    async def async_pre_call_hook(
        ...
        try:
            ...
            log.warning(
                "execution-decree: could not analyse a request (%s); passing it", exc
            )
            return None
