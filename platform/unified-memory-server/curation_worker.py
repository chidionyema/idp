    ...
    async with conn.cursor() as cur:
        ...
        for m in unpromoted:
            should_promote = MAPLEGuard.evaluate_promotion_gate(
                rho=m["relevance_rho"], tau=m["truthfulness_tau"], h=m["harm_h"]
            )
            if should_promote:
                await cur.execute(
                    "UPDATE memories SET is_shared = TRUE WHERE id = %s;", (m["id"],)
                )
            elif m["harm_h"] > 0.40 or m["taint_level"] > 0.70:
                # Quarantine harmful or highly tainted memories
                await cur.execute(
                    "UPDATE memories SET is_quarantined = TRUE WHERE id = %s;",
                    (m["id"],),
                )
        ...
        try:
            async with pool.connection() as conn:
                async with conn.cursor() as cur:
                    ...
                    await cur.execute(
                        "SELECT pg_try_advisory_xact_lock(%s);", (CURATION_LOCK_ID,)
                    )
