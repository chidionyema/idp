...
RUN git clone --depth 1 --branch "${LIBOQS_VERSION}" \
 ...
 && cmake --install /usr/src/liboqs/build --prefix /usr/local/liboqs \
 && test -e /usr/local/liboqs/lib/liboqs.so.0.16.0 \
...
# BUILD_SHARED_LIBS=ON IS THE FIX, and it took seven attempts to find. liboqs defaults to a STATIC
# archive: a build without this flag produces liboqs.a and NO liboqs.so at all, so `cmake --install`
# writes an include tree, a pkgconfig file and a cmake dir, and no library. ctypes.CDLL cannot load
# a static archive, so the bindings report "No oqs shared libraries found" no matter where the
# install path points. Reproduced locally 2026-09-12: build #1 produced only liboqs.a, build #2
# with this flag produced liboqs.dylib, liboqs.9.dylib and liboqs.0.16.0.dylib.
#
# THE SYMLINK IS THE SECOND HALF OF THE FIX. liboqs-python loads the UNVERSIONED name -- it calls
# LoadLibrary("<dir>/lib/liboqs.so") (oqs/oqs.py:150-157, 262-268). `cmake --install` writes
# liboqs.so.9 and liboqs.so.0.16.0; the plain liboqs.so is a symlink it creates for the linker,
# and when it is absent the bindings treat the library as missing, try to install their own copy,
# and fail with "CMake Error ... Error installing liboqs / No oqs shared libraries found" --
# measured twice in this repository's CI. It is created and asserted here so it cannot go missing.
ENV OQS_INSTALL_PATH=/usr/local/liboqs
# `--prefix` is passed to BOTH commands: a build configured with -DCMAKE_INSTALL_PREFIX and then
# installed without it writes nothing to the expected directory, which is exactly how this step
# failed with "ls: cannot access 'liboqs.so.*': No such file or directory" -- an empty target dir.
# The `test -e` above makes that failure happen in the build, not here.
RUN set -eu; \
    cd /usr/local/liboqs/lib; \
    if [ ! -e liboqs.so ]; then ln -s "$(ls liboqs.so.0.* | head -1)" liboqs.so; fi; \
    ls -l liboqs.so
